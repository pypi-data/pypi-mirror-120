...
class FrameServer():
    ...
    """
    def __init__(self, cam, frame_callback, completion_callback=None, fvb=False):
        self._cam = cam
        self._thread = None
    ...
    def start(self, nbuffers=10, fvb=None):
        """
        ...
        self._thread = Thread(target=self._run, name="Andor3_FrameServer", daemon=True)
        self._thread.start()
...
class FrameDump():
    ...
    def start(self, n_images=None, fvb=None):
        ...
        if fvb is not None:
            self._fvb = bool(fvb)
        self._thread_stop.clear()
        self._thread = Thread(target=self._run, args=(n_images,), name="Andor3_FrameDump", daemon=True)
        self._thread.start()
