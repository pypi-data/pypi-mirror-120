...
def await_function_deployment(client: ExperimentalClient, function_id: int, max_wait: int) -> bool:
    t_end = time.time() + max_wait
    while time.time() < t_end:
        function = client.functions.retrieve(id=function_id)
        if function is not None:
            print(f"Function status {function.status} - Function id {function_id}")
            if function.status == "Ready":
                return True
            if function.status == "Failed":
                return False
        else:
            print(f"API returns None for Function id {function_id}")
        time.sleep(5.0)
