...
def profiles_to_json():
    """Prints aws config file contents as json for further parsing and use in other tookls"""
    parser = argparse.ArgumentParser(description=profiles_to_json.__doc__)
    argcomplete.autocomplete(parser)
    _ = parser.parse_args()
    from collections import OrderedDict
    home = expanduser("~")
