...
def get_inflight(course, semester, username, phase, psid, taskid):
    ...
    None) if no attempts to grade the given task have been made yet.
    ...
    # try again...
    if inflight is None:
        time.sleep(0.02) # delay a tiny bit
        try:
            inflight = sync.read_file(iff, missing={})
        except Exception:
            flask.flash("Failed to read in-flight file!")
            tb = potluck.html_tools.string_traceback()
            print(
                "Failed to read in-flight file (x2):\n" + tb,
                file=sys.stderr
            )
...
def put_inflight(course, semester, username, phase, psid, taskid):
    ...
    try:
        ...
        flask.flash("Failed to read in-flight file for put_inflight!")
        tb = potluck.html_tools.string_traceback()
        print(
            "Failed to read in-flight file for put_inflight:\n" + tb,
            file=sys.stderr
        )
        return ("error", None, None, None)
...
def record_time_spent(
    ...
    if time_spent == "":
        try:
            ...
            tb = potluck.html_tools.string_traceback()
            print(
                "Failed to read time spent file:\n" + tb,
                file=sys.stderr
            )
            old_info = None
...
def get_feedback_summary(
    ...
    try:
        ...
        flask.flash("Failed to summarize feedback file for summary.")
        return fallback
...
def get_feedback(
    ...
    if ts is None: # No submission
        ...
        flask.flash("Failed to read in-flight file; please refresh the page.")
        tb = potluck.html_tools.string_traceback()
        print(
            "Error reading in-flight file:\n" + tb,
            file=sys.stderr
        )
        result = "missing"
    ...
    if result != "missing":
        try:
            result = sync.read_file(
                ...
                AsFeedbackHTML,
                missing="missing", # a string if there is no such file
                cache=False
