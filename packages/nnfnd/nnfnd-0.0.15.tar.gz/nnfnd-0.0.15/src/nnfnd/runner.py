...
class InferHandler(tornado.web.RequestHandler):
    ...
    @staticmethod
    def get_dir_with_screenshots(paths):
        logging.info(f"get_dir_with_screenshots [1]: {len(paths)} | {paths}")
        for p in paths:
            files = os.listdir(p)
            logging.info(f"get_dir_with_screenshots [2]: {len(files)} | {files}")
            if len(files) >= 5:
                return p
        # return ""
    ...
    def post(self):
        ...
        logging.info("archive_filename: ", archive_filename)
        ...
        # input_path = self.get_dir_with_screenshots([upload_dir_with_ts, await_dir_with_screenshots])
        ...
        body = self.run_model(input_path=input_path, output_path=upload_dir_with_ts)
        if body is None:
            self.set_status(204)
            self.finish()
            return
        ...
        # TODO: input_path, upload_dir_with_ts - rm all files
