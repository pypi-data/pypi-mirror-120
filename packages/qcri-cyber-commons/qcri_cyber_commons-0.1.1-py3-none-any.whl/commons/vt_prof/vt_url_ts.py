'''
Description: VT URL TS - create a summary of VT URL records. Only consider FQDNs (not URLs)
Author: @nabeelxy
Note:
...
BASE_VT_DIR = "/export/sec03/vt/vt_url_feed_parsed"

#Two issues are identified in the current VT parsed data
#.domain contains only domain queries
#s is is redacted from domains names starting with s (bug)
...
def update(filename, databasename):
    ...
    fn = open(filename)
    for line in fn:
        #boosterads.online##N##2019-08-04 16:58:49##0 0##2019-08-04 16:58:49
        values = line.split("##")
        if (len(values) != 5):
        ...
        if url == None or len(url) == 0:
            print("WARNING: empty url: {}".format(line))
            continue
        arr = url.split("/")
        domain = arr[0]
