...
def filter_entities(spans, ne_exclusion_types):
    """remove inter-overlapped spans: (1)first longest match rule.
    (2)multiple label token  use pattern match to split

    NOTE: this function has a prolbem: ne_exclusion_types doesn't do the right job!
    """
    span_len = len(spans)
    if span_len == 0:
        return ([], [])
    ...
    m_ents = []
    while stop == False:
        result, m_ents = [sorted_spans[0]], []
        for i, span in enumerate(sorted_spans[1:]):
            if span.start == result[-1].start:
                if span.end > result[-1].end:
                    # span include result[-1]
                    result[-1] = span
                    # remove m_ents
                    for j, e in enumerate(m_ents):
                        if e.start == span.start and e.end < span.end:
                            m_ents = m_ents[0:j]
                            break
                elif span.end == result[-1].end:
                    # span == result[-1]
                    if result[-1].label_ not in ne_exclusion_types:
                        m_ents.append(result[-1])
                    if span.label_ not in ne_exclusion_types:
                        m_ents.append(span)
                    if span.label_ in ne_exclusion_types:
                        # save entity if equal length type
                        result[-1] = span
            elif span.start < result[-1].end:
                if span.end - span.start > result[-1].end - result[-1].start:
                    # span overlap result[-1] and longer
                    sorted_spans.pop(i)
                    break
            else:
                result.append(span)
