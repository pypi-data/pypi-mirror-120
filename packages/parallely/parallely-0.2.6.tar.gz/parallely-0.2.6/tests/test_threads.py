...
def test_uneven_iterators(multi_kwarg):
    ...
    assert list(multi_kwarg.acmap([1, 1], [1])) == [2]
    assert list(multi_kwarg.acmap(a=[1, 1], b=[1])) == [2]
    assert list(multi_kwarg.acmap([1], b=[1, 1])) == [2]
...
def test_iterators_dicts():
    ...
    assert list(test.acmap([1, 1], dict_like={"one": 1})) == [1, 1]

def test_big_list(single_arg):
    single_arg = convert(single_arg)
    
    args = list(range(100000))
    assert single_arg.map(args) == args
