...
def assert_valid_thermo_output_format(output_format: str, logger: log.Logger = log.DEFAULT_LOGGER) -> \
    ...
    logger.assert_true(output_format in get_thermo_raw_file_parser_output_formats(),
                       f"Invalid output format '{output_format}'. Currently allowed formats are: "
                       f"[{get_string_of_thermo_raw_file_parser_output_formats()}]")
    return None
...
def convert_raw_file(filename: Optional[str],
    ...
    base_filename, file_ext = separate_extension(filename=filename,
                                                 extensions={"raw"})
    if len(file_ext) == 0:
        logger.debug(f"Cannot convert file '{filename}', unknown extension")
        return None

    converted_filename = base_filename + "." + output_format

    if skip_existing and os.path.isfile(converted_filename):
        logger.info(f"Skipping conversion, because '{converted_filename}' already exists")
    ...
    logger.assert_true(utils.is_docker_container_running(thermo_docker_container_name),
                       ...
                       f"'{thermo_docker_container_name}'")
    ...
    logger.debug(f"Converting file using command '{command}'")
    return_code = os.system(command)
    if return_code == 0:
        logger.info(f"Converted file '{filename}'")
        return converted_filename
    else:
        logger.warning(f"Failed converting file '{filename}' (return code = {return_code})")
        return None
