...
class Askdata(SignUp):
    ...
    def join(self, df:pd.DataFrame, with_query:str, workspace:str, how="inner", on=None, left_on=None, right_on=None, lang="en"):
        ...
        if (on == None and (left_on == None and right_on == None)):
            join_conditions = self.__get_join_info()["join_list"][0]
    ...
    def __get_join_info(self, df1:pd.DataFrame, df2:pd.DataFrame):
        ...
        columns = []
        for field in json.loads(df1.to_json(orient="table"))["schema"]["fields"]:
        ...
        columns2 = []
        for field in json.loads(df2.to_json(orient="table"))["schema"]["fields"]:
            field["label"] = field.pop("name")
            field["type"] = self.__choose_type(field.pop("type"))
        ...
        try:
            ...
            if(len(filtered_result)>1):
                ...
                for r in filtered_result[1:]:
                    ...
                    display(HTML("<p>     {} from the dataset: {} / {} ({}) <- Use this source</p>".format(self.extract_fields(r["attachment"]["body"][0]["details"]["columns"]), workspace, r["attachment"]["body"][0]["details"]["datasetName"], url_i)))
