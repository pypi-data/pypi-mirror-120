...
def sf_path(nshot, diag, exp='AUGD', ed=0):
    ...
    if exp == 'augd':
        sf_dir = '/afs/ipp/home/a/augd/shots/%s' %(sshot[:4])
        a = os.listdir(sf_dir)
        path_out = None
        for subdir in a:
        ...
        path1 = '/afs/ipp/home/%s/%s/shotfiles/%s/%s' %(exp[0], exp, diag, sshot[:2])
        if os.path.isdir(path1):
            if not os.path.isfile('%s/ed_cntl' %path1):
...
def ed_zero(diag, nshot, exp='augd'):
    ...
    if exp == 'augd':
        ...
        fed_dir = '/afs/ipp/home/%s/%s/shotfiles/%s/%s' %(exp[0], exp, diag, sshot[:2])
        nshot = nshot % 1000
...
if __name__ == '__main__':
    ...
    fed_in = '/afs/ipp/home/g/git/shotfiles/TRA/35'
    ed = read_ed_cntl(fed_in, exp='git')
    print(fed_in)
    print(ed)
    fed_in = '/afs/ipp/home/a/augd/shots/3956/L1/NSP'
    fed_in = '/afs/ipp/home/a/augd/shots/3956/L1/TOT'
