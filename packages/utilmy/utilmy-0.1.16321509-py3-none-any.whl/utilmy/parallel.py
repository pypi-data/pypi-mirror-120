...
def test_benchmark():
    """Benchmarking of functions using timeit"""
    import time
    # pd_random benchmark
    start = time.time()
    res = pd_random()
    end = time.time()
    print('pd_random ', end - start)

    #pd_groupby_parallel benchmark
    df = pd.DataFrame(data={'result':[5, 8, 1, 7, 0, 3, 2, 9, 4, 6],
                            'user_id':[1, 1, 2, 3, 4, 4, 5, 8, 9, 9],
                            'value'  :[27, 14, 26, 19, 28, 9, 11, 1, 26, 18],
                            'data_chunk':[1, 1, 2, 3, 4, 4, 5, 8, 9, 9]})
    start = time.time()
    res = pd_groupby_parallel(df.groupby("user_id"), func=test_fun_apply, npool=50)
    end = time.time()
    print('\npd_groupby_parallel with npool = 5, ' ,end - start)
    ...
    df = pd.DataFrame(data={'result':[5, 8, 1, 7, 0, 3, 2, 9, 4, 6],
                            ...
                            'data_chunk':[1, 1, 2, 3, 4, 4, 5, 8, 9, 9]})
    start = time.time()
    res = pd_groupby_parallel(df.groupby("user_id"), func=test_fun_apply, npool=1)
    end = time.time()
    print('\npd_groupby_parallel with npool = 0, ' ,end - start)

    #pd_groupby_parallel2 benchmark -> AttributeError: 'DataFrameGroupBy' object has no attribute 'groupby'
    df = pd.DataFrame(data={'result':[5, 8, 1, 7, 0, 3, 2, 9, 4, 6],
                            'user_id':[1, 1, 2, 3, 4, 4, 5, 8, 9, 9],
                            'value'  :[27, 14, 26, 19, 28, 9, 11, 1, 26, 18],
                            'data_chunk':[1, 1, 2, 3, 4, 4, 5, 8, 9, 9]})
    # start = time.time()
    # res = pd_groupby_parallel2(df.groupby("user_id"), func=test_fun_apply, npool=5)
    # end = time.time()
    # print('pd_groupby_parallel2 ' , end - start)

    # # pd_read_file2 benchmark
    start = time.time()
    df= pd.DataFrame(np.random.randint(0,100,size=(100, 4)), columns=list('ABCD'))
    df.to_csv('mytest.csv')
    res = pd_read_file2('mytest.csv')
    end = time.time()
    print('\npd_read_file2 ' ,end - start)

    # #pd_read_file benchmark
    start = time.time()
    res = pd_read_file2('mytest.csv')
    end = time.time()
    print('\npd_read_file ' ,end - start)
    ...
    start = time.time()
    res = pd_apply_parallel(df, addition)
    end = time.time()
    print('\npd_apply_parallel ' ,end - start)
    ...
    print('\nmultithread_run ' ,end - start)

    # #multiproc_run -> AttributeError: Can't pickle local object 'test_benchmark.<locals>.fun_async'
    # start = time.time()
    # res = multiproc_run(fun_async, li_of_tuples, npool=2, start_delay=0.1, verbose=True)
    # end = time.time()
    # print('\nmultiproc_run ' ,end - start)

    # #multithread_run_list benchmark
    def test_print(x):
        print(x)
