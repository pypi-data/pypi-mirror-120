...
class TableJoins:
    ...
    last24_hours: Optional[List[TableJoin]] = None
    last30_days: Optional[List[TableJoin]] = None
    last365_days: Optional[List[TableJoin]] = None
    last7_days: Optional[List[TableJoin]] = None
    last90_days: Optional[List[TableJoin]] = None

    @staticmethod
    def from_dict(obj: Any) -> 'TableJoins':
        assert isinstance(obj, dict)
        last24_hours = from_union([lambda x: from_list(TableJoin.from_dict, x), from_none], obj.get("last24Hours"))
        last30_days = from_union([lambda x: from_list(TableJoin.from_dict, x), from_none], obj.get("last30Days"))
        last365_days = from_union([lambda x: from_list(TableJoin.from_dict, x), from_none], obj.get("last365Days"))
        last7_days = from_union([lambda x: from_list(TableJoin.from_dict, x), from_none], obj.get("last7Days"))
        last90_days = from_union([lambda x: from_list(TableJoin.from_dict, x), from_none], obj.get("last90Days"))
        return TableJoins(last24_hours, last30_days, last365_days, last7_days, last90_days)

    def to_dict(self) -> dict:
        result: dict = {}
        result["last24Hours"] = from_union([lambda x: from_list(lambda x: to_class(TableJoin, x), x), from_none], self.last24_hours)
        result["last30Days"] = from_union([lambda x: from_list(lambda x: to_class(TableJoin, x), x), from_none], self.last30_days)
        result["last365Days"] = from_union([lambda x: from_list(lambda x: to_class(TableJoin, x), x), from_none], self.last365_days)
        result["last7Days"] = from_union([lambda x: from_list(lambda x: to_class(TableJoin, x), x), from_none], self.last7_days)
        result["last90Days"] = from_union([lambda x: from_list(lambda x: to_class(TableJoin, x), x), from_none], self.last90_days)
        return result
