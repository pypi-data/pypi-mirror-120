...
from modelingDash.configModelisation import ThermicIndicateurs
...
class SmallPowerMaster():
    ...
    def rendementCondenseur(self,timeRange_Window,fuites=0,nb='01',wholeDF=False,**kwargs):
        ...
        if not df.empty:
            ...
            dfTh['refroidissement vapeur'] = (dfTh['debit g/s(yc fuites)']*self.ind.Cp_eau_vap*(df['t_entree_condenseur']-100)).apply(lambda x :max(0,x))
            dfTh['condensation'] = dfTh['debit g/s(yc fuites)']*self.ind.Cl_H2O
            dfTh['Refroidissement eau condensée'] = (dfTh['debit g/s(yc fuites)']*self.ind.Cp_eau_liq*(100-df['t_sortie_condenseur'])).apply(lambda x :max(0,x))
            dfTh['thermique azote'] = 3.9/60/22.4*self.ind.Mmol_N2*self.ind.Cp_N2*(df['t_entree_condenseur']-df['t_sortie_condenseur'])*(1-fuites)
    ...
    def bilanValo(self,timeRange_Window,nb='01',wholeDF=False,**kwargs):
        ...
        if not df.empty:
            ...
            dfValo['echange valo'] = dfValo['debit eau(g/s)']*self.ind.Cp_eau_liq*(df['t_sortie_valo']-df['t_entree_valo'])
            df.columns=[k + ' : ' + l  for k,l in zip(df.columns,listTags)]
    ...
    def rendement_GV(self,timeRange_Window,activePower=True,wholeDF=False,**kwargs):
        ...
        if not df.empty:
            ...
            dfGV['power chauffe eau liq']  = (dfGV['debit eau(g/s)']*self.ind.Cp_eau_liq*(100-df['t_entree_GV'])).apply(lambda x :max(0,x))
            dfGV['power vaporisation eau'] = dfGV['debit eau(g/s)']*self.ind.Cl_H2O
            dfGV['power chauffe vapeur']   = (dfGV['debit eau(g/s)']*self.ind.Cp_eau_vap*(df['t_sortie_GV']-100)).apply(lambda x :max(0,x))
    ...
    def pertesThermiquesStack(self,timeRange_Window,fuel='N2',wholeDF=False,activePower=True,**kwargs):
        ...
        if not df.empty:
            ...
            cp_fuel,M_fuel = self.ind.dfConstants.loc['Cp_' + fuel,'value'],self.ind.dfConstants.loc['Mmol_' + fuel,'value']
            dfPertes['surchauffe_Air']  = (df['TstackAir']-df['air_entreeStack'])*self.ind.Cp_air*self.ind.Mmol_Air*df['debitAir(Nl/mn)']/22.4/60
            dfPertes['surchauffe_Fuel'] = (df['TstackFuel']-df['fuel_entreeStack'])*cp_fuel*M_fuel*df['debitFuel(Nl/mn)']/22.4/60
            dfPertes['surchauffe_AirBalayage'] = (df['TstackAir']-df['air_entreeStack'])*self.ind.Cp_air*self.ind.Mmol_Air*df['debitAir(Nl/mn)']/22.4/60
...
class ConfigFilesSmallPowerSpark(ConfigDashSpark):
    def __init__(self,sparkData,sparkConfFile,confFile=None,encode='utf-8'):
        ...
        super().__init__(sparkData,sparkConfFile,confFile=confFile)
...
class AnalysisPerModule(ConfigFilesSmallPower):
    def __init__(self,folderPkl,encode='utf-8'):
        super().__init__(folderPkl,encode)
...
class ConfigFilesSmallPower_RealTime(ConfigDashRealTime,SmallPowerMaster):
    ...
    def __init__(self,confFolder=None,connParameters=None,encode='utf-8'):
        self.appDir  = os.path.dirname(os.path.realpath(__file__))
