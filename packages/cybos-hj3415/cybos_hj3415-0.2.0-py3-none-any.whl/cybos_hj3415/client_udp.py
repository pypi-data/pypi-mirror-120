...
class Command:
    def __init__(self):
        ...
        if self.setting['udp_port'] != '':
            self.addr = (self.setting['addr'], self.setting['udp_port'])
        else:
    ...
    def account(self) -> AccountData:
        ...
        try:
            udp_sock.sendto(CMD.ACCOUNT.encode(), self.addr)
    ...
    def _code_manager(self, code: str, cmd: CMD):
        ...
        try:
            if cmd not in [CMD.CMGR_GET_NAME, CMD.CMGR_GET_STATUS, CMD.CMGR_IS_KOSPI200]:
                raise ValueError
            udp_sock.sendto(f'{cmd}/{code}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
    ...
    def cprice(self, code) -> CurrentPriceData:
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{CMD.CPRICE}/{code}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
    ...
    def inquire_order(self) -> list:
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(CMD.IORDER.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
    ...
    def buy_order(self, code: str, amount: int, price: int):
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{CMD.BUY}/{code}/{amount}/{price}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
    ...
    def sell_order(self, code: str, amount: int, price: int):
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{CMD.SELL}/{code}/{amount}/{price}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def cancel_one(self, ordernum: str):
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{CMD.CANCEL}/one/{ordernum}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def cancel_all(self, code: str):
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{CMD.CANCEL}/all/{code}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def get_market_info(self) -> MarketData:
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(CMD.MARKET_INFO.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
    ...
    def get_top_n_stocks(self, market: str = 'all', n: int = 5) -> TopStocksData:
        ...
        udp_sock = socket(AF_INET, SOCK_DGRAM)
        try:
            udp_sock.sendto(f'{CMD.TOP_STOCKS}/{market}/{n}'.encode(), self.addr)
            raw_data, addr = udp_sock.recvfrom(setting.UDP_BUFSIZ)
