...
class Command:
    def __init__(self):
        ...
        if self.setting['tcp_port'] != '':
            self.addr = (self.setting['addr'], self.setting['tcp_port'])
        else:
    ...
    def account(self) -> AccountData:
        ...
        try:
            tcp_sock.sendall(CMD.ACCOUNT.encode())
    ...
    def cprice(self, code) -> CurrentPriceData:
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(f'{CMD.CPRICE}/{code}'.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
    ...
    def inquire_order(self) -> list:
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(CMD.IORDER.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
    ...
    def buy_order(self, code: str, amount: int, price: int):
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(f'{CMD.BUY}/{code}/{amount}/{price}'.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
    ...
    def sell_order(self, code: str, amount: int, price: int):
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(f'{CMD.SELL}/{code}/{amount}/{price}'.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def cancel_one(self, ordernum: str):
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(f'{CMD.CANCEL}/one/{ordernum}'.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def cancel_all(self, code: str):
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(f'{CMD.CANCEL}/all/{code}'.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
            result = pickle.loads(raw_data)
            logger.info(result)
        finally:
    ...
    def get_market_info(self) -> MarketData:
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(CMD.MARKET_INFO.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
    ...
    def get_top_n_stocks(self, market: str = 'all', n: int = 5) -> TopStocksData:
        ...
        tcp_sock.connect(self.addr)
        try:
            tcp_sock.sendall(f'{CMD.TOP_STOCKS}/{market}/{n}'.encode())
            raw_data = tcp_sock.recv(setting.TCP_BUFSIZ)
