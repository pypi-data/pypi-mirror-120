...
class ImpedanceController(ArmPlugin):
  ...
  __slots__ = (
    ...
    '_kp', '_kd', '_gains_in_end_effector_frame')
  ...
  def __init__(self, gains_in_end_effector_frame=False):
    ...
    self._kp = np.zeros(6, dtype=np.float64)
    self._kd = np.zeros(6, dtype=np.float64)
  ...
  def set_kp(self, x, y, z, roll, pitch, yaw):
    """
    Sets the proportional gains for the impedance controller.
    Units are (N/m) or (Nm/rad).
  ...
  def set_kd(self, x, y, z, roll, pitch, yaw):
    """
    Sets the damping gains for the impedance controller.
    Units are (N/(m/sec)) or (Nm/(rad/sec)).
