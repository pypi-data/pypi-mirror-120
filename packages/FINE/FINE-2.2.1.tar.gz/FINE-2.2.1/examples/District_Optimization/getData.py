...
def getData(engine='openpyxl'):
    ...
    # Locations
    locations = set(pd.read_excel(os.path.join(inputDataPath, 'Locations.xlsx'), engine=engine))

    data.update({'locations': locations})

    # PV data
    gen_capacityMax = pd.read_excel(os.path.join(inputDataPath, 'PV_Capacity.xlsx'),
                                    sheet_name ='GenerationCapacities', index_col=0, engine=engine)
    gen_capacityMax = gen_capacityMax.loc['PV_south'].T
    gen_operationRateMax = pd.read_excel(os.path.join(inputDataPath, 'PV_Generation.xlsx'), engine=engine)

    data.update({'PV, capacityMax': gen_capacityMax})
    data.update({'PV, operationRateMax': gen_operationRateMax})
    
    # Heat & Battery Storage
    st_capacityMax = pd.read_excel(os.path.join(inputDataPath, 'Storage_capacities.xlsx'), index_col=0, engine=engine)
    ...
    tr_distances_el = pd.read_excel(os.path.join(inputDataPath, 'grid_length_matrix.xlsx'), index_col=0, engine=engine)
    tr_capacityFix_el = pd.read_excel(os.path.join(inputDataPath, 'grid_capacity_matrix.xlsx'), index_col=0, engine=engine)
    ...
    # Denand Data
    Edemand_operationRateFix = pd.read_excel(os.path.join(inputDataPath, 'E_Demand.xlsx'), engine=engine)
    Hdemand_operationRateFix = pd.read_excel(os.path.join(inputDataPath, 'Heat_Demand.xlsx'), engine=engine)
    ...
    # Purchase Data
    Pu_operationRateMax_El = pd.read_excel(os.path.join(inputDataPath, 'purchaseElectricity.xlsx'), index_col=0, engine=engine)
    Pu_operationRateMax_NG = pd.read_excel(os.path.join(inputDataPath, 'purchaseNaturalGas.xlsx'), index_col=0, engine=engine)
