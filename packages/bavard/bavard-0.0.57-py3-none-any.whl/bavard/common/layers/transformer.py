...
class Encoder(layers.Layer):
    def __init__(
        ...
        **_
        ...
        """
        An implementation of a Transformer encoder block, with an additional
        option for performing attentive pooling over the final outputs, in case
        the desired task is to perform some kind of sequence-level prediction,
        rather then token-level predictions.
        """
        super().__init__()
        ...
        if not self.return_sequences:
            self.pooler = GlobalAttentionPooling1D(units)
