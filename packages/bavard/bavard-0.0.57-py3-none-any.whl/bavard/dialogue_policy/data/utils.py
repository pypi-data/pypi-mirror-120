...
class LabelBinarizer:
    ...
    as there are classes. Also, can handle `None` values.
    """
    ...
    def inverse_transform(self, y: np.ndarray) -> np.ndarray:
        ...
        labels = self._encoder.inverse_transform(indices).astype(np.object)
        # The `argmax` erroneously assigns the first label for encoded label
        # rows that were all 0, when those rows actually correspond to the `None` label.
        labels[np.equal(y, 0).all(axis=1)] = None
        return labels
...
class TypeEncoder(t.Generic[_T], ABC):
    """
    Generic base class for utility classes that convert other classes into numpy arrays. Provides default
    functionality for null numpy encoding.
    """

    @staticmethod
    @abstractmethod
    def encode(obj: _T, enc_context: EncodingContext) -> t.Union[np.ndarray, t.Dict[str, np.ndarray]]:
        """Encodes `obj` into a numpy array representation.
        """
        pass
    ...
    def get_encoding_shape(enc_context: EncodingContext) -> t.Union[t.Sequence[int], t.Dict[str, t.Sequence[int]]]:
        """Returns the shape an encoding of type _T will have, given an encoding context.
        """
        pass
