...
class LSTMBlocks(Layer):
    ...
    def __init__(
        ...
        **_
        ...
        # Intermediate blocks should always return their sequences, so subsequent blocks can
        # have all contextual information available.
        return_sequences_flags = [True] * self.num_blocks
        if self.num_blocks > 0:
            return_sequences_flags[-1] = self.return_sequences

        self.blocks = tf.keras.Sequential([
            LSTMBlock(units, dropout, l2, return_sequences_flag) for return_sequences_flag in return_sequences_flags
        ])
