...
from bavard_ml_common.types.agent import AgentConfig
...
class ChatbotPipeline(WebService, Persistent):
    ...
    def fit(self, agent: AgentConfig):
        # Set the language field.
        self.language = agent.language

        # Train the ML models.
        self._nlu_model.train(agent.to_nlu_dataset())
        if len(agent.trainingConversations) > 0:
            self._dp_model.fit(agent.to_conversation_dataset())
            self._do_dp = True
        else:
            self._do_dp = False
        self._fitted = True
