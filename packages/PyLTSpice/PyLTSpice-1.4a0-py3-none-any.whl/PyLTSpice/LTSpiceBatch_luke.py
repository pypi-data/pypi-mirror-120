...
###############################################################################
# Location of .exe within wine, WINEDEBUG minimises error/warning messages 
LTspice_exe = [r"WINEDEBUG=-all wine C:/'Program Files'/LTC/LTspiceXVII/XVIIx64.exe"]
...
##############################################################################
        # unix seems to need str sent to terminal
        command_str = ' '.join(map(str, command))
        # unix needs shell="True" to execute terminal commands
...
##############################################################################
        # Get filename of generated netlist without the location
        # Couldn't run the .net file given the absolute path
        netlist_filename = os.path.basename(self.netlist_file)
        cmd_run =  LTspice_exe + LTspice_arg.get('run') + [netlist_filename] + cmdline_switches
...
if __name__ == "__main__":
    ...
    # select spice model
    LTC = SimCommander(meAbsPath + "\\testfile.asc")
