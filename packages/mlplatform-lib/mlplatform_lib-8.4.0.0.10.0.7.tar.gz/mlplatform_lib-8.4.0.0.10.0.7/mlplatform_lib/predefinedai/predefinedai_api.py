...
class PredefinedAIApi:
    ...
    def upload_inference_csv(self, inference_csv_path: str):
        if self.api_client.run_mode == RunMode.KUBERNETES:
            ...
            do_info_list = self.hyperdata_api.hyperdata_client.get_do_list_by_datasource_id(
                auth=self.hyperdata_api.hyperdata_auth,
                datasource_id=self.hyperdata_api.hyperdata_client.get_default_datasource_id(
                    auth=self.hyperdata_api.hyperdata_auth
                )
            )
            table_name = self._get_inference_result_table_name()

            inference_do_info = None
            for do_info in do_info_list.data_object_info_list:
                if do_info.name == table_name:
                    inference_do_info = do_info
                    self.target_do_id = int(do_info.id)
                    break

            if inference_do_info is None:
                self.target_do_id = None

            if self.target_do_id is None or self.target_do_id == "":
