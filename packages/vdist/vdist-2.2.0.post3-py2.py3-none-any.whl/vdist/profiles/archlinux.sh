...
# pacman-key command is included because this message at Archlinux Docker Hub page:
#   NOTE: For Security Reasons, these images strip the pacman lsign key. This is
#   because the same key would be spread to all containers of the same image,
#   allowing for malicious actors to inject packages (via, for example, a
#   man-in-the-middle). In order to create an lsign-key run pacman-key --init
#   on the first execution, but be careful to not redistribute that key
pacman-key --init

# We need to refresh keyring.
# https://bbs.archlinux.org/viewtopic.php?id=242639
...
if $setup; then
    ...
    {% endif %}
    cp {{package_tmp_root}}/*.pkg.tar.zst {{shared_dir}}
