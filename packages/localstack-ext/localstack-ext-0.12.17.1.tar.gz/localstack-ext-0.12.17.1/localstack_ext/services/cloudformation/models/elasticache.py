...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=eDPuM(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not eDPuY
   return result
  except eDPuE:
   props=self.props
   ...
   if parts[0]in candidates and eDPuJ(parts)>1:
