...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or yOwak
 @yOwaP
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return yOwau(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @yOwai
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if yOwaB(origins,yOwap)else origins.get("Items")
  origins_to_create=yOwaD([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if yOwaB(aliases,yOwap)else aliases.get("Items")
  aliases_to_create=yOwaD(aliases)
  for distr in distrs:
   origins=yOwaD([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=yOwaD(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",yOwaj)
   config["Enabled"]=yOwaF(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if yOwaB(custom_errors,yOwaC)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=yOwaF(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":yOwaj}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=yOwaF(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=yOwak):
    parent=config if parent is yOwak else parent
    items=parent.get(key)or[]
    if yOwaB(items,yOwaC)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":yOwaR(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if yOwaB(cache_behavs,yOwaC)else cache_behavs)
