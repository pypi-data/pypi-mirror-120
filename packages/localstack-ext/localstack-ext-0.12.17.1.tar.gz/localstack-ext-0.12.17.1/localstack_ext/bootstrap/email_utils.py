...
SENT_EMAILS=[]
EMAIL_BLACKLIST=rYJNo()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",rYJNh(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=rYJNg,from_name=rYJNg,smtp_host=rYJNg,smtp_user=rYJNg,smtp_pass=rYJNg,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if rYJNF(recipients,rYJNu)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=rYJNg):
 ...
 result["From"]=formataddr((rYJNh(Header(from_name,"utf-8")),from_email))
 return result
