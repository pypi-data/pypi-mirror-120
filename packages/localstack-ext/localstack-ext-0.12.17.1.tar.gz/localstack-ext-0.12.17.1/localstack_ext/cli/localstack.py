...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return BCxAn if e else BCxAQ
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->BCxAG:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(BCxAG)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:BCxAz):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,BCxAG)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:BCxAz):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,BCxAG)
def print_table(columns:Dict[BCxAz,BCxAz],rows:List[Dict[BCxAz,Any]],formatters:Dict[BCxAz,Callable[[Any],BCxAz]]=BCxAG):
 from rich.table import Table
 if formatters is BCxAG:
  formatters=BCxAI()
 t=Table()
