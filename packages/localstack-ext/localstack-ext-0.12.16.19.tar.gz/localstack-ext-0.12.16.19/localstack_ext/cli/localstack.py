...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return lMgWA if e else lMgWa
 def attach(self,cli:LocalstackCli)->lMgWH:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(lMgWH)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:lMgWx):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,lMgWH)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:lMgWx):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,lMgWH)
def print_table(columns:Dict[lMgWx,lMgWx],rows:List[Dict[lMgWx,Any]],formatters:Dict[lMgWx,Callable[[Any],lMgWx]]=lMgWH):
 from rich.table import Table
 if formatters is lMgWH:
  formatters=lMgWX()
 t=Table()
