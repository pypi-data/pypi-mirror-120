...
SENT_EMAILS=[]
EMAIL_BLACKLIST=XWezP()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",XWezi(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=XWezS,from_name=XWezS,smtp_host=XWezS,smtp_user=XWezS,smtp_pass=XWezS,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if XWezd(recipients,XWezV)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=XWezS):
 ...
 result["From"]=formataddr((XWezi(Header(from_name,"utf-8")),from_email))
 return result
