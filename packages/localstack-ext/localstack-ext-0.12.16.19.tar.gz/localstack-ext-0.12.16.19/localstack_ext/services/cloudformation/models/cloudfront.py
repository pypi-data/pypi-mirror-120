...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or OtuTE
 @OtuTg
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return OtuTQ(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @OtuTb
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if OtuTi(origins,OtuTD)else origins.get("Items")
  origins_to_create=OtuTJ([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if OtuTi(aliases,OtuTD)else aliases.get("Items")
  aliases_to_create=OtuTJ(aliases)
  for distr in distrs:
   origins=OtuTJ([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=OtuTJ(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",OtuTS)
   config["Enabled"]=OtuTM(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if OtuTi(custom_errors,OtuTj)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=OtuTM(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":OtuTS}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=OtuTM(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=OtuTE):
    parent=config if parent is OtuTE else parent
    items=parent.get(key)or[]
    if OtuTi(items,OtuTj)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":OtuTy(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if OtuTi(cache_behavs,OtuTj)else cache_behavs)
