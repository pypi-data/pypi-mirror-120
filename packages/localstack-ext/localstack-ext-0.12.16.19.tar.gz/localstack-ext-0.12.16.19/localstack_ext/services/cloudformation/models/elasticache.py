...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=xTihJ(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not xTihy
   return result
  except xTihK:
   props=self.props
   ...
   if parts[0]in candidates and xTihN(parts)>1:
