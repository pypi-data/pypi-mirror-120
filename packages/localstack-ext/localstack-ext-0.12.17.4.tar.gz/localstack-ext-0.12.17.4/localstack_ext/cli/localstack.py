...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return WXGpt if e else WXGpH
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->WXGpq:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(WXGpq)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:WXGpP):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,WXGpq)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:WXGpP):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,WXGpq)
def print_table(columns:Dict[WXGpP,WXGpP],rows:List[Dict[WXGpP,Any]],formatters:Dict[WXGpP,Callable[[Any],WXGpP]]=WXGpq):
 from rich.table import Table
 if formatters is WXGpq:
  formatters=WXGpm()
 t=Table()
