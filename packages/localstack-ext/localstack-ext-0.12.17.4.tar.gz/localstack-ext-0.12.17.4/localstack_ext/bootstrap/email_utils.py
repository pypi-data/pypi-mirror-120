...
SENT_EMAILS=[]
EMAIL_BLACKLIST=OEKhn()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",OEKhH(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=OEKhb,from_name=OEKhb,smtp_host=OEKhb,smtp_user=OEKhb,smtp_pass=OEKhb,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if OEKhj(recipients,OEKhe)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=OEKhb):
 ...
 result["From"]=formataddr((OEKhH(Header(from_name,"utf-8")),from_email))
 return result
