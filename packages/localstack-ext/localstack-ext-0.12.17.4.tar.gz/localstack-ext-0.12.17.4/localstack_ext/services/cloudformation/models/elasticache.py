...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=zWdQj(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not zWdQl
   return result
  except zWdQL:
   props=self.props
   ...
   if parts[0]in candidates and zWdQU(parts)>1:
