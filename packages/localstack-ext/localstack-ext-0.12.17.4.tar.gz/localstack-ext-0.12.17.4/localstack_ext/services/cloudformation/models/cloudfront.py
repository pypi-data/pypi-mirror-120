...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or aoLlm
 @aoLlQ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return aoLld(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @aoLlN
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if aoLlz(origins,aoLlg)else origins.get("Items")
  origins_to_create=aoLlf([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if aoLlz(aliases,aoLlg)else aliases.get("Items")
  aliases_to_create=aoLlf(aliases)
  for distr in distrs:
   origins=aoLlf([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=aoLlf(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",aoLlu)
   config["Enabled"]=aoLlU(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if aoLlz(custom_errors,aoLlV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=aoLlU(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":aoLlu}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=aoLlU(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=aoLlm):
    parent=config if parent is aoLlm else parent
    items=parent.get(key)or[]
    if aoLlz(items,aoLlV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":aoLlE(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if aoLlz(cache_behavs,aoLlV)else cache_behavs)
