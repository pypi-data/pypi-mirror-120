...
SENT_EMAILS=[]
EMAIL_BLACKLIST=BuwfF()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",BuwfX(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=Buwfh,from_name=Buwfh,smtp_host=Buwfh,smtp_user=Buwfh,smtp_pass=Buwfh,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if Buwfx(recipients,BuwfL)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=Buwfh):
 ...
 result["From"]=formataddr((BuwfX(Header(from_name,"utf-8")),from_email))
 return result
