...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def is_active(self):
  ...
  return CyKOi if e else CyKOR
 def attach(self,cli:LocalstackCli)->CyKOY:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(CyKOY)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:CyKOt):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,CyKOY)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:CyKOt):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,CyKOY)
def print_table(columns:Dict[CyKOt,CyKOt],rows:List[Dict[CyKOt,Any]],formatters:Dict[CyKOt,Callable[[Any],CyKOt]]=CyKOY):
 from rich.table import Table
 if formatters is CyKOY:
  formatters=CyKOI()
 t=Table()
