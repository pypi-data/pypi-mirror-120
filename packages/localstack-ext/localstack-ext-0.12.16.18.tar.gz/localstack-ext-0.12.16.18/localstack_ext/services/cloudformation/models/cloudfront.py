...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or iltjG
 @iltjv
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return iltjm(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @iltjF
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if iltjI(origins,iltje)else origins.get("Items")
  origins_to_create=iltjO([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if iltjI(aliases,iltje)else aliases.get("Items")
  aliases_to_create=iltjO(aliases)
  for distr in distrs:
   origins=iltjO([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=iltjO(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",iltja)
   config["Enabled"]=iltjH(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if iltjI(custom_errors,iltjg)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=iltjH(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":iltja}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=iltjH(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=iltjG):
    parent=config if parent is iltjG else parent
    items=parent.get(key)or[]
    if iltjI(items,iltjg)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":iltjQ(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if iltjI(cache_behavs,iltjg)else cache_behavs)
