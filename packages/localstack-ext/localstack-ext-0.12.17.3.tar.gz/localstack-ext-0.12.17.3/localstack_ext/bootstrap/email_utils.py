...
SENT_EMAILS=[]
EMAIL_BLACKLIST=sxCDa()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",sxCDc(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=sxCDk,from_name=sxCDk,smtp_host=sxCDk,smtp_user=sxCDk,smtp_pass=sxCDk,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if sxCDy(recipients,sxCDK)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=sxCDk):
 ...
 result["From"]=formataddr((sxCDc(Header(from_name,"utf-8")),from_email))
 return result
