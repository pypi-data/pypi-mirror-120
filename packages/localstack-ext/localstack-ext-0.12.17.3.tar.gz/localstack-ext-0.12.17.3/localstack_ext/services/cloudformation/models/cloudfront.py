...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or ISsjQ
 @ISsje
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return ISsjn(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @ISsjY
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if ISsjv(origins,ISsjX)else origins.get("Items")
  origins_to_create=ISsji([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if ISsjv(aliases,ISsjX)else aliases.get("Items")
  aliases_to_create=ISsji(aliases)
  for distr in distrs:
   origins=ISsji([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=ISsji(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",ISsjH)
   config["Enabled"]=ISsjf(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if ISsjv(custom_errors,ISsjw)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=ISsjf(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":ISsjH}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=ISsjf(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=ISsjQ):
    parent=config if parent is ISsjQ else parent
    items=parent.get(key)or[]
    if ISsjv(items,ISsjw)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":ISsjA(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if ISsjv(cache_behavs,ISsjw)else cache_behavs)
