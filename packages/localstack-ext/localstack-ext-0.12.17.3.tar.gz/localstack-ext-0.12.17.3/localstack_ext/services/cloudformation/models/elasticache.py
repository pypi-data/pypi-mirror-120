...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=XlaYN(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not XlaYb
   return result
  except XlaYc:
   props=self.props
   ...
   if parts[0]in candidates and XlaYm(parts)>1:
