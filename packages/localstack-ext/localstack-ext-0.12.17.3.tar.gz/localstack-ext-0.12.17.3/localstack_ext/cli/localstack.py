...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return bgtRY if e else bgtRD
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->bgtRd:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(bgtRd)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:bgtRf):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,bgtRd)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:bgtRf):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,bgtRd)
def print_table(columns:Dict[bgtRf,bgtRf],rows:List[Dict[bgtRf,Any]],formatters:Dict[bgtRf,Callable[[Any],bgtRf]]=bgtRd):
 from rich.table import Table
 if formatters is bgtRd:
  formatters=bgtRa()
 t=Table()
