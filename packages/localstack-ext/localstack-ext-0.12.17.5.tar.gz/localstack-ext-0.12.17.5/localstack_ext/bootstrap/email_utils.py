...
SENT_EMAILS=[]
EMAIL_BLACKLIST=JeOUb()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",JeOUy(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=JeOUw,from_name=JeOUw,smtp_host=JeOUw,smtp_user=JeOUw,smtp_pass=JeOUw,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if JeOUj(recipients,JeOUv)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=JeOUw):
 ...
 result["From"]=formataddr((JeOUy(Header(from_name,"utf-8")),from_email))
 return result
