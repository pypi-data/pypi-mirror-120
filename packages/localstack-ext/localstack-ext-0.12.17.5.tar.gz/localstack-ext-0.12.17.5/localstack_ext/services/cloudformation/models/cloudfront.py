...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or YKORn
 @YKORb
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return YKORC(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @YKORf
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if YKORg(origins,YKORB)else origins.get("Items")
  origins_to_create=YKORA([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if YKORg(aliases,YKORB)else aliases.get("Items")
  aliases_to_create=YKORA(aliases)
  for distr in distrs:
   origins=YKORA([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=YKORA(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",YKORF)
   config["Enabled"]=YKORH(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if YKORg(custom_errors,YKORV)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=YKORH(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":YKORF}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=YKORH(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=YKORn):
    parent=config if parent is YKORn else parent
    items=parent.get(key)or[]
    if YKORg(items,YKORV)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":YKORs(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if YKORg(cache_behavs,YKORV)else cache_behavs)
