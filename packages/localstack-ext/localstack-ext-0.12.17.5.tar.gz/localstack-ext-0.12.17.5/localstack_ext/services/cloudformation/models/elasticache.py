...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=kXaKT(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not kXaKx
   return result
  except kXaKR:
   props=self.props
   ...
   if parts[0]in candidates and kXaKe(parts)>1:
