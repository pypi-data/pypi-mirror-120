...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return pObFD if e else pObFl
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->pObFP:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(pObFP)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:pObFe):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,pObFP)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:pObFe):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,pObFP)
def print_table(columns:Dict[pObFe,pObFe],rows:List[Dict[pObFe,Any]],formatters:Dict[pObFe,Callable[[Any],pObFe]]=pObFP):
 from rich.table import Table
 if formatters is pObFP:
  formatters=pObFN()
 t=Table()
