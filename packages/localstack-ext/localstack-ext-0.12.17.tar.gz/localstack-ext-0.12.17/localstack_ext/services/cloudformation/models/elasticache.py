...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=tQmSE(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not tQmSD
   return result
  except tQmSV:
   props=self.props
   ...
   if parts[0]in candidates and tQmSb(parts)>1:
