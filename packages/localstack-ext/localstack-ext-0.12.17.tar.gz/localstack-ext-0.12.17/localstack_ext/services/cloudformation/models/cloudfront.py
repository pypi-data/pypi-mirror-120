...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or dxkqK
 @dxkqv
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return dxkqP(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @dxkqo
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if dxkqI(origins,dxkqH)else origins.get("Items")
  origins_to_create=dxkqM([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if dxkqI(aliases,dxkqH)else aliases.get("Items")
  aliases_to_create=dxkqM(aliases)
  for distr in distrs:
   origins=dxkqM([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=dxkqM(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",dxkqi)
   config["Enabled"]=dxkqQ(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if dxkqI(custom_errors,dxkqR)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=dxkqQ(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":dxkqi}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=dxkqQ(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=dxkqK):
    parent=config if parent is dxkqK else parent
    items=parent.get(key)or[]
    if dxkqI(items,dxkqR)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":dxkqn(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if dxkqI(cache_behavs,dxkqR)else cache_behavs)
