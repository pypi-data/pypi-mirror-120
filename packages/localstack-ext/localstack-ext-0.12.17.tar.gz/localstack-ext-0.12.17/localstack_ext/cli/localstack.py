...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return UCNqk if e else UCNqg
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->UCNqr:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(UCNqr)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:UCNqG):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,UCNqr)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:UCNqG):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,UCNqr)
def print_table(columns:Dict[UCNqG,UCNqG],rows:List[Dict[UCNqG,Any]],formatters:Dict[UCNqG,Callable[[Any],UCNqG]]=UCNqr):
 from rich.table import Table
 if formatters is UCNqr:
  formatters=UCNqd()
 t=Table()
