...
SENT_EMAILS=[]
EMAIL_BLACKLIST=pCPjy()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",pCPjD(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=pCPjc,from_name=pCPjc,smtp_host=pCPjc,smtp_user=pCPjc,smtp_pass=pCPjc,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if pCPja(recipients,pCPjv)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=pCPjc):
 ...
 result["From"]=formataddr((pCPjD(Header(from_name,"utf-8")),from_email))
 return result
