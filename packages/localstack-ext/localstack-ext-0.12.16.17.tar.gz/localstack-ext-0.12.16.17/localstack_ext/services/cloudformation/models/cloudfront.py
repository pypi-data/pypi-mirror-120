...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or BtdrJ
 @BtdrQ
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return Btdrc(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @BtdrL
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if BtdrA(origins,Btdre)else origins.get("Items")
  origins_to_create=Btdrf([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if BtdrA(aliases,Btdre)else aliases.get("Items")
  aliases_to_create=Btdrf(aliases)
  for distr in distrs:
   origins=Btdrf([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=Btdrf(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",Btdrb)
   config["Enabled"]=Btdrh(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if BtdrA(custom_errors,BtdrH)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=Btdrh(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":Btdrb}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=Btdrh(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=BtdrJ):
    parent=config if parent is BtdrJ else parent
    items=parent.get(key)or[]
    if BtdrA(items,BtdrH)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":Btdrj(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if BtdrA(cache_behavs,BtdrH)else cache_behavs)
