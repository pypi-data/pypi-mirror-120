...
class EC2ElasticIP(GenericBaseModel):
 ...
 def cloudformation_type():
  return "AWS::EC2::EIP"
 def get_physical_resource_id(self,attribute=rsAHv,**kwargs):
...
class SecurityGroupInOrEgress(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  if result:
   ...
   result=[]
   for perm in perms:
    if rsAHX(perm["IpProtocol"])!=rsAHX(rp["IpProtocol"]):
     continue
 ...
 def get_deploy_templates(cls):
  def create_params(params,**kwargs):
   ...
   source_group_id=result.pop("SourceSecurityGroupId",rsAHv)
   vpc_id=rsAHv
   if cls.is_ingress()and source_group_id and not source_group_name:
    ...
    result["SourceSecurityGroupName"]=groups and groups[0]["GroupName"]or rsAHv
    vpc_id=groups and groups[0].get("VpcId")or rsAHv
   dst_group_id=result.pop("DestinationSecurityGroupId",rsAHv)
   ...
   if result.get("IpProtocol"):
    result["IpProtocol"]=rsAHX(result.get("IpProtocol"))
   description=result.pop("Description",rsAHv)
   cidr_ipv6=result.pop("CidrIpv6",rsAHv)
