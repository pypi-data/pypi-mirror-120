...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=UhXNb(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not UhXNW
   return result
  except UhXNE:
   props=self.props
   ...
   if parts[0]in candidates and UhXNp(parts)>1:
