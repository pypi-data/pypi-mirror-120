...
SENT_EMAILS=[]
EMAIL_BLACKLIST=djkmS()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",djkmB(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=djkmu,from_name=djkmu,smtp_host=djkmu,smtp_user=djkmu,smtp_pass=djkmu,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if djkmY(recipients,djkmG)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=djkmu):
 ...
 result["From"]=formataddr((djkmB(Header(from_name,"utf-8")),from_email))
 return result
