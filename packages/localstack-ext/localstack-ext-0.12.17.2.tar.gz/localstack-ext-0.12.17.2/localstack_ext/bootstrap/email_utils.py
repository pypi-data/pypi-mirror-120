...
SENT_EMAILS=[]
EMAIL_BLACKLIST=CScTD()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",CScTa(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=CScTO,from_name=CScTO,smtp_host=CScTO,smtp_user=CScTO,smtp_pass=CScTO,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if CScTy(recipients,CScTl)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=CScTO):
 ...
 result["From"]=formataddr((CScTa(Header(from_name,"utf-8")),from_email))
 return result
