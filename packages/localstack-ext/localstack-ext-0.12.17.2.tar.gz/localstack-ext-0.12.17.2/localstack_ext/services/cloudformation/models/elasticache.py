...
class ElastiCacheCluster(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  try:
   result=pcwoh(ElastiCacheCluster,self).get_cfn_attribute(attribute_name)
   assert result is not pcwot
   return result
  except pcwoV:
   props=self.props
   ...
   if parts[0]in candidates and pcwoO(parts)>1:
