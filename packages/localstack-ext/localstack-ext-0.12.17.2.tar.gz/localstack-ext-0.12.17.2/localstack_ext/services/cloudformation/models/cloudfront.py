...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or IgONs
 @IgOND
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return IgONP(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @IgONf
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if IgONS(origins,IgONy)else origins.get("Items")
  origins_to_create=IgONb([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if IgONS(aliases,IgONy)else aliases.get("Items")
  aliases_to_create=IgONb(aliases)
  for distr in distrs:
   origins=IgONb([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=IgONb(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",IgONh)
   config["Enabled"]=IgONk(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if IgONS(custom_errors,IgONc)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=IgONk(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":IgONh}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=IgONk(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=IgONs):
    parent=config if parent is IgONs else parent
    items=parent.get(key)or[]
    if IgONS(items,IgONc)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":IgONd(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if IgONS(cache_behavs,IgONc)else cache_behavs)
