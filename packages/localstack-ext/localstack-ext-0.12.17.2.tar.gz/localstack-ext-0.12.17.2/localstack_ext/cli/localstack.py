...
class ProCliPlugin(LocalstackCliPlugin):
 ...
 def should_load(self):
  ...
  return rIOkM if e else rIOkU
 def is_active(self):
  return self.should_load()
 def attach(self,cli:LocalstackCli)->rIOkA:
...
def cmd_pod_list():
 ...
 try:
  result=pods_client.list_pods(rIOkA)
  status.stop()
...
@click.argument("name")
def cmd_pod_push(name:rIOkR):
 from localstack_ext.bootstrap import pods_client
 pods_client.push_state(name,rIOkA)
@pod.command(name="pull",help="Pull the state of a cloud pod into the running LocalStack instance")
@click.argument("name")
def cmd_pod_pull(name:rIOkR):
 from localstack_ext.bootstrap import pods_client
 pods_client.pull_state(name,rIOkA)
def print_table(columns:Dict[rIOkR,rIOkR],rows:List[Dict[rIOkR,Any]],formatters:Dict[rIOkR,Callable[[Any],rIOkR]]=rIOkA):
 from rich.table import Table
 if formatters is rIOkA:
  formatters=rIOkD()
 t=Table()
