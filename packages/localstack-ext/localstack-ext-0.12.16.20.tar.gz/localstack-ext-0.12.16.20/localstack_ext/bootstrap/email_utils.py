...
SENT_EMAILS=[]
EMAIL_BLACKLIST=lKNuF()
def is_smtp_configured():
 return config.SMTP_HOST
def get_canonical_email(email):
 email=re.sub(r"\s+","",lKNue(email or ""))
 email=email.strip().lower()
 return email
def send_email(subject,message,recipients,from_email=lKNuY,from_name=lKNuY,smtp_host=lKNuY,smtp_user=lKNuY,smtp_pass=lKNuY,images={}):
 ...
 if not smtp_host:
  ...
  return
 recipients=recipients if lKNur(recipients,lKNui)else[recipients]
...
def construct_message(subject,message,from_name,from_email,images=lKNuY):
 ...
 result["From"]=formataddr((lKNue(Header(from_name,"utf-8")),from_email))
 return result
