...
class CloudFrontOriginAccessIdentity(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  return result and result[0]or Fspvh
 @FspvE
...
class CloudFrontFunction(GenericBaseModel):
 ...
 def get_cfn_attribute(self,attribute_name):
  ...
  return FspvW(CloudFrontFunction,self).get_cfn_attribute(attribute_name)
 @Fspvo
...
class CloudFrontDistribution(GenericBaseModel):
 ...
 def fetch_state(self,stack_name,resources):
  ...
  origins=origins if FspvP(origins,Fspvb)else origins.get("Items")
  origins_to_create=Fspvm([orig["DomainName"]for orig in origins])
  aliases=props.get("Aliases",[])
  aliases=aliases if FspvP(aliases,Fspvb)else aliases.get("Items")
  aliases_to_create=Fspvm(aliases)
  for distr in distrs:
   origins=Fspvm([orig.get("DomainName")for orig in distr.get("Origins",{})["Items"]])
   aliases=Fspvm(distr.get("Aliases",{}).get("Items",[]))
 ...
 def get_deploy_templates():
  def lambda_get_distribution_config(params,**kwargs):
   ...
   config["IsIPV6Enabled"]=config.pop("IPV6Enabled",FspvI)
   config["Enabled"]=Fspve(config.get("Enabled")).lower()=="true"
   cert=config["ViewerCertificate"]=config.get("ViewerCertificate",{})
   custom_errors=config.get("CustomErrorResponses",[])
   custom_errors=(custom_errors.get("Items",[])if FspvP(custom_errors,FspvC)else custom_errors)
   for item in custom_errors:
    item["ResponseCode"]=Fspve(item.get("ResponseCode"))
   cache=config["DefaultCacheBehavior"]
   cache["TrustedSigners"]=cache.get("TrustedSigners")or{"Quantity":0,"Enabled":FspvI}
   cache["MinTTL"]=cache.get("MinTTL")or 600
   forwarded=cache["ForwardedValues"]=cache.get("ForwardedValues",{})
   forwarded["QueryString"]=Fspve(forwarded.get("QueryString")).lower()=="true"
   ...
   def convert_list(key,parent=Fspvh):
    parent=config if parent is Fspvh else parent
    items=parent.get(key)or[]
    if FspvP(items,FspvC)and "Items" in items and "Quantity" in items:
     return
    parent[key]={"Quantity":FspvH(items),"Items":items}
   cache_behavs=config.get("CacheBehaviors",[])
   cache_behavs=(cache_behavs.get("Items",[])if FspvP(cache_behavs,FspvC)else cache_behavs)
