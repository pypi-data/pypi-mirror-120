...
class RedisStateHandler(StateHandler):
    ...
    @classmethod
    def init(cls, existing_connection=None):
        """
        This init can be called to initialize the class variable with a Redis connection

        :param existing_connection: An existing redis connection
        :return: New or existing redis connection
        """

        # If existing connection is passed in, it initializes the class variable and returns the same value
        if existing_connection:
            cls._redis = existing_connection
            return existing_connection

        # If not existing connection is passed, a new one is build from the cache URL
        ...
        # Initializing the class variable with the new connection
        cls._redis = _redis
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.redis_ttl = os.getenv("REDIS_TTL", 7_776_000)  # 90 days
    ...
    def save_state(self, asset_id: int, state: dict, state_key: str, timestamp: [int, None], **kwargs):
        """
        ...
        redis_ttl = kwargs.pop("redis_ttl", self.redis_ttl)
        self.redis_connection.set(state_key, value=json.dumps(state), ex=redis_ttl)
