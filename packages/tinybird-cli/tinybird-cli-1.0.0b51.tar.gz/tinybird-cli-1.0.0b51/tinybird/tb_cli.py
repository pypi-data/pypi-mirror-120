...
@click.pass_context
@coro
async def push(ctx, prefix, filenames, dry_run, check, push_deps, debug, force, populate, fixtures, wait, skip_table_checks, yes, only_response_times, workspace_map, workspace):
...
async def auth(ctx, host, connector):
    ...
    if not ctx.invoked_subcommand:
        ...
        try:
            ...
            if 'user_workspaces' in response:
                config['user_workspaces'] = response['user_workspaces']
