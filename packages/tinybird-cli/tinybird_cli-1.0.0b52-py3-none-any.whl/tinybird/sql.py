...
def _parse_table_structure(schema):  # noqa: C901
    ...
    while i < len(schema):
        if not name:
            ...
            name = parse_name()
            continue
        found = lookahead_matches([NULL, NOTNULL, DEFAULT, MATERIALIZED, ALIAS, CODEC, TTL, JSONPATH, COMMA, TYPE])
        ...
        if found == TYPE:
            advance('')
            valid_next = [NULL, NOTNULL, DEFAULT, MATERIALIZED, ALIAS, CODEC, TTL, JSONPATH, COMMA]
