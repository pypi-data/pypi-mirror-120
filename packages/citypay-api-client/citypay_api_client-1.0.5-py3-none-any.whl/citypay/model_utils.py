...
def check_validations(
    ...
    if (is_json_validation_enabled('maxLength', configuration) and
            ...
            check_len(input_values) > current_validations['max_length']):
        raise ApiValueError(
...
def is_type_nullable(input_type):
    ...
    Returns:
        bool
    """
    if type(input_type) is none_type:
        return True
...
def check_len(inputs_values):
    """
    Gets the len of inputs_values including date and datetime values
    """

    if (type(inputs_values) is date) or (type(inputs_values) is datetime):
        return len(str(inputs_values))
    else:
        return len(inputs_values)
