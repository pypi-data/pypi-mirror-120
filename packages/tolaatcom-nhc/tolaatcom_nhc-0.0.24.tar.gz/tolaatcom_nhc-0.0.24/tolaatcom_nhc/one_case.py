...
class OneCase:
    ...
    def scrape_documents(self, key, case, decisions, verdicts,  d_range, v_range, finish):

        futures = set()
        self.logger.info('Scraping documents. decisions %s, verdicts %s', d_range[1]-d_range[0], v_range[1]-v_range[0])

        with ThreadPoolExecutor(max_workers=10) as tp:
            for rng, array, d_type in [(d_range, decisions, 'decisions'), (v_range, verdicts, 'verdicts')]:
    ...
    def smart_scrape(self, case):
        ...
        if decisions_added:
            something_changed = True
            decisions_range = len(decisions), len(new_decisions)
        ...
        if verdicts_added:
            something_changed = True
            verdicts_range = len(verdicts), len(new_verdicts)
        ...
        self.scrape_documents(key, case, new_decisions, new_verdicts, decisions_range, verdicts_range, finish)
...
if __name__=='__main__':
    ...
    o=OneCase(config={'only_one': True})

    o.smart_scrape({'CaseDisplayIdentifier': '1165-09-16', 'CaseType': 'n'})
