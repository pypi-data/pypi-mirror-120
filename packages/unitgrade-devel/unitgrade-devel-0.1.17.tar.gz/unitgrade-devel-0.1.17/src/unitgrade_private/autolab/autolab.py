...
def deploy_assignment(base_name, INSTRUCTOR_BASE, INSTRUCTOR_GRADE_FILE, STUDENT_BASE, STUDENT_GRADE_FILE,
    ...
    total_ = res['total'][1]
    problems = [dict(name='Total', description='', max_score=total_, optional='false')]
...
def format_autolab_json(data, indent=None):
    ...
    for n, qs in data['details'].items():
        ...
        for item, val in qs['items'].items():
            print(item, val)
            item_name = item[1]
            rs[item_name] = {'passed': val['status'] == 'pass'}
    ...
    if indent is not None:
        s = json.dumps(pres, indent=2)
    else:
        s = json.dumps(pres)
    print(s)
    scores = {"scores": {**{"Total": data['total'][0] }, **totals} }
