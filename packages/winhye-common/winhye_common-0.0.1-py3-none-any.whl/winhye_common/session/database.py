...
class DatabaseClient:
    ...
    @staticmethod
    def get_session(url: str, timeout: int = 30, echo: bool = False, **configs):
        """
        ...
        if url in DatabaseClient.engines:
            ...
            engine = create_engine(url, pool_size=DatabaseClient.pool_size, echo=echo, pool_timeout=timeout, **configs)
            DatabaseClient.engines[url] = engine
        _release_lock()
        return sessionmaker(bind=engine)()

    def __init__(self, url: str, timeout: int = 30, echo: bool = False, **configs):
        """
        ...
        if DatabaseClient.engines is None:
            DatabaseClient.init()
        self.session = DatabaseClient.get_session(url=url, timeout=timeout, echo=echo, **configs)
