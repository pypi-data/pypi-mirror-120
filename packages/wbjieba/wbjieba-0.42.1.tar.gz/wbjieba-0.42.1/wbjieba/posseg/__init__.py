...
class POSTokenizer(object):
    ...
    def __init__(self, tokenizer=None):
        self.tokenizer = tokenizer or wbjieba.Tokenizer()
...
def cut(sentence, HMM=True, use_paddle=False):
    ...
    if use_paddle and is_paddle_installed:
        ...
        if sentence is None or sentence == "" or sentence == u"":
            return
        import wbjieba.lac_small.predict as predict
        ...
        return
    global dt
    if wbjieba.pool is None:
        for w in dt.cut(sentence, HMM=HMM):
            yield w
    else:
        parts = strdecode(sentence).splitlines(True)
        if HMM:
            result = wbjieba.pool.map(_lcut_internal, parts)
        else:
            result = wbjieba.pool.map(_lcut_internal_no_hmm, parts)
        for r in result:
            for w in r:
                yield w
