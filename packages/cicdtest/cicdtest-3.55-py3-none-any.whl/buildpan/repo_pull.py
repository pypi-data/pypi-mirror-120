...
def repo_pull(path, project_id, repo_name, username):
    ...
    try:
        ...
        # response = requests.post('http://35.225.89.124/fetch_log?'+'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=pull success'+'&status=pull operation performed')
        ...
        print("e = ",e)
        curtime = datetime.datetime.now()          
        requests.post(fetch_log+ "?" +'project_id='+project_id+'&repo_name='+repo_name+'&Time='+str(curtime)+'&user_name='+username+'&message=pull failed'+'&status=pull operation failed')
