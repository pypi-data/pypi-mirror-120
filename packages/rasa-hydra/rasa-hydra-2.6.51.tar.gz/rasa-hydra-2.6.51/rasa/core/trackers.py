...
class DialogueStateTracker:
    ...
    app_events = {'capture_utterance', 'dtmf', 'get_speech', 'hangup'}
    ...
    def add_conversation_steps(self, event: Event) -> None:
        if isinstance(event, BotUttered) and event.data and 'custom' in event.data and 'type' in event.data['custom'] \
                and event.data['custom']['type'] in self.app_events:
            self.total_steps += 1
        elif isinstance(event, UserUttered) and event.text is not None and not event.text.startswith('/greet') \
                and not event.text.startswith('/restart') \
                and (event.intent and 'name' in event.intent and event.intent['name'] != "out_of_scope"):
            self.total_steps += 1
