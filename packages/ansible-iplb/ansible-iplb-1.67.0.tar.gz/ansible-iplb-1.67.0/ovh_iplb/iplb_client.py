...
class Farm(Resource):
    ...
    def servers(self):
        # We are limited to 99 servers max on an IPLB
        return [self.server(server_def) for server_def in self.farm_def.get('servers', [])][:99]
...
class IPLB(Resource):
    ...
    def task_in_progress(self):
        ...
        return len(tuple(
            filter(lambda task: task['status'] != 'done' and not task['action'].startswith('order'), tasks)  # Certificate ordering can be blocked for a few days
        )) > 0
