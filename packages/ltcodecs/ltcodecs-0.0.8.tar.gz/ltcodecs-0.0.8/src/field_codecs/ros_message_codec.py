from field_codecs import RosMsgFieldCodec, metadata_decoders, metadata_encoders
import msgpack
    ...
    def __init__(self, ros_type, fields_dict: dict = None, checksum=None):
        ...
        if fields_dict:
            ...
            for field_name, field_params in fields_dict.items():
                ...
                if codec_name in set(list(metadata_decoders.keys()) + list(metadata_encoders.keys())):
                    self.metadata_fields.append({field_name: codec_name})
        else:
            self.metadata_fields = None

        self.root_field_codec = RosMsgFieldCodec(ros_type=ros_type, fields=fields_dict)
    ...
    def _decode_metadata(self, ros_msg, received_packet = None):
        ...
        if self.metadata_fields:
            for field_name, codec in self.metadata_fields.items():
                metadata_attribute = metadata_decoders[codec]
