...
class RawDataset:
    ...
    def __init__(self, data, kind=DataKind.MEM_SEQ, transform: Optional[Callable] = None, filename=None):
        """

        Args:
            data:
              - list of things: if kind=DataKind.MEM_SEQ
              - filename: if kind=DataKind.FILE
              - mmap: if kind=DataKind.MMAP_FILE
            kind: @see DataKind
            transform: func for mapping raw data into feature
            filename: only valid if kind=DataKind.MMAP_FILE
        ...
        if self.kind == DataKind.MEM_SEQ:
            ...
            self.offset, self.n_data = get_offset(self.filename)
            self.data = data  # mmap
            logger.warning('with mmap lead to huge memory cost')
