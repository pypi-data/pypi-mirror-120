...
def checkgeodetic():
    ...
    import os
    rval = True
    geodeticdir = os.path.dirname(ctsys.resolve('geodetic/IERSpredict'))
    if geodeticdir == 'geodetic':
        casalog.post('Data repository directory not found. Cannot check Measures tables.', 'WARN')
        rval = False
    else:
        casalog.post('\n', 'INFO')
