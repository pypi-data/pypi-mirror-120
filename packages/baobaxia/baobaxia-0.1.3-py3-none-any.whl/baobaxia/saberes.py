...
class SaberesDataset():
    ...
    def create_obj(self, name: str, path: Optional[Path] = None, **kwargs):
        """Cria um saber a partir das informações fornecidas.
        """
        ...
        if self.balaio is not None:
            data['balaio'] = self.balaio
            if self.mucua is not None:
                data['mucua'] = self.mucua
        if path is not None:
            data['path'] = path / slug
        else:
            data['path'] = Path(slug)
