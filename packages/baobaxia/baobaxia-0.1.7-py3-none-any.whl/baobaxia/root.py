...
class Baobaxia():
    ...
    def put_mucua(self, *,
        ...
        if mucua_slug is None:
            mucua = dataset.settle(dataset.create_obj(name))
    ...
    def discover_saberes(self, *,
        ...
        def get_method_template(balaio: str, mucua: str, path: Path, token: str):
            session = self._sessions[token]
            self._check_cache(model, field_name, patterns, balaio, mucua)
            return self.saberes[balaio][mucua][
                field_name][path].copy()
        setattr(self, 'get_'+field_name, get_method_template)

        def put_method_template(balaio: str, mucua: str, saber: model,
                                token: str,
                                slug_dir: bool = False):
            session = self._sessions[token]
            self._check_cache(model, field_name, patterns, balaio, mucua)
        ...
        def post_method_template(balaio: str, mucua: str, name: str,
                                path: Path,
                                token: str,
                                **kwargs):
            session = self._sessions[token]
            ...
            dataset = self.datastore.create_dataset(
                    ...
                    mucua = mucua,
                    mocambola = session.mocambola.username)
            saber = dataset.settle(
                dataset.create_obj(name, path, **kwargs))
            self.saberes[balaio][mucua][
                field_name][path] = saber
            ...
            return saber.copy()
        setattr(self, 'post_'+field_name, post_method_template)
