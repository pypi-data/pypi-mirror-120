...
def query_rates(
    ...
    rates["date"] = to_datetime(rates["date"])
    return rates
...
def _to_usd(
    ...
    if df.empty:
        ...
        df["date"] = to_datetime(df[dates_field].dt.date)
        rated = df.merge(rates, how="left", on=["date", "currency"])
...
def _to_target_currency(
    ...
    df_in_usd = _to_usd(df, values_fields, dates_field, currencies_field).drop(
        columns=["rate"]
    )
    rates_in_target_currency = query_rates(
        df_in_usd[dates_field].dt.date.unique(), target_currency
    ).drop(columns=["currency"])
    merged_target_currency_rates = df_in_usd.merge(
        rates_in_target_currency, how="left", on="date"
    )
    if isinstance(values_fields, list):
        ...
        ] = merged_target_currency_rates.loc[:, values_fields].mul(
        ...
        merged_target_currency_rates = merged_target_currency_rates.assign(
            **{
                f"{values_fields}_{target_currency.lower()}": lambda x: x[values_fields]
                * x["rate"]
            }
        )
        return merged_target_currency_rates.drop(columns=["date", "rate"])
