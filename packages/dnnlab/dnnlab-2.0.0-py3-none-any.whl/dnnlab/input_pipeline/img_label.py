...
def create_dataset(tfrecords,
    ...
    if encoding:
        parsed_dataset = raw_dataset.map(
            lambda example: _parse_tfrecords_encoded(example, keys,
                                                     one_hot_labels),
            num_parallel_calls=tf.data.experimental.AUTOTUNE)
    else:
        parsed_dataset = raw_dataset.map(
            lambda example: _parse_tfrecords_raw(example, keys),
            num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def _parse_tfrecords_encoded(example, keys, one_hot_labels):
    ...
    img_source = tf.io.decode_jpeg(img_source)
...
def _parse_tfrecords_raw(example, keys):
    # Create a dictionary describing the features.
    feature_description = {
        keys[0]: tf.io.FixedLenFeature([], tf.string, default_value=""),
