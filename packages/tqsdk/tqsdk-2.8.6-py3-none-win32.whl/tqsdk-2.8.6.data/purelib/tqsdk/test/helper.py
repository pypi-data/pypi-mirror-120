...
class MockWebsocketsServer():
    ...
    async def _server(self):
        async with websockets.serve(self._handler, "127.0.0.1", ping_timeout=None) as server:
    ...
    async def on_received(self, pack):
        ...
        if pack["aid"] != "peek_message":
            ...
            elif self._expecting["aid"] == "subscribe_quote" and pack["aid"] == "subscribe_quote":
                assert sorted(self._expecting["ins_list"].split(",")) == sorted(pack["ins_list"].split(","))
            else:
