...
class TestFuncBasic(TQBaseTestcase):
    ...
    def test_is_changing(self):
        ...
        self.md_mock.run(os.path.join(dir_path, "log_file", "test_func_basic_is_changing.script.lzma"))
        ...
        quote = api.get_quote("SHFE.rb2106")
        position = api.get_position("SHFE.rb2106")
        order1 = api.insert_order("DCE.m2109", "BUY", "OPEN", 1)
        api.wait_update()
        order2 = api.insert_order("SHFE.rb2106", "SELL", "OPEN", 2, quote.bid_price1)
        api.wait_update()
        ...
        order3 = api.insert_order("SHFE.rb2106", "BUY", "CLOSETODAY", 1, quote.ask_price1)
