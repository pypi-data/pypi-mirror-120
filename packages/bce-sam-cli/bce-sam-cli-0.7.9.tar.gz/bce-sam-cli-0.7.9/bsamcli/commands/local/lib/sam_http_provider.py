...
class SamHttpProvider(EventSourceProvider):
    ...
    def _extract_http(self, resources):
        ...
        return self.merge_http_events(result)

    
    def merge_http_events(self, function_events):
        """
        Merge result into an array
        """

        all = []
        for _, http_events in function_events.items():
    ...
    def _extract_http_from_function(logical_id, function_resource):
        ...
        ----------
        logical_id : str
            Logical ID of the resource, which is the function Name
    ...
    def _convert_event_http(logical_id, event_properties):
        ...
        is_binary = event_properties.get(SamHttpProvider._IS_BINARY)

        if isinstance(methods, str):            
            methods = methods.lstrip('[')
        ...
        methods = [m.upper().lstrip('[').rstrip(']') for m in methods]
        for m in methods:
        ...
        return HttpEvent(resource_path=resource_path, method=methods,
            auth_type=auth_type, function_name=logical_id, is_binary=is_binary)
