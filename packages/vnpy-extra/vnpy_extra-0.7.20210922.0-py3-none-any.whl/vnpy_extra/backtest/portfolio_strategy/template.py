...
class TargetPosAndPriceSpreadTemplate(TargetPosAndPrice2SymbolTemplate):
    ...
    def __init__(
        ...
        self._bg_spread = BarGenerator(
            ...
            window=self.window, on_window_bar=self.on_win_spread_bar,
    ...
    def on_bars(self, bars: Dict[str, BarData]) -> Optional[bool]:
        ...
        spread = self.calc_spread_bar(bars).close_price
        self._update_spread(spread)
        spread_bar = self.calc_spread_bar(bars)
    ...
    def calc_spread_bar(self, bars: Dict[str, BarData]) -> BarData:
        """
        通过比值计算时，当symbol2的volume和open_interest为0时，由于分母为0，把计算结果算成了0
        """
        bar1 = bars[self.vt_symbol1]
        ...
        if bar1 is None:
            ...
            last_bar1 = self.last_bars[self.vt_symbol1]
            bar1 = BarData(
        ...
        if bar2 is None:
            self.write_log(f"calc_spread_bar bars={bars}", 'warning')
            last_bar2 = self.last_bars[self.vt_symbol2]
            bar2 = BarData(
