...
class ArrayManager(ArrayManagerBase):
    ...
    def er_ratio(self, period, smooth_fast=2, smooth_slow=30, array: bool = False):
        """
        ER效率系数
        """
        ...
        if array:
            direction = self.close_array[period - 1:] - self.close_array[:-period + 1]
            close_rolling = sliding_window_view(self.close_array, period)
            volatility = np.sum(np.abs(close_rolling[:, 1:] - close_rolling[:, -1]), axis=1)
        else:
            direction = self.close_array[-1] - self.close_array[-period]
            volatility = np.sum(np.abs(self.close_array[-period + 1:] - self.close_array[-period:-1]))
