...
def analyze_document_2D (folder_path, obj_channel, k2, k1 = 2.5, pixel_size = 1, exclude_true_blank = False):
    ...
    for i in range(len(samples_path)):
        ...
        new_row = analyze_actin_2d_standard (img = img, img_dif = img_dif, pixel_size = pixel_size, exclude_true_blank = exclude_true_blank)
...
def analyze_document_3D (folder_path, obj_channel, k1, k2, xy_unit, z_unit, pixel_size = 1, single_k1 = True, oversampling_for_bundle = False, use_GPU = False):
    '''
    ...
    for i in range(len(samples_path)):
        ...
        img_dif = ILEE_3d (img,
                           ...
                           gauss_dif = True)
        new_row = analyze_actin_3d_standard (img = img, img_dif_ori = img_dif, xy_unit = xy_unit, z_unit = z_unit, oversampling_for_bundle = oversampling_for_bundle, pixel_size = pixel_size)
        new_row['file_path'] = samples_path[i]
        new_row['file_name'] = file_name_array[i]
        df_result = df_result.append(new_row, ignore_index=True)
        print('current sample done.\n')
