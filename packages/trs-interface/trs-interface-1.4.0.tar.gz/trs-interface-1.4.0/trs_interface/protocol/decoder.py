...
class StreamDecoder:
    ...
    """
    def __init__(self, stream=None, on_error="warn", buffersize=2**22):
        if stream is None:
        ...
        #: Pointer to current end of buffer
        self._p = 0
        #: Action to take upon decoding error
    ...
    def _append(self, data):
        """
        Append byte array data to the buffer.
        """
        if len(data) > 0:
    ...
    def __next__(self):
        ...
        if self._p < HEADER_SIZE:
            self._append(self._file.read(HEADER_SIZE - self._p))
        # Hopefully enough data in buffer now to try to decode a message
        while self._p >= HEADER_SIZE:
            # Ensure the data follows the message format
            msg_magic, msg_id, msg_data = struct.unpack_from("<4sHi", self._buffer)
            if not msg_magic == b"MSG:":
        ...
        length = 0
        # Check length of message will fit in buffer
        if (HEADER_SIZE + length > len(self._buffer)):
            self._reset()
            raise BufferError("Expected message length would exceed allocated buffer size.")
        if long_form and msg_data > 0:
            # Long form message, and data field encodes fixed payload length
            length = msg_data
            if self._p < HEADER_SIZE + length:
                # Not enough data in buffer to decode long form message, attempt to read some more data
                self._append(self._file.read(length - self._p + HEADER_SIZE))
                if self._p < HEADER_SIZE + length:
            ...
            while (self._p < HEADER_SIZE + 4) or not (struct.unpack_from("<i", self._buffer[self._p - 4:])[0] == msg_data):
                # Not enough data in buffer or terminator not found yet
                self._append(self._file.read(1))
                if (self._p <= HEADER_SIZE + length):
                    # Another byte wasn't available
                    raise StopIteration
                length = self._p - HEADER_SIZE
                # Check for buffer overflow
                if (self._p >= len(self._buffer)):
                    self._reset()
                    raise BufferError("Variable length message has exceeded allocated buffer size.")
        # Have enough data in buffer to decode the full message
        data = self._buffer[:length + HEADER_SIZE]
        # Can now start a new message buffer
        self._reset()
