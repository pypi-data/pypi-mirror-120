...
def create_build_from_git_revision(
    session: Session, project_id: str, git_revision: str, deploy: bool
):
    """Creates a build based on a git revision and returns the result."""
    build_spec = {"configFilesFromGit": {"revision": git_revision}}
...
def _graphql_query(session: Session, query: str, variables: dict):
    r = session.post(
        GLEAN_BASE_URI + "/graphql/", json={"query": query, "variables": variables}
    )
