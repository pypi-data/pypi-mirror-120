...
class MistunePPTXFilter(DexyFilter):
    """
    Runs mistune processor to convert markdown to HTML.
    ...
    _settings = {
            'input-extensions' : ['.md'],
            'output-extensions' : ['.pptx'],
            'output' : True,
            }

    def is_active(self):
        return MISTUNE_AVAILABLE and PPTX_AVAILABLE

    def process(self):
        input_text = str(self.input_data)
        renderer = PythonPptxRenderer(Presentation())
...
class PythonPptxRenderer(mistune.Renderer):
    def __init__(self, presentation, **kwargs):
        super(PythonPptxRenderer, self).__init__(**kwargs)
        self.table_memory = []
        self.img_counter = 0
    ...
    def hrule(self):
        self.presentation.add_page_break()
        return ""
