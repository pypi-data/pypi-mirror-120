...
from bing_translation_for_python import Translator, setting
from bing_translation_for_python.core import Text, SemanticItem
...
class TranslateTest(unittest.TestCase):
    ...
    def test_translator_get_iterable_obj(self):

        try:
            data = [self.faker.color_name() for i in range(2)]
            Translator(self.default_language).translator(data, separator='\n')
...
class SemanticTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.semantic = core.Semantic(
            core.Text('en', '你好', **token),
            **token
        )
    ...
    def test_attr(self):
        """Semantici必须包含的属性"""
        try:
            print(self.semantic.from_lang)
            print(self.semantic.to_lang)
