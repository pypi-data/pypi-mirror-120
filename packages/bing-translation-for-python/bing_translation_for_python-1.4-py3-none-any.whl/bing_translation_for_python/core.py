...
class DataTemplate():
    def __init__(self):
        self.__get_token_and_key__()

    def __get_token_and_key__(self):
        try:
            ...
            self.token = re.search(r'".{32}"', string).group().strip('"'),
            self.key = int(re.search(r'[0-9]{13}', string).group()),
    ...
    def template_of_semantic(self, fromlang, tolang, text) -> dict:
        return {
...
class Semantic:
    ...
    def __init__(self, from_lang, to_lang, reper_text):
        ...
        self.data_template = DataTemplate()

        template = self.data_template.template_of_semantic(
            fromlang=from_lang,
            text=reper_text,
            tolang=to_lang,
        )
...
class Text:
    def __init__(self, to_lang, reper_text, fromlang='auto-detect',):
        ...
        if reper_text.strip():
            data = requests.post(
                **self.data_template.template_of_translator(
                    fromlang=fromlang,
                    text=reper_text,
                    tolang=to_lang,
                )).json()
