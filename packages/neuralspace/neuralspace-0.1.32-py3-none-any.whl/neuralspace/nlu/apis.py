...
async def get_entity_list_for_cli(
    entity_type: Text, language: Text, pagesize=1, count=None
):
    if count is None:
        print(pagesize)
        payload = {
    ...
    async with get_async_http_session().post(
        ...
        if is_success_status(response.status):
            if count is None:
                ...
                entities_supported = [
                    data["entity"] for data in json_response[DATA][ENTITIES]
                ]
                return entities_supported
        return json_response
