...
class Depends(Element[DependsComplexType]):
    ...
    def step(self) -> str:
        """ `step` attribute """
        assert self._step is not self.__unset, "Uninitialized attribute"
        return self._step

    @step.setter
    def step(self, step: str):
        assert self._step is self.__unset, "Immutable attribute"
...
class Step(
    ...
    def priority(self) -> str:
        """ `priority` attribute """
        assert self._priority is not self.__unset, "Uninitialized attribute"
        return self._priority.value

    @priority.setter
    def priority(self, priority: str):
        assert self._priority is self.__unset, "Immutable attribute"
    ...
    def depends(self) -> typing.Optional[FilterableTuple[Depends]]:
        """ `depends` """
        assert self._depends is not self.__unset, "Uninitialized attribute"
        return self._depends

    @depends.setter
    def depends(self, depends: typing.Optional[typing.Iterable[Depends]]):
        assert self._depends is self.__unset, "Immutable attribute"
