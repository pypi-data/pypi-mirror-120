...
class NameMixin(typing.Generic[NTS, NTG]):
    ...
    _name: typing.Optional[NTG] = __unset

    @property
    def name(self) -> typing.Optional[NTG]:
        """ The `name` attribute """
        assert self._name is not self.__unset, "Uninitialized attributed"
        return self._name

    # noinspection PyShadowingBuiltins
    @name.setter
    def name(self, name: typing.Optional[NTS]):
        assert self._name is self.__unset, "Immutable attribute"
...
class NameStrMixin(NameMixin[str, str]):
    ...
    def _convert_name(name: str) -> str:
        assert isinstance(name, str), "Invalid type"
        return name
...
class NamePathMixin(NameMixin[typing.Union[str, PurePath], PurePosixPath]):
    ...
    """
    @staticmethod
    def _convert_name(name: typing.Union[str, PurePath]) -> PurePosixPath:
        if isinstance(name, str):
            name = PurePath(name)

        assert isinstance(name, PurePath), "Invalid type"
        return typing.cast(
            PurePosixPath,
