...
class TestResultBundle(Bundle[TestresultRootType]):
    ...
    def results(self) -> typing.List[Results]:
        """ `results` children """
        assert self._results is not self.__unset, "Uninitialized attributed"
        return list(self._results.values())

    @results.setter
    def results(self, results: typing.Iterable[Results]):
        assert self._results is self.__unset, "Immutable attribute"
        assert isinstance(results, collections.abc.Iterable), "Invalid type"
