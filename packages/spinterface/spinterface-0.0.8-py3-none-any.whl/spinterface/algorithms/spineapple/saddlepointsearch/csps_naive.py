...
class CSPS_2D_naive:
    ...
    def __init__(self, f: Callable, initial: np.ndarray, iterations: int = 1000, ds: float = 0.001, h: float = 0.00001) -> None:
        r"""
        Initialize SPS
        ...
        for iter in range(self._iterations):
            ...
            if np.mod(iter, gralog) == 0:
                with open(str(write_out), 'a') as f:
                    f.write(f'{iter}  {current_point[0]}  {current_point[1]}  {eigenvals[0]}  {eigenvals[1]}  {eigenvecs[0,0]}  {eigenvecs[1,0]}  {eigenvecs[0,1]}  {eigenvecs[1,1]}  {force[0]}  {force[1]}\n')
