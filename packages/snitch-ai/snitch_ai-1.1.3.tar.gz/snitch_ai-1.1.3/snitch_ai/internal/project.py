...
def _upload_quality_analysis_file(project_id, quality_id, client, file_name, file_path):
    with open(file_path, "rb") as f:
        resp = client.patch(f"project/{project_id}/quality/{quality_id}", files={file_name: f})
...
def _upload_drift_analysis_file(project_id, drift_id, client, file_name, file_path):
    with open(file_path, "rb") as f:
        resp = client.patch(f"project/{project_id}/drift/{drift_id}", files={file_name: f})
        if resp.status_code != 200:
            raise Exception(f"Error uploading file: {resp.text}")
...
class Project:
    ...
    def run_quality_analysis(self, model, train_x, train_y, test_x, test_y, label: str = None, no_wait: bool = False) -> QualityAnalysis:
        """
        ...
        if not label:
            label = f"Quality Analysis {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
        ...
        try:
            if not model is None:
                ...
                _upload_quality_analysis_file(self.project_id, quality_id, client, "model", path)

            if not train_x is None:
                path = _save_dataset_to_path(train_x, os.path.join(temp_dir, "train_x"))
                _upload_quality_analysis_file(self.project_id, quality_id, client, "train_x", path)

            if not train_y is None:
                path = _save_dataset_to_path(train_y, os.path.join(temp_dir, "train_y"))
                _upload_quality_analysis_file(self.project_id, quality_id, client, "train_y", path)

            if not test_x is None:
                path = _save_dataset_to_path(test_x, os.path.join(temp_dir, "test_x"))
                _upload_quality_analysis_file(self.project_id, quality_id, client, "test_x", path)

            if not test_y is None:
                path = _save_dataset_to_path(test_y, os.path.join(temp_dir, "test_y"))
                _upload_quality_analysis_file(self.project_id, quality_id, client, "test_y", path)
        finally:
    ...
    def run_drift_analysis(self, train_x, updated_x, label: str = None, no_wait: bool = False) -> DriftAnalysis:
        """
        ...
        if not label:
            label = f"Drift Analysis {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}"
        ...
        try:
            if not train_x is None:
                path = _save_dataset_to_path(train_x, os.path.join(temp_dir, "train_x"))
                _upload_drift_analysis_file(self.project_id, drift_id, client, "train_x", path)

            if not updated_x is None:
                path = _save_dataset_to_path(updated_x, os.path.join(temp_dir, "updated_x"))
                _upload_drift_analysis_file(self.project_id, drift_id, client, "updated_x", path)
        finally:
            shutil.rmtree(temp_dir)
