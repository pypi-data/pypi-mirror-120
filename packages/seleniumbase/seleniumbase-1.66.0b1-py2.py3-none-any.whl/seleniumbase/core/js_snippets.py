""" Table of Contents:
1. live_js - Reload web pages when changes are detected on the server side.
2. recorder_js - Record browser actions while using the best CSS selectors.
"""
...
var getBestSelector = function(el) {
    ...
    for (var i = 0; i < non_id_attributes.length; i++) {
        ...
        num_by_attr[i] = document.querySelectorAll(selector_by_attr[i]).length;
        if (!selector_by_attr[i].includes(child_sep) && num_by_attr[i] == 1)
    ...
    for (var i = 0; i < non_id_attributes.length; i++) {
        if (child_count_by_attr[i] < lowest_child_count && num_by_attr[i] == 1)
        {
...
document.body.addEventListener('change', function (event) {
    ...
    if (tag_name === 'select')
    {
        optxt = element.options[element.selectedIndex].text;
...
document.body.addEventListener('mouseup', function (event) {
        ...
        tag_name !== 'select')
    {
        ...
        document.recorded_actions.pop();
        document.recorded_actions.push(['click', selector, ahref, d_now]);
