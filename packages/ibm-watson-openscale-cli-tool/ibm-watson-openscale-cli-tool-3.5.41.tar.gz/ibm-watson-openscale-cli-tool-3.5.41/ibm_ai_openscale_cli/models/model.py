...
class Model:
    ...
    def __init__(self, name, args, model_instances=1):
        ...
        if "Challenger" not in self.name:
            ...
            model_meta_filename = "{}/{}".format(WML_SPARK_VERSION_SUPPORT_MAPPING[self._args.env], Model.MODEL_META_FILENAME)
            model_content_filename = "{}/{}".format(WML_SPARK_VERSION_SUPPORT_MAPPING[self._args.env], Model.MODEL_CONTENT_FILENAME)
        else:
