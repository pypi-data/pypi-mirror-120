...
class OpenScaleClient(OpenScaleReset):
    ...
    def configure_drift(self, params):
        if params:
            ...
            scikit_folder = DRIFT_ARCHIVE_ENV_MAPPING[self._args.env]

            if self.is_mrm_challenger:
                filename = "drift_archives/{}/{}".format(scikit_folder, Model.DRIFT_MODEL_FILENAME)
            else:
                filename = "drift_archives/{}_model/{}/{}".format(WML_SPARK_VERSION_SUPPORT_MAPPING[self._args.env], scikit_folder, Model.DRIFT_MODEL_FILENAME)
