...
FAKERS = {
    ...
    "email": generic(lambda _: FAKER.email(domain="example.com"), doc='Returns an @example.com email address'),
...
def mask_pii(table: str, pii_spec, dsn, keepers, fixed):
    ...
    if keepers is None:
        ...
        read_cursor.execute(
            f"SELECT * FROM {table} WHERE {pks[0]} NOT IN ({','.join([p]*len(keepers))})", keepers
        )
