...
class DatasetTestCase(SdkIntegrationTestCase):
    ...
    def setUp(self):
        super().setUp()
        # n.b. MAKE SURE CATALOGUE PREVIEW BUCKET IS SET CORRECTLY IN .ENV (MUST EXIST DATA_PREVIEW_BUCKET_NAME=preview-bucket)
        # I set it back to the bucket we create at the start, as I know that will exist - it will put preview stuff there
        self._setup_bucket_and_prefix('preview-bucket', 'test')
    ...
    def test_get_unknown_dataset_raises_dataset_not_found_error(self):
        with self.assertRaises(CatalogueEntityNotFoundException):
            self.client.datasets.get("unknown")
    ...
    def test_can_create_with_data_preview(self):
        ...
        dataset = self.client.register_dataset(bidet)
    ...
    def test_can_get_dataset_by_short_code(self):
        ...
        dataset_by_short_code = self.client.datasets.get(short_code=short_code)
    ...
    def test_get_dataset_by_short_code_returns_404_if_organisation_does_not_match(self):
        ...
        with pytest.raises(CatalogueEntityNotFoundException):
            self.client.datasets.get(
                short_code='testdatasetfunctions', organisation_short_code='abc'
            )
...
def test_get_dataset_by_short_code_uses_cache():
    dataset_id = f'testGetByShortCodeUsesCache{str(uuid.uuid4())[:8]}'
    short_code = f'testGetByShortCodeUsesCache{str(uuid.uuid4())[:8]}'
    dsm = DatasetModule()

    # Mock call to catalogue `datasets`.
    dsm._client = MagicMock()
    datasets_mock: Dict[str, DatasetModel] = {short_code: DatasetModel(short_code=short_code)}
    dsm._client.datasets.return_value = datasets_mock

    # Mock call to identity `visible_orgs`.
    dsm._visible_orgs = MagicMock()
    identity_visible_orgs_mock = {
        'data': [{
            'id': dataset_id,
            'attributes': {
                'short_code': short_code,
    ...
    dsm._visible_orgs.return_value = identity_visible_orgs_mock

    # First call to `get`.
    dataset_by_short_code_original: DatasetModel = dsm.get(short_code=short_code)
    assert dataset_by_short_code_original.short_code == short_code

    # Only one call to get the datasets so far.
    assert dsm._client.datasets.call_count == 1

    # The call to identity must only be called one time, the second call goes to the cache instead.
    assert dsm._visible_orgs.call_count == 1

    # Second call to `get`.
    dataset_by_short_code_from_cache: DatasetModel = dsm.get(short_code=short_code)
    assert dataset_by_short_code_from_cache == dataset_by_short_code_original

    # Second call to get the datasets (we have not cached this call).
    assert dsm._client.datasets.call_count == 2

    # The call to identity must only be called one time, the second call goes to the cache instead.
    assert dsm._visible_orgs.call_count == 1

    # Testing the behaviour of cache TTL expiry is already done here:
    # https://github.com/tkem/cachetools/blob/master/tests/test_ttl.py#L31
    # so we do not need to re-test.
