...
PIP_VERSIONS_LIST = [
    ...
    '20.3.3',
    # Latest pip on 30th January 2021. Commented out as it takes too long as the build betas and
    # bleeding edge releases as it uses backtracking to resolve the dependencies.
    #'21.0.1',
]
...
class RequirementsModifier:
    ...
    def set_requirement_details(self):
        url = f'https://pypi.org/pypi/{self.package}/{self.current_version}/json'
        import requests
        try:
            ...
            print()
            raise Exception(
                f'Nox setup failed: Failed to get valid response from pypi,'
                f'\nurl: {url}'
                f'\nexc type: {type(exc)}'
            ) from exc
        ...
        try:
            ...
            raise Exception(
                f'Nox setup failed: Failed to get valid json from pypi response:'
                f'\nurl: {url}'
        ...
        # Guard
        if 'releases' not in dc:
            raise Exception(
                'Nox setup failed: Failed to find `releases` key in the pypi json response:'
                f'\nurl: {url}'
                f'\nresponse text: {response.text}'
            )
