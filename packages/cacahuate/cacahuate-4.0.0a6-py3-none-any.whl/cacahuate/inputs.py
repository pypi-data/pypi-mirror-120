...
# TODO: dry. maybe the inputs should not care about the xmls
def get_text(node):
    node.normalize()
...
class Input:
    ...
    @property
    def type(self):
        return self.__class__.__name__[:-len('Input')].lower()
...
class BoolInput(RadioInput):
    ...
    def make_caption(self, value):
        ...
        for opt in self.options:
            clean = self.validate(opt.value, '<unknown>')
            if clean == value:
                return opt.label
...
class DateInput(DatetimeInput):
    def make_caption(self, value):
        ...
        return datetime.strptime(value, '%Y-%m-%dT%H:%M:%S.%fZ').strftime('%x')
