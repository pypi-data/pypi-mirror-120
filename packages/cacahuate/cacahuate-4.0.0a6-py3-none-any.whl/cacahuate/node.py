...
class Form:
    ...
    def __init__(self, element, context=None):
        ...
        for input_el in element.getElementsByTagName('input'):
            self.inputs.append(make_input(element=input_el, context=context))
        ...
        for inpt in self.inputs:
            try:
                ...
                input_description = {
                    **inpt.to_json(),
                    'value': value,
                    'value_caption': inpt.make_caption(value),
                    'state': 'valid',
                }
...
class Call(FullyContainedNode):
    ...
    def work(self, config, state, mongo):
        ...
        execution = xml.start(node, collected_input, mongo, '__system__')
...
class CaptureValue:
    ...
    def capture(self, data, parentpath):
        ...
        try:
            capturable = jsonpathparse(path).find(data)[0].value
        except IndexError:
