...
class RuExpressions(EnRuExpressions):
    expressions = EnRuExpressions.expressions + ('rep_positional_spaces_for_words',)

    rep_positional_spaces_for_words = {
        'after': (
            'без',
            'перед',
            'при',
    ...
    def expr_rep_positional_spaces_for_words(self):
        """
        Replaces whitespaces after and before certain words with non-breakable space.
        """
        after_words = self.rep_positional_spaces_for_words['after']
        return [(fr'(?<={word}){WHSP}+', NBSP) for word in after_words]
