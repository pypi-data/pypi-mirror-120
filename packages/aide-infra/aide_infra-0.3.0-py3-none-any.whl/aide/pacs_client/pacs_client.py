...
class AbstractPacsClient(abc.ABC):
    ...
    @abc.abstractmethod
    def store_dicom_series(self, datasets: Iterator[pydicom.Dataset]):
        raise NotImplementedError
...
class PacsClient(AbstractPacsClient):
    ...
    def _associate(self,
                   requested_contexts: Iterable[PresentationContext]):
        ae = AE(self.ae_title)
    ...
    def store_dicom_series(self, datasets: Iterator[pydicom.Dataset]):
        ...
        try:
            for i, ds in enumerate(datasets):
                if i == 0:
                    assoc = self._associate(self._build_context(ds))
    ...
    def store_dicom_dataset(self, dataset: pydicom.Dataset):
        assoc = None
        try:
            assoc = self._associate(self._build_context(dataset))
