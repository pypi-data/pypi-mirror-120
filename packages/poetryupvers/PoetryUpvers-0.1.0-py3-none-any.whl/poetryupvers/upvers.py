...
class PyProjectUpVers():
    ...
    def write_version_file(self):
        """
            Write VERSION file containig the version. To be used by a tool liek release-cli
        """
    ...
    def get_message_config(self, messages_file=None):
        ...
        if messages_file:
            ...
            path = "./messages.json"
        if not exists(path):
            path = f"{util.get_install_dir(self.__class__)}/messages.json"
    ...
    def set_version(self, version):
        ...
        self.pyproject["tool"]["poetry"]["version"] = str(version)
