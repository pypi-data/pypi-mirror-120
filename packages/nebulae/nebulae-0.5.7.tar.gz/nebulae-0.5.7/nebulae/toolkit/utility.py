...
class GPUtil():
    ...
    def status(self):
        ...
        with open('./temp_gpu_stat.csv', 'r') as f:
            ...
            area_t = area_u = area_m = 0
            while True:
                line = f.readline()
                if not line:
                    break
                line = line.strip().split(', ')
                if len(line)!=4 or line[-1][-3:]!='MiB': # skip title line and error lines
                    continue
                ...
                if t_ == '': # first line
                    ...
                    area_t += duration * (curr_t[gpu_id] + prev_t[gpu_id]) / 2
                    area_u += duration * (curr_u[gpu_id] + prev_u[gpu_id]) / 2
                    area_m += duration * (curr_m[gpu_id] + prev_m[gpu_id]) / 2
        ...
        for i, g in enumerate(self.gpus):
            stat += '\n| {:^3d} | {:<25s} |  {:<2.1f}ºC   | {:2.1f}%  | {:<5.1f}MiB |'.format(
                i, g, area_t[i]/t, area_u[i]/t, area_m[i]/t)
        stat += '\n+' + 73 * '-' + '+'
