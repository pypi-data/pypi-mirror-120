...
class DashBoard(object):
    ...
    def gauge(self, entry, mile, epoch, mpe, stage, interval=1, duration=None, bias=0):
        ...
        if flag_display:
            ...
            if curve_exists:
                print(f'\033[{self.param["divisor"]+bias+7}A')
            else:
                print(f'\033[2A')
        if flag_epoch_end:
            for _ in range(self.param["divisor"]+bias+6):
                print(w * ' ')
            print(f'\033[{self.param["divisor"]+bias+7}A')
            ordinal = self._getOridinal(epoch)
