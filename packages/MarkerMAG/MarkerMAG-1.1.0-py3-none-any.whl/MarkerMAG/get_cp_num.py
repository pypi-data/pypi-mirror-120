...
def get_16s_copy_num(args):
    ...
    report_and_log(('Get_cp_num: Running V-Xtractor on 16S rRNA genes'), pwd_log_file, keep_quiet)
    ...
    if mag_depth_gc_txt is None:
        ...
        if reads_subsample_pct_for_mag_cov_gc != 100:
            report_and_log(('Get_cp_num: Subsampling reads for MAG coverage estimation'), pwd_log_file, keep_quiet)
        ...
        report_and_log(('Get_cp_num: Mapping reads to combined prefixed MAGs'), pwd_log_file, keep_quiet)
        ...
        # sort filtered_sam
        report_and_log(('Get_cp_num: Sorting filtered sam file'), pwd_log_file, keep_quiet)
        ...
        # get depth file
        report_and_log(('Get_cp_num: Getting depth with samtools'), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Get_cp_num: Reading in MAG sequences'), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Get_cp_num: reading in Barrnap outputs'), pwd_log_file, keep_quiet)
    ...
    if (mag_depth_gc_txt is None) or (mag_gc_bias_folder is None):
        ...
        report_and_log(('Get_cp_num: Reading in depth file'), pwd_log_file, keep_quiet)
        ...
        report_and_log(('Get_cp_num: getting Coverage and GC bias for %s Genomes with %s cores' % (len(get_gc_bias_argument_lol), num_threads)), pwd_log_file, keep_quiet)
        pool = mp.Pool(processes=num_threads)
    ...
    if sorted_read_to_16s_sam_file is None:
        ...
        report_and_log(('Get_cp_num: Mapping 16S reads to all 16S rRNA gene sequences'), pwd_log_file, keep_quiet)
        ...
        report_and_log(('Get_cp_num: sorting sam file by read id'), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Get_cp_num: Splitting sorted sam file '), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Get_cp_num: Parsing alignments of reads to all 16S with multiple threads'), pwd_log_file, keep_quiet)
    pool_parse_sam16s = mp.Pool(processes=num_threads)
    ...
    report_and_log(('Get_cp_num: Parsing alignments of reads to linked 16S with multiple threads'), pwd_log_file, keep_quiet)
    pool_parse_sam16s = mp.Pool(processes=num_threads)
    ...
    report_and_log(('Get_cp_num: filtering sam file for all 16S'), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Get_cp_num: filtering sam file for linked 16S'), pwd_log_file, keep_quiet)
    ...
    # sort sam file
    report_and_log(('Get_cp_num: sorting filtered sam file by contig id'), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Get_cp_num: Getting depth directly from sam file (all 16S)'), pwd_log_file, keep_quiet)
    ...
    report_and_log(('Get_cp_num: Getting depth directly from sam file (linked 16S)'), pwd_log_file, keep_quiet)
    ...
    # Final report
    report_and_log(('Get_cp_num: Estimating 16S copy number done!'), pwd_log_file, keep_quiet)
