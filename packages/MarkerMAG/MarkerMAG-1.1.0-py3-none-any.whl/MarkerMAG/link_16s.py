...
MarkerMAG link -p Demo -marker 16S.fa -mag MAGs -x fa -r1 R1.fa -r2 R2.fa -t 6 -get_cn 
...
def link_16s(args, config_dict):
    ...
    report_and_log(('mismatch:\t%s%s'                 % (mismatch_cutoff, '%')),              pwd_log_file, keep_quiet)
    ...
    report_and_log(('min_link_num_gnm:\t%s'           % min_link_num),                        pwd_log_file, keep_quiet)
    report_and_log(('min_link_num_ctg:\t%s'           % (ctg_level_min_link)),                pwd_log_file, keep_quiet)
    report_and_log(('rd2_end_seq_len:\t%s bp'         % (end_seq_len)),                       pwd_log_file, keep_quiet)
    report_and_log(('max_short_cigar_pct:\t%s,%s'      % (max_short_cigar_pct_cutoff_linking_reads_num_high, max_short_cigar_pct_cutoff_linking_reads_num_low)),   pwd_log_file, keep_quiet)
    if skip_calculate_copy_num is False:
        report_and_log(('Parameters for estimating copy number of 16S rRNA genes'), pwd_log_file, keep_quiet)
        report_and_log(('MAG_cov_subsample_pct:\t%s%s'  % (args['subsample_pct'], '%')),      pwd_log_file, keep_quiet)
        report_and_log(('min_insert_size_16s:\t%s bp'   % args['min_insert_size_16s']),       pwd_log_file, keep_quiet)
        report_and_log(('ignore_ends_len_16s:\t%s bp'   % args['ignore_ends_len_16s']),       pwd_log_file, keep_quiet)
        report_and_log(('ignore_lowest_pct:\t%s%s'      % (args['ignore_lowest_pct'], '%')),  pwd_log_file, keep_quiet)
        report_and_log(('ignore_highest_pct:\t%s%s'     % (args['ignore_highest_pct'], '%')), pwd_log_file, keep_quiet)
        report_and_log(('both_pair_mapped:\t%s'         % args['both_pair_mapped']),          pwd_log_file, keep_quiet)
    ...
    # QC 16S
    report_and_log(('Round 1: Quality control provided 16S rRNA gene sequences to:'), pwd_log_file, keep_quiet)
    if no_polish is False:
        report_and_log(('Round 1: Remove non-16S sequences (if any)'), pwd_log_file, keep_quiet)
    else:
        report_and_log(('Round 1: Remove sequences shorter than %s bp' % min_len_16s), pwd_log_file, keep_quiet)
    if no_cluster is False:
        report_and_log(('Round 1: Cluster them at %s%s identity and keep only the longest one from each cluster' % (cluster_iden, '%')), pwd_log_file, keep_quiet)
...
if __name__ == '__main__':
    ...
    link_16s_parser_marker.add_argument('-cluster_iden',    required=False, metavar='', type=float, default=99,             help='Identity for 16S clustering, (default: %(default)s)')
    link_16s_parser_marker.add_argument('-no_cluster',      required=False, action="store_true",                            help='Skip checking if there are non-16S sequences at the end of provided 16S rRNA gene sequences. Specify only if you are confident with the quality of your 16S rRNA gene sequences. ')
...
8. include a small test dataset
9. use subset of reads to calculate MAG coverage and GC bias
'''
