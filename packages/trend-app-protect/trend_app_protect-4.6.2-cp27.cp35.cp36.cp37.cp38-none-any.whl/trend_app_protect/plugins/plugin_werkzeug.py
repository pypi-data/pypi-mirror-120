...
def add_hooks(run_hook, get_agent_func=None, timer=None):
    ...
    # Install a hook to capture the Request params
...
def hook_werkzeug_input_params(run_hook, get_agent_func, timer):
    """
    Wrap the `Request` methods that access values from request
    forms and query parameters. The `values()` call will call the
    two other properties (args/form) and combine them.
    """
    import werkzeug

    if StrictVersion(werkzeug.__version__) < StrictVersion("2.0"):
    ...
    if not (
        isinstance(Request.args, cached_property)
        and isinstance(Request.form, cached_property)
    ):
        log.warning("Werkzeug Request values not cached_properties")
        raise Exception("Request values must be cached_properties")
    ...
    Request.args = WrappedProperty(Request.args)
    Request.form = WrappedProperty(Request.form)
