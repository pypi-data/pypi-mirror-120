...
class Agent(object):
    ...
    def __init__(self, config=None, plugin_manager=None, deployment_type=None):
        if config is None:
            config = Config()
        ...
        self._agent = libagent.Agent(
            __version__, self._config, deployment_type)
    ...
    def run_hook(self, plugin, hook, meta, transaction=None):
        ...
        # returned if there was an unexpected error, or vmcode etc hadn't
        # been loaded and fail_closed=true
        if result.get("agent_error"):
            raise AgentError(result["agent_error"])
