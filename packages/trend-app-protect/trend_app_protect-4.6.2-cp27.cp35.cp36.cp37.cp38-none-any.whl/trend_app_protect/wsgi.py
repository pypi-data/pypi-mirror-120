...
class WsgiWrapper(object):
    ...
    def __init__(self, agent, app, transaction_uuid_header):
        self._agent = agent
        # if possible, keep a weakref of the original app to prevent
        # cyclical references: https://github.com/pallets/flask/issues/3761
        # WeakMethod only exists in python3, and only works with bound methods
        if hasattr(app, '__self__') and hasattr(weakref, 'WeakMethod'):
            self._app = weakref.WeakMethod(app)
        else:
            self._app = app
    ...
    def __call__(self, environ, start_response):
        ...
        if (hasattr(weakref, 'WeakMethod')
                and isinstance(app, weakref.WeakMethod)):
            app = app()
        ...
        self._local.wsgi_transaction = WsgiTransaction(
            self._agent, app, transaction, self._transaction_uuid_header)
    ...
    def handle_request(self, environ, start_response):
        ...
        try:
            ...
            if inspect_body:
                ...
                self._wrapped_input = WsgiInputWrapper(
                    self._transaction, self._agent, environ["wsgi.input"],
                    content_length, environ.get("wsgi.input_terminated"))
                environ["wsgi.input"] = self._wrapped_input
            ...
            # Some clients panic if the request body isn't entirely consumed
            # before the connection is closed, so read it here before blocking
            if (environ.get("CONTENT_LENGTH")
                    or environ.get("wsgi.input_terminated")):
                input_stream = self._wrapped_input or WsgiInputWrapper(
                    self._transaction, self._agent,
                    environ["wsgi.input"], environ.get("CONTENT_LENGTH"),
                    environ.get("wsgi.input_terminated")
                )
                input_stream.exhaust()
...
class WsgiInputWrapper(object):
    ...
    def __init__(self, transaction, agent, original_input, content_length,
        ...
        self._input_terminated = input_terminated

    def report_chunk(self, chunk, requested_size=None):
        try:
    ...
    def _data_read(self, read, requested):
    ...
    def exhaust(self, chunk_size=1024 * 64):
        """Read remaining input, without reporting"""
        if self._content_left is None:
            self._input.read(None)
        else:
            while self._content_left:
                to_read = min(self._content_left, chunk_size)
                chunk = self._input.read(to_read)
                self._data_read(len(chunk), to_read)
        self.report_done()
    ...
    def read(self, *args, **kwargs):
        ...
        if args:
            ...
            requested_size = kwargs.get("length")

        if self._done_reported:
            # if it calls read again after an exception was raised
            # it's probably just trying to exhaust the stream
            # so return whatever as long as it's the expected length
            if self._input_terminated or not requested_size:
                return b""
            return b"\0" * requested_size

        chunk = self._input.read(*args, **kwargs)
        self.report_chunk(chunk, requested_size)
