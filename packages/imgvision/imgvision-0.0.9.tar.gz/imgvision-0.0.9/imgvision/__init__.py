...
def downsample(img,scale,type = 'spatial',band2 = None):
    '''
        :img 被下采样图像
    ...
    if type.lower() =='spatial':
        ...
        需要传入band2 列表，进行插值
        '''
        Band = np.array([400, 403, 406, 409, 412, 415, 418, 420, 423, 426, 429, 432, 435, 438, 441, 443, 446, 449, 452, 455, 458,
             ...
             646, 649, 652, 655, 658, 661, 664, 666, 669, 672, 676, 679, 682, 685, 688, 691, 694, 697, 700])
        if band2 ==None:
            band2 = np.array(range(400, 710, 10))
        _img = imgshape(img)
        img = _img.flatten()
