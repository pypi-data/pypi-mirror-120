    ...
    for i in range(grid.shape[0]):
        for j in range(grid.shape[1]):
            for k in range(grid.shape[2]):
                if result[i,j,k] == np.nan: # we have to search for the nearest neighbor and take its value - this is usually a rare edge case
                    pos = np.array([gx*dx, gy*dy, gz*dz])
                    dx = np.zeros(3)
                    min_dist = 1e100
                    min_dist_idx = -1
