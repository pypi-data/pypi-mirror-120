...
class AdamW_AGC(Optimizer):
    r"""Implements AdamW algorithm with adaptive gradient clipping (AGC).
    .. math::
       \begin{aligned}
            &\rule{110mm}{0.4pt}                                                                 \\
    ...
    This implementation of AdamW was taken from the official PyTorch Sources and the code for the AGC was adapted from
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=1e-2, amsgrad=False, clipping=1e-2, agc_eps=1e-3):
        if not 0.0 <= lr:
    ...
    def __setstate__(self, state):
        super(AdamW_AGC, self).__setstate__(state)
