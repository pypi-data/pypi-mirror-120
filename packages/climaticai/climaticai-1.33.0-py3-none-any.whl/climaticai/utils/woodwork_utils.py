...
from climaticai.utils.gen_utils import is_all_numeric
import dask
...
def _daskarray_to_daskdataframe(array):
    if len(array.shape) == 1:
        data = dd.from_dask_array(array, columns=['predictions'])
    else:
        data = dd.DataFrame(array)
    return data
...
def _raise_value_error_if_nullable_types_detected(data):
    try:
        ...
        if cols_with_nullable_types:
            print(
                "climaticai does not support the new pandas nullable types because "
...
def infer_feature_types(data, feature_types=None):
    ...
    if isinstance(data, list):
        ...
        data = _daskarray_to_daskdataframe(data)
