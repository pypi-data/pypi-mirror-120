...
import climaticai
from climaticai.utils import get_logger


def print_info():
    """Prints information about the system, climaticai, and dependencies of climaticai."""
    logger = get_logger(__name__)
    logger.info("climaticai version: %s" % climaticai.__version__)
    logger.info("climaticai installation directory: %s" % get_climaticai_root())
    print_sys_info()
    print_deps()
...
def get_climaticai_root():
    """Gets location where climaticai is installed.

    Returns:
        Location where climaticai is installed.
    """
    return os.path.dirname(climaticai.__file__)
