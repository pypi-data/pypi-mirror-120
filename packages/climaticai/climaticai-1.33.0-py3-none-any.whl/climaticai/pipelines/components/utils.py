"""Utility methods for climaticai components."""
import inspect
...
from climaticai.exceptions import MissingComponentError
from climaticai.model_family.utils import handle_model_family
from climaticai.pipelines.components.component_base import ComponentBase
from climaticai.pipelines.components.estimators.estimator import Estimator
from climaticai.pipelines.components.transformers.transformer import Transformer
from climaticai.problem_types import ProblemTypes, handle_problem_types
from climaticai.utils import get_importable_subclasses
...
class WrappedSKClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, pipeline):
        """Scikit-learn classifier wrapper class. Takes an climaticai pipeline as input and returns a scikit-learn classifier class wrapping that pipeline.

        Args:
            pipeline (PipelineBase or subclass obj): climaticai pipeline.
        """
...
class WrappedSKRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, pipeline):
        """Scikit-learn regressor wrapper class. Takes an climaticai pipeline as input and returns a scikit-learn regressor class wrapping that pipeline.
...
def scikit_learn_wrapped_estimator(climaticai_obj):
    """Wraps an climaticai object as a scikit-learn estimator."""
    from climaticai.pipelines.pipeline_base import PipelineBase

    """Wrap an climaticai pipeline or estimator in a scikit-learn estimator."""
    if isinstance(climaticai_obj, PipelineBase):
        if climaticai_obj.problem_type in [
            ProblemTypes.REGRESSION,
            ProblemTypes.TIME_SERIES_REGRESSION,
        ]:
            return WrappedSKRegressor(climaticai_obj)
        elif (
            climaticai_obj.problem_type == ProblemTypes.BINARY
            or climaticai_obj.problem_type == ProblemTypes.MULTICLASS
        ):
            return WrappedSKClassifier(climaticai_obj)
    else:
        # climaticai Estimator
        if climaticai_obj.supported_problem_types == [
            ProblemTypes.REGRESSION,
            ProblemTypes.TIME_SERIES_REGRESSION,
        ]:
            return WrappedSKRegressor(climaticai_obj)
        elif climaticai_obj.supported_problem_types == [
            ProblemTypes.BINARY,
            ...
            return WrappedSKClassifier(climaticai_obj)
    raise ValueError("Could not wrap climaticai object in scikit-learn wrapper.")


def generate_component_code(element):
    """Creates and returns a string that contains the Python imports and code required for running the climaticai component.
