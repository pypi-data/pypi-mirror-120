...
from climaticai.pipelines.components import ComponentBase, RandomForestClassifier
...
def test_scikit_learn_wrapper_invalid_problem_type():
    climaticai_pipeline = MulticlassClassificationPipeline([RandomForestClassifier])
    climaticai_pipeline.problem_type = None
    with pytest.raises(
        ValueError, match="Could not wrap climaticai object in scikit-learn wrapper."
    ):
        scikit_learn_wrapped_estimator(climaticai_pipeline)
...
def test_scikit_learn_wrapper(X_y_binary, X_y_multi, X_y_regression, ts_data):
    for estimator in [
        ...
        for problem_type in estimator.supported_problem_types:
            ...
            climaticai_pipeline = pipeline_class([estimator])
            scikit_estimator = scikit_learn_wrapped_estimator(climaticai_pipeline)
