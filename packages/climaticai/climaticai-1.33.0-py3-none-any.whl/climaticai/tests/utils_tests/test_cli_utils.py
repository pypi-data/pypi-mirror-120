...
from climaticai.__main__ import cli
from climaticai.utils.cli_utils import (
    get_climaticai_root,
...
def test_print_cli_info_cmd(caplog):
    ...
    assert result.exit_code == 0
    assert "climaticai version:" in caplog.text
    assert "climaticai installation directory:" in caplog.text
...
def test_print_info(caplog):
    print_info()
    out = caplog.text
    assert "climaticai version:" in out
    assert "climaticai installation directory:" in out
...
def test_get_climaticai_root(current_dir):
    root = os.path.abspath(os.path.join(current_dir, "..", ".."))
    assert get_climaticai_root() == root
