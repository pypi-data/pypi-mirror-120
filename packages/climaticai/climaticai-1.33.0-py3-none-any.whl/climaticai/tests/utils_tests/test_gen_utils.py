...
from climaticai.model_understanding.graphs import visualize_decision_tree
from climaticai.pipelines.components import ComponentBase
from climaticai.utils.gen_utils import (
    SEED_BOUNDS,
...
def test_import_or_raise_errors(dummy_importlib):
    ...
    with pytest.raises(ImportError, match="Missing optional dependency '_climaticai'"):
        import_or_raise("_climaticai")
    with pytest.raises(
        ImportError,
        match="Missing optional dependency '_climaticai'. Please use pip to install _climaticai. Additional error message",
    ):
        import_or_raise("_climaticai", "Additional error message")
    with pytest.raises(
        Exception,
...
def test_import_or_warn_errors(dummy_importlib):
    ...
    with pytest.warns(UserWarning, match="Missing optional dependency '_climaticai'"):
        import_or_raise("_climaticai", warning=True)
    with pytest.warns(
        UserWarning,
        match="Missing optional dependency '_climaticai'. Please use pip to install _climaticai. Additional error message",
    ):
        import_or_raise("_climaticai", "Additional error message", warning=True)
    with pytest.warns(
        UserWarning,
        match="An exception occurred while trying to import `attr_error_lib`: Mock Exception executed!",
    ):
...
@patch("climaticai.utils.gen_utils.import_or_raise")
