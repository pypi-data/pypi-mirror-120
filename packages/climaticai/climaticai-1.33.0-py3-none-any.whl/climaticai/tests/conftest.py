...
from climaticai.objectives import BinaryClassificationObjective
...
from climaticai.pipelines.components.ensemble.sklearn_stacked_ensemble_base import (
    _nonstackable_model_families,
)
from climaticai.pipelines.components.utils import _all_estimators
from climaticai.preprocessing import load_data
...
@pytest.fixture
def techsprint2021_local():
    currdir_path = os.path.dirname(os.path.abspath(__file__))
    data_folder_path = os.path.join(currdir_path, "data")
    techsprint2021_data_path = os.path.join(data_folder_path, "techsprint2021.csv")
    return load_data(
        path=techsprint2021_data_path,
        index="customerID",
        target="techsprint2021",
    )
...
class _AutoMLTestEnv:
    ...
    def _pipeline_class(self):
        return {
            ...
            ProblemTypes.TIME_SERIES_MULTICLASS: "climaticai.pipelines.TimeSeriesMulticlassClassificationPipeline",
            ProblemTypes.TIME_SERIES_BINARY: "climaticai.pipelines.TimeSeriesBinaryClassificationPipeline",
        }[self.problem_type]
    ...
    def test_context(
        ...
        mock_get_names = patch(
            "climaticai.pipelines.components.FeatureSelector.get_names", return_value=[]
        )
        ...
        if is_regression(self.problem_type):
            pipeline_to_mock = "climaticai.pipelines.BinaryClassificationPipeline"
        ...
        mock_optimize = patch(
            "climaticai.objectives.BinaryClassificationObjective.optimize_threshold",
            return_value=optimize_threshold_return_value,
        )

        mock_tell = patch("climaticai.tuners.skopt_tuner.Optimizer.tell")
        ...
        mock_sleep = patch(
            "climaticai.automl.AutoMLSearch._sleep_time", new_callable=sleep_time
        )
    ...
    X = pd.DataFrame(
        {
            ...
            "url": [
                "https://climaticai.alteryx.com/en/stable/",
                ...
                "https://www.climaticai.alteryx.com/en/stable/demos/text_input.html",
            ],
        }
    )
    X.ww.init(
