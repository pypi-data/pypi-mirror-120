...
from climaticai.automl.engine.cf_engine import CFClient, CFComputation, CFEngine
from climaticai.automl.engine.engine_base import (
    JobLogger,
    evaluate_pipeline,
    train_pipeline,
)
from climaticai.automl.engine.sequential_engine import SequentialEngine
from climaticai.automl.utils import AutoMLConfig
from climaticai.pipelines import BinaryClassificationPipeline
from climaticai.pipelines.pipeline_base import PipelineBase
from climaticai.tests.automl_tests.dask_test_utils import (
    DaskPipelineSlow,
...
def test_init(process_pool):
    with CFClient(process_pool) as client:
        ...
        with pytest.raises(
            TypeError,
            match="Expected climaticai.automl.engine.cf_engine.CFClient, received",
        ):
...
def test_cfengine_convenience():
    ...
    with pytest.raises(
        TypeError, match="Expected climaticai.automl.engine.cf_engine.CFClient, received"
    ):
