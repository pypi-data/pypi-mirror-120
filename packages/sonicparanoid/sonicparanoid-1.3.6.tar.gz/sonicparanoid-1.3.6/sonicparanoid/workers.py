...
def dmnd_search(inSeq, dbSeq, dbDir: str, outDir: str, sensitivity: str = "sensitive", indexDB: bool = False, useBlastDb: bool = False, cbstats: int = 1, threads: int = 4, debug: bool = False) -> Tuple[str, float, float]:
    ...
    searchCmd: str = f"{get_dmnd_path()} blastp --query {inSeq} --db {targetDBpath} {idxSettings} --out {blastOutPath} -p {threads:d} {sensitivity} --comp-based-stats {cbstats} --quiet -f 6 qseqid sseqid qstart qend sstart send bitscore --max-hsps {maxHspsPerTarget}"
    if debug:
...
# TODO: this the 'complicated version' with multiple job type
# it can cause deadlocks in when perfrming the final alignment
# Remove once a version with no deadloack has been implemented
'''
def perform_parallel_essential_alignments(requiredAln: Dict[str, Tuple[float, int, int]], protCntDict:Dict[str, int], spSizeDict: Dict[str, int], runDir:str, dbDir:str, alnDir:str, create_idx:bool=True, sensitivity:float=4.0, alnTool: str = "mmseqs", dmndSens:str="sensitive", cutoff:int=40, pmtx:str="blosum62", essentialMode:bool=True, threads:int=4, keepAln:bool=False, compress:bool=False, complev:int=5, debug:bool=False) -> None:
    auxDir: str = os.path.join(runDir, "aux")
    """Create FASTA subsets in parallel."""
    if debug:
        print("\nperform_parallel_essential_alignments :: START")
    ...
    if len(reqSpDict) > 0:
        ...
        # overwrites the create_idx variable
        if create_idx:
            create_idx = check_storage_for_db_indexing(dbDir, reqSp=len(reqSpDict), gbPerSpecies=0.95, debug=debug)

        # create databases
        sys.stdout.write(f"\nCreating {len(reqSpDict)} {alnToolName} databases...\n")
        # timer for databases creation
        start_time: float = time.perf_counter()

        # create databases in parallel
        parallel_dbs_creation(list(reqSpDict.keys()), fastaDir, dbDir, create_idx=create_idx, alnTool=alnTool, dmndSens=dmndSens, threads=threads, debug=debug)
        # end time for databases creation
        end_time: float = time.perf_counter()
        sys.stdout.write(f"\n{alnToolName} databases creation elapsed time (seconds):\t{round(end_time - start_time, 3)}\n")
        # delete timers
    ...
    # Set variables shared among processes
    # completedAlnCnt: int = mp.Value("i", 0)
    sharedValues: List[int] = mp.Array("i", [0, len(requiredAln), 0, threads])
    sharedValsLock = mp.Lock()

    # create the queue and start adding
    essential_queue: mp.Queue.queue = mp.Queue(maxsize=len(requiredAln) + threads)
    # directory with the original alignments
    originalAlnDir = os.path.join(os.path.dirname(os.path.dirname(runDir)), "alignments")
    if not os.path.isdir(originalAlnDir):
        sys.stderr.write("\nERROR: The directory with alignments was not found.")
    ...
    for pair, tpl in requiredAln.items():
        ...
        # proteome pair as tuple: e.g. "1-2" as ("1", "2")
        # and sequence counts  and proteome sizes for each proteome
        essential_queue.put(((tmpA, tmpB), tpl[1], tpl[2], protCntDict[tmpA], protCntDict[tmpB], spSizeDict[tmpA], spSizeDict[tmpB]))
    ...
    # Queue to contain the execution times
    results_queue: mp.Queue.queue = mp.Queue(maxsize=len(requiredAln))

    # List of running jobs
    runningJobs: List[Any] = []

    if alnTool == "diamond":
        runningJobs = [mp.Process(target=consume_dmnd_aln_jobs, args=(essential_queue, results_queue, runDir, dbDir, alnDir, create_idx, keepAln, dmndSens, cutoff, compress, complev, sharedValues, sharedValsLock)) for i_ in range(threads)]
    elif alnTool == "mmseqs":
        runningJobs = [mp.Process(target=consume_mmseqs_aln_jobs, args=(essential_queue, results_queue, runDir, dbDir, alnDir, keepAln, sensitivity, cutoff, pmtx, compress, complev)) for i_ in range(threads)]
    elif alnTool == "blast":
        runningJobs = [mp.Process(target=consume_blast_aln_jobs, args=(essential_queue, results_queue, runDir, dbDir, alnDir, keepAln, cutoff, compress, complev)) for i_ in range(threads)]
    ...
    align_start = time.perf_counter()
    # write some message...
    sys.stdout.write(f"\nPerforming {len(requiredAln)} alignment jobs using {alnToolName}...")
    # will contain the results from an alignment job
    resList: List[Tuple[str, float, float, float, float, float]] = []
    ...
    # stop the counter for the alignment time
    sys.stdout.write(f"\nElapsed time for all-vs-all alignments using {alnToolName} (seconds):\t{round(time.perf_counter() - align_start, 3)}\n")
    sys.exit("DEBUG: @workers.perform_parallel_essential_alignments, after jobs completition")
'''
