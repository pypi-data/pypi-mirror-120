...
def sin_cos_transformation(df: DataFrame, col: str, max_val: int) -> DataFrame:
    """
    ...
    """
    df[col + '_sin'] = sin(2 * pi * df[col] / max_val)  # sine transformation
    df[col + '_sin'] = df[col + '_sin'].astype('float32')

    df[col + '_cos'] = cos(2 * pi * df[col] / max_val)  # cosine transformation
