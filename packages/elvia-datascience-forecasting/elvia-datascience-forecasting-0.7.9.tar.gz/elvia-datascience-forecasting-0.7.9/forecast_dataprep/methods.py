...
def ingest_data(  # NOSONAR
    ...
    hist_cons_df['datetime_utc'] = pd.to_datetime(hist_cons_df['datetime_utc'],
    ...
    if inference:
        ...
        for hist_feat in Features.SHIFT_HOURS:
            ...
            hours_req = pd.date_range(
                ...
                timedelta(hours=hist_feat - forecast_horizon + 1),
                freq='H',
                tz='utc')

            # Interesection between required hours and existing hours
            missing_hours = [
                hour_req for hour_req in hours_req
                if hour_req not in hist_cons_df.index
            ]
        ...
        minimum_hour = min(hist_cons_df.index[-1].replace(tzinfo=None),
        ...
        inference_df = pd.DataFrame(index=pd.date_range(
            start=minimum_hour,
        ...
        hist_cons_df = hist_cons_df.append(inference_df).rename_axis(
            'datetime_utc').reset_index(drop=False)

        # Set the trafo_id to the value of the first entry
        hist_cons_df.loc[hist_cons_df['trafo_id'].isna(),
                         'trafo_id'] = hist_cons_df['trafo_id'][0]

        # If any duplcations in the index, remove it by keeping the first (happens because of timezones...)
        hist_cons_df = hist_cons_df.drop_duplicates(['datetime_utc'],
                                                    keep='first')
        ...
        df = _check_input_data(hist_cons_df,
                               ...
                               trafo_col='trafo_id',
                               len_history=len(
                                   hist_cons_df['datetime_utc'].unique()))
        ...
        # Remove outliers
        for station_id in hist_cons_df['trafo_id'].unique():
            v = hist_cons_df.loc[hist_cons_df['trafo_id'] == station_id,
                                 'value_wh']
            w = _remove_outliers(v, -outliers_exclusion, outliers_exclusion)
            hist_cons_df.loc[hist_cons_df['trafo_id'] == station_id,
                             'value_wh'] = w.values

        log_counter = add_debug_trace(
            f'df.shape after removing outliers: {str(hist_cons_df.shape)}',
            logger, endpoint_request, log_counter)
        log_counter = add_debug_trace(
    ...
    if 'trafo_id' in hist_cons_df.columns:
        ...
        log_counter = add_debug_trace(
            'DataNotFound: Could not find column trafo_id in hist_cons_df',
            logger, endpoint_request, log_counter)
        raise DataNotFound
    # The logic is present in the function add_metadata and create the column substation_id
    hist_cons_df['substation_id'] = hist_cons_df['trafo_id'].apply(
        add_metadata, df_metadata=df_metadata)
    # Logging the step above
    trafo_ids_uni = hist_cons_df['trafo_id'].unique()
    station_ids_uni = [
        hist_cons_df.loc[hist_cons_df['trafo_id'] == v, 'value_wh'].unique()
        for v in trafo_ids_uni
    ]
    log_counter = add_debug_trace(
    ...
    hist_cons_df = _cast_type_df(hist_cons_df, log_counter, logger,
                                 endpoint_request)
    log_counter = add_debug_trace(
...
def enrich(
    ...
    # Check the incoming table size
    log_counter = add_debug_trace(
        f'Shape after adding the historical features: {str(df.shape)}', logger,
        endpoint_request, log_counter)
    ...
    #
    log_counter = add_debug_trace(
        f'Shape of the DF before dropping unused columns : {str(df.shape)}',
        logger, endpoint_request, log_counter)
    df.drop([
        'time', 'substation_id', 'fylkesnavn', 'long', 'lat', 'kommunenavn',
    ...
    if inference:
        ...
        df.drop(['trafo_id'], axis=1, inplace=True)
        log_counter = add_debug_trace(
            f'Shape of the DF before dropping the NaN in the inference : {str(df.shape)}',
            logger, endpoint_request, log_counter)
...
def _cast_type_df(df: pd.DataFrame,
                  log_counter: int,
                  logger: Optional[Logger] = None,
    ...
    if 'long' in df.columns:
        ...
        log_counter = add_debug_trace(
            'DataNotFound: Could not find column long in hist_cons_df', logger,
            endpoint_request, log_counter)
        raise DataNotFound
    ...
    if 'lat' in df.columns:
        ...
        log_counter = add_debug_trace(
            'DataNotFound: Could not find column lat in hist_cons_df', logger,
            endpoint_request, log_counter)
        raise DataNotFound
