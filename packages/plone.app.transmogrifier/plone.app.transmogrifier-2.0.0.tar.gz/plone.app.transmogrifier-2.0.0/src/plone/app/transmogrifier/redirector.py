        ...
        for item in self.previous:
            ...
            for key in keys:
                ...
                for idx, obj in enumerate(paths):
                    ...
                    if not six.moves.urllib.parse.urlsplit(new_path).netloc:
                        new_path = leading + new_path[len(self.context_path) :]
                    new_path = six.moves.urllib.parse.urlunsplit(
                ...
                if item[key] != paths:
                    self.logger.debug(
                        "Updating %r path(s): %r => %r", key, old_paths, paths
                    )
                    item[key] = paths
            ...
            if pathkey:
                ...
                for old_path in old_paths:
                    ...
                    if (
                        ...
                        self.logger.debug(
                            "Adding %r redirect: %r => %r", pathkey, old_path, new_path
                        )
