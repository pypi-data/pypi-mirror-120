...
def custom_collate(batch):
    ...
    if isinstance(elem, collections.Sequence) and (len(elem) > 0) and isinstance(elem[0], six.string_types):
        return batch
    if isinstance(elem, collections.Sequence) and (len(elem) > 0) and isinstance(elem[0], int):
        return torch.tensor(batch)
    if isinstance(elem, collections.Sequence) and (len(elem) > 0) and isinstance(elem[0], float):
