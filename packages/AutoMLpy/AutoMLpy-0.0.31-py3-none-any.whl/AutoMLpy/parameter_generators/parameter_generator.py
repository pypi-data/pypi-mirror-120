...
class ParameterGenerator:
    ...
    def __init__(self,
        ...
        kwargs:
            ...
            save_dir: The default saving directory. (str)
            save_name: The default saving name. (str)
        ...
        self.default_save_dir (str): The default saving directory for all files that will be saved later.
                                     Default: "[current working directory]/
                                              parameter_generators_data/self.__class__.__name__".
                                     Set by the kwarg "save_dir".
        self.default_save_name (str): The default save name of the current object.
                                      Default: '-'.join(self._values_names).replace(' ', '_').
                                      Set by the kwarg "save_name".
        """
        # ------- Conversion tables -------- #
        ...
        self.default_save_dir = kwargs.get("save_dir",
                                           f"{os.getcwd()}/parameter_generators_data/{self.__class__.__name__}/")
    ...
    def get_optimization_table(self, **kwargs) -> pd.DataFrame:
        ...
        ----------
        kwargs:
            sort_scores: True to sort the scores else False. (bool)
            ascending: True to sort the scores in ascending values else False. (bool)
        ...
        if kwargs.get("sort_scores", False):
            df = df.sort_values("score", ascending=kwargs.get("ascending", True))
        return df
    ...
    def show_optimization(self, _parameter_name: Optional[str] = None, **kwargs):
        if _parameter_name is None:
            ...
            _fig_dir = kwargs.get('fig_dir', f'{self.default_save_dir}/figures/')
            ...
            if _save:
                os.makedirs(f"{_fig_dir}", exist_ok=True)
                plt.savefig(f"{_fig_dir}/{_save_name}.png", dpi=300)
            if _show:
                plt.show()

    def _show_optimization_of_hp(self, _parameter_name: str, _axe: plt.Axes, **kwargs):
        _optimization_table = self.get_optimization_table(**kwargs).sort_values("score", ascending=True)
        # get args
        _fontsize = kwargs.get("fontsize", ParameterGenerator.DEFAULT_FONT_SIZE)
        _title = kwargs.get('title', '')
        _legend = kwargs.get('legend', False)
        _score_as_xaxis = kwargs.get("score_as_xaxis", False)

        # plot sub fig
        _values = _optimization_table[_parameter_name].to_numpy()
        _scores = _optimization_table["score"].to_numpy()
        if _score_as_xaxis:
            _axe.plot(_scores, _values, '.', label=f"{_parameter_name}")
            if _parameter_name in self._param_idx_to_name:
                _axe.set_yticks(list(range(len(_values))))
                _axe.set_yticklabels(self.convert_idx_to_param(_parameter_name, _values))
            _axe.set_ylabel(f"{_parameter_name} [-]", fontsize=_fontsize)
            _axe.set_xlabel("Score [-]", fontsize=_fontsize)
        else:
            _axe.plot(_values, _scores, '.', label=f"{_parameter_name}")
            if _parameter_name in self._param_idx_to_name:
                _axe.set_xticks(list(range(len(_values))))
                _axe.set_xticklabels(self.convert_idx_to_param(_parameter_name, _values))
            _axe.set_xlabel(f"{_parameter_name} [-]", fontsize=_fontsize)
            _axe.set_ylabel("Score [-]", fontsize=_fontsize)
        # _axe.set_title(f"{_parameter_name}", fontsize=_fontsize)
        if _legend:
            _axe.legend(fontsize=_fontsize)
        return _axe
    ...
    def _show_full_optimization(self, **kwargs):
        ...
        _show = kwargs.get("show", True)
        _figsize = kwargs.get("figsize", (12, 10))
        _fig_dir = kwargs.get('fig_dir', f'{self.default_save_dir}/figures/')
        _save = kwargs.get("save", True)
        ...
        j = 0
        fig, subfigs = plt.subplots(k, k, tight_layout=True, sharex='all', figsize=_figsize)
