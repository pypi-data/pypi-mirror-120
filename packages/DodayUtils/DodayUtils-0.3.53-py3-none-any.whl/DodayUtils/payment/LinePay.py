...
class LinePay(DodayPayment):
	...
	def __init__(self, price, qr_data, server_url):
		super(IntegratedEPay, self).__init__(price, DodayPaymentType.linepay)
	...
	def execute(self):
		...
		"""
		self.linepay_terminate_flag = False
		logging.info("[Inside linepay Payment] start linepay helper, " + serverTime(TimeMode.STRING) + ", $" + self.price)
