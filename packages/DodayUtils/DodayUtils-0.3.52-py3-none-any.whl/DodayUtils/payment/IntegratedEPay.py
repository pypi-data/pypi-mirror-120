...
class IntegratedEPay(DodayPayment):
	...
	def execute(self):
		...
		"""
		self.epay_terminate_flag = False
		logging.info("[Inside EPay Payment] start epay helper, " + serverTime(TimeMode.STRING) + ", $" + self.price)
		# Main Epay function
		...
		if status == 0:
			...
			return {"indicator":True, "message":"payment succeed", "payment_method":self.payment_method}
		
		logging.error("[EPay Failed]: Payment Failed: "+str(self.payment_method))
		return {"indicator":False, "message":"payment failed", "payment_method":self.payment_method}
