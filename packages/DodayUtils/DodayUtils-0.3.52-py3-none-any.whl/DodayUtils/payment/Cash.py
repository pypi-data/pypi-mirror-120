...
class Cash(DodayPayment):
	...
	def execute(self):
		...
		"""
		self.cash_terminate_flag = False
		logging.info("[Inside Cash Payment] start cash helper, " + serverTime(TimeMode.STRING) + ", $" + self.price)
		...
		while total_num < self.price:
			...
			if dol != 0:
				self.gui_response_page.add_money_signal.emit(str(self.price), str(min(total_num, self.price)), str(max(0, total_num-self.price)))
				tout_start_time = time.time()
			...
			if self.cash_terminate_flag:
				...
				payment_feedback = self.gbridge_return_changes(total_num, self.plib)
				
				# Prepare return change and remaining change info and return
				ret_cash_info = self.prepare_return_change_info(payment_feedback)
				return {"indicator":False, "message":"cashing terminate", "remaining_changes":str(payment_feedback[-1]), 
			...
			payment_feedback = self.gbridge_return_changes(total_num-self.price, self.plib)
			ret_cash_info = self.prepare_return_change_info(payment_feedback)
			...
			if payment_feedback[-1] > 0:
				...
				return {"indicator":False, "message":"hopper no enough coins", "remaining_changes":str(payment_feedback[-1]), 
				"return_changes": ret_cash_info}
		...
		# timeout failure
		return {"indicator":False, "message":"payment timeout", "remaining_changes":str(payment_feedback[-1]), 
					"return_changes": ret_cash_info}
