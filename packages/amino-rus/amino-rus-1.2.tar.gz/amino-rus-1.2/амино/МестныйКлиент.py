        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f"{self.интерфейс}/create-chat-thread", json = информация, headers = self.заголовки) as результат:
                ...
                if ответ["code"] == 200:
                    ...
                    if ответ["api_status_code"] == 270:
                        raise ЗапрошеноПодтверждение(ответ)
                    elif ответ["api_status_code"] == 603:
                        raise ВасЗаблокировали(ответ["message"])
                    elif ответ["api_status_code"] == 1611:
                        raise ПриглашенияОтключены(ответ)
                    else:
                        raise Исключение(ответ)