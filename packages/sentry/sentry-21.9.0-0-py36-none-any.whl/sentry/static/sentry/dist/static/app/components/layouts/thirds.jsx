Object.defineProperty(exports, "__esModule", { value: true });
exports.Side = exports.Main = exports.HeaderNavTabs = exports.Header = exports.Title = exports.HeaderActions = exports.HeaderContent = exports.Body = void 0;
const tslib_1 = require("tslib");
const styled_1 = (0, tslib_1.__importDefault)(require("@emotion/styled"));
const navTabs_1 = (0, tslib_1.__importDefault)(require("app/components/navTabs"));
const overflowEllipsis_1 = (0, tslib_1.__importDefault)(require("app/styles/overflowEllipsis"));
const space_1 = (0, tslib_1.__importDefault)(require("app/styles/space"));
/**
 * Base container for 66/33 containers.
 */
exports.Body = (0, styled_1.default)('div') `
  padding: ${(0, space_1.default)(2)};
  margin: 0;
  background-color: ${p => p.theme.background};
  flex-grow: 1;

  @media (min-width: ${p => p.theme.breakpoints[1]}) {
    display: grid;
    grid-template-columns: 66% auto;
    align-content: start;
    grid-gap: ${(0, space_1.default)(3)};
    padding: ${(0, space_1.default)(3)} ${(0, space_1.default)(4)};
  }

  @media (min-width: ${p => p.theme.breakpoints[2]}) {
    grid-template-columns: minmax(100px, auto) 325px;
  }
`;
/**
 * Use HeaderContent to create horizontal regions in the header
 * that contain a heading/breadcrumbs and a button group.
 */
exports.HeaderContent = (0, styled_1.default)('div') `
  display: flex;
  flex-direction: column;
  justify-content: normal;
  margin-bottom: ${(0, space_1.default)(2)};
  overflow: hidden;
  max-width: 100%;

  @media (max-width: ${p => p.theme.breakpoints[1]}) {
    margin-bottom: ${(0, space_1.default)(1)};
  }
`;
/**
 * Container for action buttons and secondary information that
 * flows on the top right of the header.
 */
exports.HeaderActions = (0, styled_1.default)('div') `
  display: flex;
  flex-direction: column;
  justify-content: normal;
  min-width: max-content;

  @media (max-width: ${p => p.theme.breakpoints[1]}) {
    width: max-content;
    margin-bottom: ${(0, space_1.default)(2)};
  }
`;
/**
 * Heading container that includes margins.
 */
exports.Title = (0, styled_1.default)('h2') `
  font-size: ${p => p.theme.headerFontSize};
  font-weight: normal;
  line-height: 1.2;
  color: ${p => p.theme.textColor};
  margin-top: ${(0, space_1.default)(3)};
  /* TODO(bootstrap) Remove important when bootstrap headings are removed */
  margin-bottom: 0 !important;
  min-height: 30px;
  align-self: center;
  ${overflowEllipsis_1.default};

  @media (max-width: ${p => p.theme.breakpoints[1]}) {
    margin-top: ${(0, space_1.default)(1)};
  }
`;
/**
 * Header container for header content and header actions.
 *
 * Uses a horizontal layout in wide viewports to put space between
 * the headings and the actions container. In narrow viewports these elements
 * are stacked vertically.
 */
exports.Header = (0, styled_1.default)('div') `
  display: grid;
  grid-template-columns: minmax(0, 1fr);
  padding: ${(0, space_1.default)(2)} ${(0, space_1.default)(2)} 0 ${(0, space_1.default)(2)};
  background-color: transparent;
  border-bottom: 1px solid ${p => p.theme.border};

  @media (min-width: ${p => p.theme.breakpoints[1]}) {
    grid-template-columns: minmax(0, 1fr) auto;
    padding: ${(0, space_1.default)(2)} ${(0, space_1.default)(4)} 0 ${(0, space_1.default)(4)};
  }
`;
/**
 * Styled Nav Tabs for use inside a Layout.Header component
 */
exports.HeaderNavTabs = (0, styled_1.default)(navTabs_1.default) `
  margin: 0;
  border-bottom: 0 !important;

  & > li {
    margin-right: ${(0, space_1.default)(3)};
  }
  & > li > a {
    padding: ${(0, space_1.default)(1)} 0;
    font-size: ${p => p.theme.fontSizeLarge};
    margin-bottom: 4px;
  }
  & > li.active > a {
    margin-bottom: 0;
  }
`;
/**
 * Containers for two column 66/33 layout.
 */
exports.Main = (0, styled_1.default)('section') `
  grid-column: ${p => (p.fullWidth ? '1/3' : '1/2')};
  max-width: 100%;
`;
exports.Side = (0, styled_1.default)('aside') `
  grid-column: 2/3;
`;
//# sourceMappingURL=thirds.jsx.map