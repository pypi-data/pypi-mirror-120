...
class UploadPlayblastToKitsu(flow.Action):
    ...
    def _get_message_and_buttons(self):
        ...
        return msg, ['Upload', 'Cancel']
    
    def child_value_changed(self, child_value):
        if child_value is self.target_task_type:
            message, _  = self._get_message_and_buttons()
            self.message.set(message)
    ...
    def run(self, button):
        ...
        _, buttons = self._get_message_and_buttons()

        if 'Upload' not in buttons:
            return self.get_result(close=False)
