...
def confirm_sw_statistic_data(ocsnum, sw_name, ret_text):
    if ret_text == "sw lock":
        newlog = "订单：http://ocs.gz.cvte.cn/tv/Tasks/view/range:my/" + str(ocsnum) + "   软件版本：" + str(sw_name) + "  软件被禁用,请确认是否需要更新软件!\n"
    else:
        if ret_text == None:
            newlog = "订单：http://ocs.gz.cvte.cn/tv/Tasks/view/range:my/"+str(ocsnum)+"   软件版本："+str(sw_name)+"  确认失败!\n"
        elif int(ret_text) > 0:
            newlog = "订单：http://ocs.gz.cvte.cn/tv/Tasks/view/range:my/"+str(ocsnum)+"   软件版本："+str(sw_name)+"  确认成功!\n"
        else:
            newlog = "订单：http://ocs.gz.cvte.cn/tv/Tasks/view/range:my/"+str(ocsnum)+"   软件版本："+str(sw_name)+"  确认失败!\n"
    #统计当天的数据
