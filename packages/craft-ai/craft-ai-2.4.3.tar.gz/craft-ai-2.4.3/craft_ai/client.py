...
from craft_ai.websocket_client import (
    initialize_websocket,
    send_payload_websocket,
    wait_for_server_response_websocket,
)
...
class Client(object):
    ...
    def generate_websocket_headers(self, agent_id, nb_operations):
        return [
            ("Authorization", "Bearer " + self.config.get("token")),
            ("agent-name", agent_id),
            (
                "expected-task-count",
                ceil(nb_operations / self.config["operationsChunksSize"]),
            ),
        ]
    ...
    # Context methods #
    ###################
    def add_agent_operations(self, agent_id, operations, use_websocket=False):
        ...
        if use_websocket:
            headers = self.generate_websocket_headers(agent_id, len(operations))
            websocket = initialize_websocket(self.config["url"], headers)
        is_looping = True
        ...
        try:
            while is_looping:
                ...
                if use_websocket:
                    send_payload_websocket(websocket, json_pl)
                else:
            ...
            if use_websocket:
                added_operations_count = wait_for_server_response_websocket(websocket)
        except Exception as e:
            if use_websocket:
                web_socket_event_loop = asyncio.get_event_loop()
                web_socket_event_loop.run_until_complete(websocket.close(1003))
            raise e
        return {
    ...
    def _decode_response(response):
        ...
        try:
            body = Client._parse_body(response)
            if type(body) == dict:
                message = body["message"]
            elif type(body) == list:
                message = body[0]["message"]
        except (CraftAiInternalError, KeyError, TypeError, IndexError):
            pass
