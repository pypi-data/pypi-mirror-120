...
class Cropper:
    ...
    def center_from_faces(self, image):
        ...
        face_cascade = cv2.CascadeClassifier(os.path.join(os.path.dirname(__file__),'utils',HAARCASCADE_FRONTALFACE))
        faces = face_cascade.detectMultiScale(image, FACE_DETECT_REJECT_LEVELS, 
                                              FACE_DETECT_LEVEL_WEIGHTS)

        # Profile face detection
        if len(faces) == 0:
            face_cascade = cv2.CascadeClassifier(os.path.join(os.path.dirname(__file__),'utils',HAARCASCADE_PROFILEFACE))
            faces = face_cascade.detectMultiScale(image, FACE_DETECT_REJECT_LEVELS, 
                                                  FACE_DETECT_LEVEL_WEIGHTS)
