...
class LoggingState:
    ...
    def __init__(
        ...
        self.configuration = LoggingConfiguration(
            **{
                ...
                "log_file_path": (
                    pathlib.Path(DEFAULT_LOG_FILE) if not disable_file_logging else None
                ),
    ...
    def set_logging_state(self: T) -> None:
        ...
        log.debug(
            "Applied logging configuration, {0}".format(self.configuration.dict())
        )
    ...
    def __set_log_level(self: T, root_logger: logging.Logger) -> None:
        ...
        for this_handler in root_logger.handlers:
            this_handler.setLevel(self.configuration.log_level)
        # Ensure that the logging level propagates to any subsequently created handlers.
