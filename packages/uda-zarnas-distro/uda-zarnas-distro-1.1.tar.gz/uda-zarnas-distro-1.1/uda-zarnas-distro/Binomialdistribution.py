...
class Binomial(Distribution):
    ...
    def plot_bar(self):
        ...
        for i in range(self.n):
            if self.data[i] == 1:
                ...
                tail += 1
        dic = dict()
        dic.update(("heads",head),("tails",tail))
        xaxis = list(dic.keys())
    ...
    def pdf(self, k):
        ...
        prob_den_func = (math.factorial(self.n) / (math.factorial(k) * math.factorial(self.n - k))) * (self.p ** k) * (1 - self.p) ** (self.n - k)
        return prob_den_func
    ...
    def plot_bar_pdf(self):
        ...
        for i in range(self.n+1):
            xlist.append(i)
            ylist.append(self.pdf(i))
        
        plt.bar(xlist, ylist)
        plt.xlabel("k")
        plt.ylabel("pdf value")
        plt.title("bar for pdf")
