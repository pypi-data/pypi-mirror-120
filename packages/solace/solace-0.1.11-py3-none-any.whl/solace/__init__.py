...
class Solace:
    def __init__(self, config = {}):
        ...
        # if self.config.static_url != None:
        if hasattr(self.config, "static_url"):
            path = f"{self.config.static_url}/<path:path>"
    ...
    def __static_files(self, req, res):
        ...
        directory = f"{os.getcwd()}/src/assets"
        return send_from_directory(directory, path, req.environ)
    ...
    def __dispatch(self, req):
        ...
        try:
            ...
            if r is not None and str(type(r)) == "<class 'werkzeug.wrappers.response.Response'>":
                return r
