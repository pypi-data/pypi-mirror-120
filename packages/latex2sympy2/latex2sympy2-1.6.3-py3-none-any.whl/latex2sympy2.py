...
def latex2sympy(sympy: str, variable_values={}):
    ...
    sympy = re.sub(r"\((.*?)\)_{(.*?)}", r"\\frac{(\1)!}{((\1)-(\2))!}", sympy)
...
def latex2latex(tex):
    # !Use Global variances
    return sympy.latex(simplify(latex2sympy(tex).subs(variances).doit().doit())).replace(r'\left[\begin{matrix}', r'\begin{bmatrix}', -1).replace(r'\end{matrix}\right]', r'\end{bmatrix}', -1)
