...
import cv2.ml
import numpy as np
from config.Config import DATABASES_PATH, MODELS_PATH, RESOURCES_PATH
from facenet.Facenet import Facenet
from mvc.utils.Utils import measure, mupemenet_singleton
...
def scope_db(func):
    def decorator(*args, **kwargs):
        global db
        filename = DATABASES_PATH + '/users.json'
        storage = CachingMiddleware(JSONStorage)
        os.makedirs(DATABASES_PATH, exist_ok = True)
...
class UserDB:
    ...
    def build_fr_model(self):
        ...
        filename = MODELS_PATH + "/faces.xml"
        model.save(filename)
