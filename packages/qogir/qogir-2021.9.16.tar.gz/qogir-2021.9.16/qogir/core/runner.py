...
class JobRunner(object):
    ...
    def configure(self):
        ...
        self.command_ = getattr(self, "get_{}_command".format(job_type))()

    def get_default_command(self):
        tpl_path = os.path.join(self.root_dir, "templates", "default.submit.sh.tpl")
        runner_path = os.path.join(self.root_dir, "core", "jobs", "default.py")
    ...
    def get_pyspark_command(self):
        ...
        tpl_path = os.path.join(self.root_dir, "templates", "pyspark.submit.sh.tpl")
        runner_path = os.path.join(self.root_dir, "core", "jobs", "pyspark_.py")
        spark_python_path = os.path.join(self.env, "bin", "python")
        submit_exec_path = os.path.join(self.env, "bin", "spark-submit")
        environments = ""
        if "environments" in self.config:
            environments="\n".join(
                [
                    "export {k}={v}".format(k=k, v=v)
                    for k, v in self.config["environments"].items()
                ]
            )
        _command = open(tpl_path).read()
        _command = _command.format(
    ...
    def _check_venv(self):
        ...
        if not _include_paths:
            ...
            include_paths = [repr(_path) for _path in _include_paths]
        tpl_path = os.path.join(self.root_dir, "templates", "check_venv.sh.tpl")
        _command = open(tpl_path).read()
        _command = _command.format(
            job_path=self.path,
