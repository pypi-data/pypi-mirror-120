...
def build(submitter):
    ...
    knob = nuke.String_Knob("cio_custom_frames", "Custom frames", "1-3")
    submitter.addKnob(knob)
    ...
    knobChanged(submitter, None)


def knobChanged(node, knob):
    """Hide/show string knobs based on checkbox changes."""
    if not knob or (knob.name() in ["cio_use_scout_tasks", "cio_use_custom_frames"]):
        return
