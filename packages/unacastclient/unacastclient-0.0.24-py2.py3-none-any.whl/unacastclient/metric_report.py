...
class MetricReport(object):
    ...
    def print(self):
        ...
        restricted_date = self._metric.your_lens.lens_filters.period_filters[0]
        print(f"""
            metric_id: {self._metric.id}
            description: {self.to_dict()['dimensions'][0]['description']}
            data range: {f"start: '{obs_date.start.year}-{obs_date.start.month}-{obs_date.start.day}'" +
                        f", end: '{obs_date.end.year}-{obs_date.end.month}-{obs_date.end.day}' "}
            ...
            your time period restrictions: {f"start: '{restricted_date.start.year}-{restricted_date.start.month}-{restricted_date.start.day}'" +
                                            f", end: '{restricted_date.end.year}-{restricted_date.end.month}-{restricted_date.end.day}' "}
            """)
