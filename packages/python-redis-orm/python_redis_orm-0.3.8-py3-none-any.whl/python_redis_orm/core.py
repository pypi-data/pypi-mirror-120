    ...
    def _get_instance_keys_to_update(self, instances, model_name):
        if self.use_keys:
            if instances is None:
                return list(self.redis_instance.keys(f'{self.prefix}:{model_name}:*'))
            else:
                ids_to_update = get_ids_from_untyped_data(instances)
                for instance_id in ids_to_update:
                    return list(self.redis_instance.keys(f'{self.prefix}:{model_name}:{instance_id}'))
        else:
            if instances is None:
                return list(self.redis_instance.scan_iter(f'{self.prefix}:{model_name}:*'))
            else:
                ids_to_update = get_ids_from_untyped_data(instances)
                for instance_id in ids_to_update:
                    return list(self.redis_instance.scan_iter(f'{self.prefix}:{model_name}:{instance_id}'))
