...
class ModelDeploymentProperties(data_science_models.ModelDeployment):
    ...
    def to_create_deployment(self):
        ...
        model_config = (
            create_details.model_deployment_configuration_details.model_configuration_details
        )
        if (
            not model_config.instance_configuration
            or not model_config.instance_configuration.instance_shape_name
        ):
            raise ValueError("instance_shape is required when creating a deployment.")
        if (
            not model_config.scaling_policy
            or not model_config.scaling_policy.instance_count
        ):
            ...
            if hasattr(from_obj, attr):
                ...
                if from_attr_type == to_attr_type:
                    ...
                    setattr(
                        to_obj,
                        attr,
                        ModelDeploymentProperties.copy_attributes(value, model_class()),
                    )
        return to_obj
