...
class IpythonProgressBar(ProgressBar):
    ...
    def update(self, description=None):
        ...
        """
        if self.verbose:
            print(str.format('{0:.3f}', time.time() - self.start_time), self.progress_label.value)
            self.start_time = time.time()
...
class TqdmProgressBar(ProgressBar):
    ...
    def __init__(self, max_progress=100, description="Running", verbose=False):
        self.max_progress = max_progress
        self.progress_bar = tqdm_notebook(range(max_progress), desc='loop1', mininterval=0, leave=False)
    ...
    def update(self, description=None):
        ...
        if self.verbose and description is not None:
            print("%s:%ss" % (self.description, str.format('{0:.3f}', time.time() - self.start_time)))
