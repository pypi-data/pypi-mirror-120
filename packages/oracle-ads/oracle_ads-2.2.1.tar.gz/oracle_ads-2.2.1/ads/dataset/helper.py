...
from pandas.core.dtypes.common import is_numeric_dtype, is_bool_dtype, is_categorical_dtype, is_datetime64_any_dtype, \
    is_float_dtype
...
from ads.type_discovery.typed_feature import ContinuousTypedFeature, DateTimeTypedFeature, CategoricalTypedFeature, \
    UnknownTypedFeature, OrdinalTypedFeature, DocumentTypedFeature
...
def get_dtype(feature_type, dtype):
    if isinstance(feature_type, ContinuousTypedFeature) or isinstance(feature_type, OrdinalTypedFeature):
...
def convert_to_html(plot):
    ...
    return '<img width=95%" src="data:image/png;base64, {}"><hr><br>'.format(encoded.decode('utf-8'))
...
class ElaboratedPath:
    ...
    * A sqlalchemy connection url
    ...
    def __init__(self, source: Union[str, List[str]], format: str = None, name: str = None, **kwargs):
        """
        :param source:
        :param format:
        :param kwargs:
        ...
        if self.num_paths == 0:
            raise FileNotFoundError(f"Error: We could not find any files associated with the source: " \
    ...
    def _determine_format(self):
        ...
        if self._format in [None, "infer"]:
            ...
            if len(format_keys) == 0:
                raise ValueError(f"Could not determine the format key for source: {self._original_source}")
    ...
    def _determine_protocol_type(self):
        ...
        if self._kwargs.get("fs") is not None:
            ...
            try:
                self._fs = fsspec.filesystem(self._protocol, **self._kwargs.get("storage_options", dict()))
                self._type = "glob"
            except ValueError:
                try:
                    self.engine = utils.get_sqlalchemy_engine(self._original_source, **self._kwargs)
                    self._type = "db"
                except:
                    if self._protocol in _known_db_protocols:
                        self._type = "db"
                    else:
                        raise ValueError(f"Error in trying to understand the protocol for source: "
    ...
    def _remove_compressions(filename: str):
        for compression in _compression_to_extension.keys():
            if filename.strip().endswith(_compression_to_extension[compression]):
                return ElaboratedPath._remove_compressions(os.path.splitext(filename.rstrip("/*"))[0])
...
def _get_dtype_from_error(e):
    ...
    if 'mismatched dtypes' in error_string.lower():
        ...
        found_dtype_dict_str_list = re.findall(r'dtype=({[^{}]+})', error_string, re.MULTILINE)
        ...
        if found_dtype_dict:
            logger.warning("Dask type-inference/coercion failed. Retrying with dtype=%s.".format(found_dtype_dict),
                           exc_info=True)
...
def _find_dtypes_from_table(error_string):
    ...
    pattern = re.compile("\\s*\\|\\s*Column\\s*\\|\\s*Found\\s*\\|\\s*Expected\\s*\\|\\s*")
...
def rename_duplicate_cols(original_cols):
    ...
    assert (len(new_cols) == len(original_cols)), "There has been an error in re-naming duplicate columns"
    return new_cols
...
def is_text_data(df, target=None):
    if len(df.columns.values) == 2:
        feature_name = list(set(df.columns.values) ^ set([target]))[0] if target else list(set(df.columns.values))[0]
    elif len(df.columns.values == 1):
        feature_name = df.columns.values[0]
    else:
        return False
    return isinstance(TypeDiscoveryDriver().discover(feature_name, df[feature_name]),
                      DocumentTypedFeature)


def generate_sample(ddf: dd.DataFrame, n: int, confidence_level: int = DatasetDefaults.sampling_confidence_level,
                    confidence_interval: float = DatasetDefaults.sampling_confidence_interval, **kwargs):
    ...
    if "sample_max_rows" in kwargs:
        ...
        if requested_sample_size < 0:
            sample_size = calculate_sample_size(n, min_size_to_sample, confidence_level, confidence_interval)
        else:
            if min_size_to_sample < requested_sample_size < n:
                utils.print_user_message(
                    "Sampling (from {} rows), to user specified {} rows for efficient graphing".format(n, kwargs[
                        "sample_max_rows"]),
                    display_type='info', title=None)
                sample_size = requested_sample_size
            elif requested_sample_size >= n:
                utils.print_user_message(
                    "Using entire dataset for graphing ({} rows)".format(n, kwargs["sample_max_rows"]),
                    display_type='info',
                    title=None)
                sample_size = n
            else:
                sample_size = min_size_to_sample
                utils.print_user_message("Sampling (from {} rows) to {} rows for graphing".format(n, sample_size),
                                         display_type='info', title=None)
...
def calculate_sample_size(population_size, min_size_to_sample, confidence_level=95, confidence_interval=1.0):
    ...
    utils.print_user_message(
        "Auto sampling from {} to approx {} rows (confidence_level: {}, confidence_interval: {})".format(
            population_size, sample_size, confidence_level, confidence_interval), display_type='info', title=None)
...
def visualize_transformation(transformer_pipeline, text=None):
    ...
    dot.attr('node', shape='tab', style='filled', fontname='courier', fontsize='12', fontcolor='white',
             resolution='144')
    if text:
        dot.node('partitions', text, margin="0.25", fillcolor='dimgray')

    dot.attr('node', shape='component', style='filled', fontname='courier', fontsize='10', fontcolor='black',
             resolution='144')
    for step in transformer_pipeline.steps:
        name, clazz, clazzname, is_ads = step[0], step[1], step[1].__class__.__name__, 'ads' in str(step[1].__class__)
        ads_node = str(step[1].__class__.__name__) in ['AutoMLPreprocessingTransformer', 'DataFrameTransformer',
                                                       'RecommendationTransformer', 'AutoMLFeatureSelection',
                                                       'FeatureEngineeringTransformer']
        if ads_node:
            text2html = "< {} >".format(html.escape(step[1].__repr__()).replace("\n", "<br/>"))
    ...
    if sampler != 'default':
        if inspect.getattr_static(sampler, 'fit_resample', None) is None:
            ...
            if len(sig.parameters) < 2 or num_no_default_params > 2:
                raise RuntimeError('The signature for `sampler.fit_resample` has to be `fit_resample(X, y)`.')
    ...
    if len(columns_with_nans) > 0:
        ...
        for column in columns_with_nans:
            if column in feature_types and "mode" in feature_types[column]["stats"]:
                ...
                utils.print_user_message("Sampling from Column that is missing values. This may cause an error",
                                         display_type='error')
...
def _get_imblearn_sampler(X, y):
    ...
    categorical_feature_indices = [
        ...
        X.select_dtypes(include=['category', 'object', 'datetime64']).columns.values
    ]
    # ----------
    # no longer use SMOTENC for data with categorical features, as imblearn's implementation
    # could result in significant memory usage due to one-hot-encoding and numpy array slicing.
    # ----------
    # # SMOTENC fails when all features are categorical
    # if len(X.select_dtypes(include=['category', 'object']).columns) == len(X.columns):
    #     utils.print_user_message(
    #         "All features are categorical. Up-sampling is only possible for datasets \
    #         containing both continuous and categorical features.",
    #         display_type='error', title='Error'
    #     )
    #     raise ValueError("""
    #         Dataset contained only categorical features.
    #         Make sure you have both continuous and categorical features in order for SMOTENC to work.
    #     """)

    if len(categorical_feature_indices) > 0:
        logger.info("""
            Using RandomOverSampler for over sampling. 
    ...
    if k_neighbors == 0:
        ...
        if 5 > k_neighbors > 0:
            utils.print_user_message(
                "<code>k_neighbors()</code> of SMOTE has changed to {}"
                " as the target has at least one class which appeared "
                "only {} times in the data. The smaller <code>k_neighbors</code> is,"
                "the high influence that the noise in the data will have on the "
                "result.".format(k_neighbors, min_sample_size),
                display_type='tip', title='Warning'
            )
        logger.info('Using SMOTE for over sampling. Classes will be equalized.')
        utils.print_user_message('Using SMOTE for over sampling.', title='Info')
...
def down_sample(df, target):
    ...
    """
    dfs = []
    if isinstance(df, dd.core.DataFrame) and all([division is None for division in df.divisions]):
    ...
    for key, value in target_value_counts.iteritems():
        if key != min_key[0]:
            dfs.append(df[df[target] == key].sample(frac=1 - ((value - min_key[1]) / value)))
    dfs.append(df[df[target] == min_key[0]])
    return pd.concat(dfs) if isinstance(df, pd.DataFrame) else dd.concat(dfs, interleave_partitions=True)


def _sample(sampler, X, y):
    if isinstance(y, pd.Series) and (isinstance(y[0], bool) or isinstance(y[0], np.bool_)):
    ...
    if isinstance(X_resampled, pd.DataFrame):
        X_resampled = dd.from_pandas(X_resampled, npartitions=_num_partitions_for_dataframe(X_resampled))
    else:
        X_resampled = dd.from_array(X_resampled, columns=X.columns.values)
    if isinstance(y_resampled, pd.Series):
        y_resampled = dd.from_pandas(y_resampled, npartitions=_num_partitions_for_dataframe(y_resampled))
    else:
...
def get_fill_val(feature_types, column, action, constant='constant'):
    ...
    try:
        fill_val = feature_types[column].meta_data["stats"][fill_type] if action_ != 'constant' else constant
