...
class DataFrameTransformer(TransformerMixin):
    ...
    def __init__(self, func_name, target_name, target_sample_val, args=None, kw_args=None):
    ...
    def __repr__(self):
        return "\n{}({})\n\n{}".format(
            self.function_name_,
            self.target_name,
            "\n".join([expand_lambda_function(f) if f.__name__ == "<lambda>"
                       else "{} {}".format(f.__name__, f.__code__.co_varnames)
                       for f in self.function_args_ if callable(f)]))
    ...
    def _transform(self, df):
        ...
        elif isinstance(df, pd.DataFrame) and len(self.function_args_) > 0 and callable(self.function_args_[0]):
