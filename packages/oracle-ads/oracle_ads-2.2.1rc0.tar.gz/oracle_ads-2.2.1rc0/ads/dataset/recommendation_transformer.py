...
from ads.type_discovery.typed_feature import ContinuousTypedFeature, DiscreteTypedFeature, OrdinalTypedFeature,\
    CategoricalTypedFeature


class RecommendationTransformer(TransformerMixin):
    def __init__(self, feature_metadata=None, correlation=None, target=None, is_balanced=False, target_type=None,
                 feature_ranking=None, len=0, fix_imbalance=True, auto_transform=True, correlation_threshold=0.7):
    ...
    @staticmethod
    def _build_recommendation(recommendations, recommendation_type, column_names, message, actions,
            ...
            if recommendation_type in ['constant_column']:
                ...
                for column in column_names:
                    ...
                    recommendations[recommendation_type][column]["Selected Action"] = \
                        recommended_action if recommended_action is not None else actions[0]
    ...
    def _get_recommendations(self, df):
        ...
        self._build_recommendation(recommendations, "constant_column",
                                   constant_columns,
                                   "Constant Column", ["Drop"])

        # primary key
        for column in df.columns.values[df.apply(lambda x: x.nunique() / len(x) > 0.99)]:
            ...
            if ("constant_column" not in recommendations or column not in recommendations["constant_column"]) \
                    and column != self.target_ and df[column].dtype.name.startswith('int'):
                self._build_recommendation(recommendations, "primary_key", [column],
                                           "Contains mostly unique values({0:.2%})"
                                           .format(df[column].nunique() / len(df[column])),
                                           ["Drop", "Do nothing"], "Drop")
        ...
        for column in df.columns.values[df.isnull().any()]:
            ...
            if ("constant_column" not in recommendations or column not in recommendations["constant_column"]) and \
                    ("primary_key" not in recommendations or column not in recommendations["primary_key"]):
        ...
        if self.correlation_ is not None:
            ...
            for corr in self.correlation_:
                ...
                corr_features.extend([(corr.index[x], corr.columns[y], corr.iat[x, y])
            ...
            if len(corr_features) > 0:
                ...
                for corr_feature in corr_features:
                    if self.target_ not in corr_feature and corr_feature[0] in df.columns.values \
                            and corr_feature[1] in df.columns.values and corr_feature[0] != corr_feature[1]:
                        feature1_rank_and_score = \
                            self.feature_ranking_[self.feature_ranking_["features"] == corr_feature[0]]["scores"]
                        rank1, score1 = (feature1_rank_and_score.index[0], feature1_rank_and_score.values[0])
                        feature2_rank_and_score = \
                            self.feature_ranking_[self.feature_ranking_["features"] == corr_feature[1]]["scores"]
                        rank2, score2 = (feature2_rank_and_score.index[0], feature2_rank_and_score.values[0])
                        ...
                        self._build_recommendation(recommendations, "strong_correlation", [corr_feature[0]],
                                                   "Strongly correlated with " + corr_feature[1]
        ...
        if isinstance(self.target_type_, DiscreteTypedFeature):
            ...
            if len(unique_vals) == 2 and True not in unique_vals:
                ...
                if not self.auto_transform:
                    ...
                    pos_label_index = np.where(np.isin(pos_vals, unique_vals) == True)[0]
                    selected_action = pos_vals[pos_label_index[0]] if len(pos_label_index) > 0 else unique_vals[0]

                self._build_recommendation(recommendations, "positive_class", [self.target_],
                                           "Set Positive Class",
                                           unique_vals,
                                           selected_action)
            ...
            if not self.is_balanced and self.fix_imbalance:
                ...
                minority_class_len = min(target_value_counts.iteritems(), key=lambda k: k[1])[1]
                majority_class_len = max(target_value_counts.iteritems(), key=lambda k: k[1])[1]
                minor_majority_ratio = minority_class_len / majority_class_len

                # Suggest down sampling if minor_majority_ratio is 1:1000
                suggested_sampling = "Do nothing" if len(df) <= utils.MAX_LEN_FOR_UP_SAMPLING else "Down-sample" \
                    if minor_majority_ratio >= utils.MIN_RATIO_FOR_DOWN_SAMPLING else "Do nothing"
                self._build_recommendation(recommendations, "fix_imbalance", [self.target_],
    ...
    def transform(self, X, progress=DummyProgressBar(), fit_transform=False, update_transformer_log=False):
        df = self._transform(X, progress=progress, fit_transform=fit_transform,
    ...
    def _transform(self, X, progress=DummyProgressBar(), fit_transform=False, update_transformer_log=False):
        if hasattr(self, "reco_dict_") and len(self.reco_dict_) > 0:
            self.drop_columns = []
            # self.combine_columns = []
            #self.fill_nan_dict_ = {}
            columns_to_drop = []
            for recommendation_type_index in range(0, len(Recommendation.recommendation_types)):
                recommendation_type = Recommendation.recommendation_types[recommendation_type_index]
                if recommendation_type in self.reco_dict_:
                    if recommendation_type_index == 0:
                        ...
                        if update_transformer_log:
                            self.transformer_log(utils.wrap_lines(columns_to_drop, heading="Drop constant columns:"))
                    elif recommendation_type == "positive_class":
                        value = self.reco_dict_[recommendation_type][self.target_]["Selected Action"]
                        if value != "Do nothing":
                            if self.target_ in X.columns:
                                # X = X.set_positive_class(value, missing_value=False)
                                X[self.target_] = X[self.target_].map(lambda x: x == value)
                                if update_transformer_log:
                                    self.transformer_log("Set %s as positive class" % value)
                    elif recommendation_type == "fix_imbalance" and self.fix_imbalance:
                        self.balancing_strategy = self.reco_dict_[recommendation_type][self.target_]["Selected Action"]
                        if update_transformer_log:
                            self.transformer_log("Fix imbalance using technique: %s" % self.balancing_strategy)
                    else:
                        # Get the new column name if it has been combined with another
                        for column in self.reco_dict_[recommendation_type]:
                            selected_action = self.reco_dict_[recommendation_type][column]["Selected Action"]
                            ...
                            if selected_action.startswith('Drop'):
                                ...
                                if column not in columns_to_drop:
                                    ...
                                    if update_transformer_log:
                                        self.transformer_log('Drop: "{}"'.format(column))
                            ...
                            #             self.transformer_log('Combine: "{}" with "{}"'.format(column1, column2))
                                ...
                                fill_val = self.fill_nan_dict_[column] \
                                    if selected_action == "Fill missing values with constant" \
                                    else get_fill_val(self.feature_metadata_, column, selected_action, constant='constant')
                                    ...
                                    if column == self.target_:
                                        ...
                                        if update_transformer_log:
                                            self.transformer_log(
                                                '{} in {}: "{}"'.format(selected_action, column, fill_val))
        ...
        if len(self.fill_nan_dict_) > 0:
            logger.info("Filling NaN values in " + str(list(self.fill_nan_dict_.keys())))
        for col, fill_val in self.fill_nan_dict_.items():
            if col in X:
                if X[col].dtype.name == "category" and fill_val not in X[col].cat.categories.tolist():
        ...
        if fit_transform and self.fix_imbalance:
            if self.balancing_strategy and self.balancing_strategy != "Do nothing":
                ...
                X = up_sample(X, self.target_, feature_types=self.feature_metadata_) if self.balancing_strategy == "Up-sample" else \
                    down_sample(X, self.target_)
            else:
                progress.update()
        else:
            progress.update()
    ...
    def _get_na_action(self, recommendations, column, null_counts, null_ratio):
        if isinstance(self.feature_metadata_[column], ContinuousTypedFeature):
            if null_ratio == 1:
                possible_actions = ["Drop", "Fill missing values with constant", "Do nothing"]
                selected_action = "Drop"
            else:
                possible_actions = ["Drop", "Fill missing values with mean", "Fill missing values with median",
            ...
            if null_ratio == 1:
                ...
                possible_actions = ["Drop", "Fill missing values with frequent",
                                    "Fill missing values with constant", "Do nothing"]
                if null_ratio <= 0.4:
        ...
        self._build_recommendation(recommendations, "imputation", [column],
                                   msg,
                                   possible_actions,
                                   selected_action)
