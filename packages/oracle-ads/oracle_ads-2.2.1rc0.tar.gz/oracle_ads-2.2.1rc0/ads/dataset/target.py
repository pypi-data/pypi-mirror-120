...
class TargetVariable:
    ...
    def __init__(self, sampled_ds, target, target_type):
        ...
        if isinstance(self.type, CategoricalTypedFeature):
            ...
            self.skewness = np.abs(skew(preprocessing.LabelEncoder()
                                        .fit_transform(self.sampled_ds.sampled_df[target])))
        elif isinstance(self.type, ContinuousTypedFeature):
            try:
                self.skewness = np.abs(self.sampled_ds.sampled_df[target].skew())
            except TypeError as e:
                self.skewness = np.abs(self.sampled_ds.sampled_df[target].astype('float').skew())
        else:
            ...
            self.skewness = None
        self.numeric_columns = self.sampled_ds.sampled_df._get_numeric_data().columns.values
    ...
    def show_in_notebook(self, feature_names=None): # pragma: no cover
        ...
        if feature_names is not None:
            for feature_name in feature_names:
                self.sampled_ds.plot(feature_name, self.name, verbose=verbose).show_in_notebook()
                verbose = False
        else:
    ...
    def is_balanced(self):
        ...
        is_balanced: bool
        """
        if isinstance(self.type, CategoricalTypedFeature) or isinstance(self.type, ContinuousTypedFeature):
