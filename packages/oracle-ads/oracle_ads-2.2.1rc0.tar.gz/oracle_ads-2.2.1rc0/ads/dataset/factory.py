...
from ads.dataset.classification_dataset import BinaryClassificationDataset, MultiClassClassificationDataset, \
    BinaryTextClassificationDataset, MultiClassTextClassificationDataset
from ads.dataset.dataset import ADSDataset
from ads.dataset.forecasting_dataset import ForecastingDataset
from ads.dataset.helper import get_feature_type, is_text_data, generate_sample, map_types, DatasetDefaults, \
    ElaboratedPath, DatasetLoadException, _num_partitions_for_dataframe
...
from ads.type_discovery.typed_feature import ContinuousTypedFeature, DateTimeTypedFeature, CategoricalTypedFeature, \
    OrdinalTypedFeature, GISTypedFeature, DocumentTypedFeature
...
class DatasetFactory:
    ...
    @staticmethod
    def open(source, target=None, format="infer", reader_fn: Callable = None, name: str = None, description="",
             npartitions: int = None, type_discovery=True, html_table_index=None, column_names="infer",
             sample_max_rows=10000, positive_class=None, transformer_pipeline=None, types={},
             **kwargs):
        """
        ...
        >>> ds = DatasetFactory.open("oci://path/to/data.tsv", column_names=["col1", "col2", "col3"], header=0)

        >>> ds = DatasetFactory.open("oci://path/to/data.csv", storage_options={"config": "~/.oci/config",
        ...
        if isinstance(source, str) or isinstance(source, list):
            ...
            reader_fn = get_format_reader(path=path, **kwargs) if reader_fn is None else reader_fn
            ...
            raise TypeError(f"The Source type: {type(source)} is not supported for DatasetFactory.")
        shape = df.shape
        ddf = dd.from_pandas(df, npartitions=_num_partitions_for_dataframe(df) if npartitions is None else npartitions)
        return DatasetFactory._build_dataset(ddf=ddf, shape=shape, target=target,
                                             sample_max_rows=sample_max_rows, type_discovery=type_discovery,
                                             types=types, positive_class=positive_class, name=name,
                                             transformer_pipeline=transformer_pipeline, description=description,
                                             progress=progress, **utils.inject_and_copy_kwargs(kwargs, **{
                "html_table_index": html_table_index,
                "column_names": column_names
            }))

    @staticmethod
    def open_to_pandas(source: str, format: str = None, reader_fn: Callable = None, **kwargs) -> pd.DataFrame:
        path = ElaboratedPath(source, format=format, **kwargs)
        reader_fn = get_format_reader(path=path, **kwargs) if reader_fn is None else reader_fn
        df = load_dataset(path=path, reader_fn=reader_fn, **kwargs)
        return df 
    ...
    def list_snapshots(snapshot_dir=None, name="", storage_options=None, **kwargs):
        ...
        if storage_options is None:
            if default_storage_options is not None:
                ...
                storage_options = dict()
        assert isinstance(storage_options, dict), "The storage options parameter must be a dictionary. You can set " \
        ...
        obj_list = [(k, v.get('timeCreated', mindate).strftime("%Y-%m-%d %H:%M:%S"))
                    for k, v in fs.glob(os.path.join(snapshot_dir, name + "**"), detail=True, **kwargs).items() if
                    v['type'] == 'file']
        ...
        if utils.is_notebook():
            display(HTML(list_df.style.set_table_attributes('class=table').hide_index().render()))
        return list_df
    ...
    def download(remote_path, local_path, storage=None, overwrite=False):
        ...
        remote_files = fsspec.open_files(remote_path, mode='rb', name_function=lambda i: "", **storage)
        if len(remote_files) < 1:
            raise FileNotFoundError(remote_path)
        display_error, error_msg = DatasetFactory._download_files(remote_files=remote_files,
                                                                  local_path=local_path,
                                                                  overwrite=overwrite)
        if display_error:
            utils.print_user_message("WARNING: " + error_msg, display_type='tip')
        else:
            utils.print_user_message(f"Successfully downloaded from remote dir/file: {remote_path} to local "
                                     f"dir/file: {local_path}", display_type='tip')
        ...
        for remote_file in remote_files:
            ...
            try:
                with remote_file as f1:
                    local_filepath = os.path.join(local_path, suffix) if suffix else local_path
                    if os.path.exists(local_filepath) and not overwrite:
                        raise FileExistsError(f"Trying to overwrite files in {local_filepath}. If you'd like to "
            ...
            for subdir, dirs, files in os.walk(local_file_or_dir):
                for file in files:
                    if os.path.abspath(subdir) == os.path.abspath(local_file_or_dir):
                        ...
                        path = os.path.join(os.path.abspath(subdir).split("/", 2)[2], file)
                    DatasetFactory._upload_file(os.path.join(subdir, file), os.path.join(remote_file_or_dir, path),
                                                storage_options=storage_options)
        else:
            DatasetFactory._upload_file(local_file_or_dir, remote_file_or_dir, storage_options=storage_options)
    ...
    def set_default_storage(snapshots_dir=None, storage_options=None):
        ...
        if storage_options is not None:
            assert isinstance(storage_options, dict), f"The storage options parameter must be a dictionary. Instead " \
    ...
    def _upload_file(cls, local_file, remote_file, storage_options=None):
        ...
        remote_file_handler = fsspec.open_files(remote_file + "*", mode='wb',
                                                name_function=lambda i: "", **kwargs)[0]
    ...
    @classmethod
    def _build_dataset(cls, ddf: dd.DataFrame, shape: Tuple[int, int], target: str = None, progress=None, **kwargs):
        n = shape[0]
        if progress:
        ...
        if target is None:
            ...
            utils.print_user_message("Use <code>set_target()</code> to type the dataset for a particular learning task")
            return result
        ...
        target_type = DatasetFactory.infer_target_type(target, sampled_df[target], discover_target_type)

        result = DatasetFactory._get_dataset(ddf=ddf, sampled_df=sampled_df, target=target, target_type=target_type,
                                             shape=shape, **kwargs)
        if progress:
            progress.update("Done")
        utils.print_user_message(
            'Use <code>show_in_notebook()</code> to visualize the dataset.\nUse <code>suggest_recommendations('
            ')</code> to view and apply recommendations for dataset optimization.')
        return result
    ...
    def infer_target_type(cls, target, target_series, discover_target_type=True):
        ...
        if discover_target_type:
            target_type = TypeDiscoveryDriver().discover(target, target_series, is_target=True)
        else:
            target_type = get_feature_type(target, target_series)
        return target_type

    @classmethod
    def _get_dataset(cls, ddf: dd.DataFrame, sampled_df: pd.DataFrame, target: str, target_type: TypedFeature,
                     shape: Tuple[int, int], positive_class=None, **init_kwargs):
        if len(ddf[target].dropna()) == 0:
            utils.print_user_message("We do not recommend using an empty column as the chosen target")
        ...
        if is_same_class(target_type, ContinuousTypedFeature):
            return RegressionDataset(ddf=ddf, sampled_df=sampled_df, target=target, target_type=target_type,
                                     shape=shape, **init_kwargs)
        elif is_same_class(target_type, DateTimeTypedFeature) or ddf.index.dtype.name.startswith('datetime'):
            return ForecastingDataset(ddf=ddf, sampled_df=sampled_df, target=target, target_type=target_type,
                                      shape=shape, **init_kwargs)

        # Adding ordinal typed feature, but ultimately we should rethink how we want to model this type
        elif is_same_class(target_type, CategoricalTypedFeature) or is_same_class(target_type, OrdinalTypedFeature):
            if target_type.meta_data['internal']['unique'] == 2:
                if is_text_data(sampled_df, target):
                    return BinaryTextClassificationDataset(ddf=ddf, sampled_df=sampled_df, target=target, shape=shape,
                                                           target_type=target_type, positive_class=positive_class,
                                                           **init_kwargs)

                return BinaryClassificationDataset(ddf=ddf, sampled_df=sampled_df, target=target, shape=shape,
                                                   target_type=target_type, positive_class=positive_class,
                                                   **init_kwargs)
            else:
                if is_text_data(sampled_df, target):
                    return MultiClassTextClassificationDataset(ddf=ddf, sampled_df=sampled_df, target=target,
                                                               target_type=target_type, shape=shape, **init_kwargs)
                return MultiClassClassificationDataset(ddf=ddf, sampled_df=sampled_df, target=target,
                                                       target_type=target_type, shape=shape, **init_kwargs)
        elif is_same_class(target, DocumentTypedFeature) or 'text' in target_type['type'] or 'text' in target:
            utils.print_user_message("Consider using another column as the target")
            raise ValueError(f"We do not recommend using {target} as the chosen target")
        elif is_same_class(target_type, GISTypedFeature) or 'coord' in target_type['type'] or 'coord' in target:
            utils.print_user_message("Consider using another column as the target")
            raise ValueError(f"We do not recommend using {target} as the chosen target")
        ...
        #   binary target, but only data on one instance
            ...
            return BinaryClassificationDataset(ddf=ddf, sampled_df=sampled_df, target=target, shape=shape,
                                               target_type=target_type, positive_class=positive_class, **init_kwargs)
        raise ValueError(f"Unable to identify problem type. Please specify the data type of {target} using 'types'. "
...
class CustomFormatReaders:
    ...
    def read_tsv(path: str, **kwargs) -> pd.DataFrame:
        return pd.read_csv(path, **utils.inject_and_copy_kwargs(kwargs, **{'sep': '\t'}))
    ...
    def read_json(path: str, **kwargs) -> pd.DataFrame:
        try:
            ...
            return pd.read_json(path, **utils.inject_and_copy_kwargs(kwargs, **{'lines': True}))
    ...
    def read_sql(cls, path: str, table: str = None, **kwargs) -> pd.DataFrame:
        ...
        if table is None:
            raise ValueError("In order to read from a database you need to specify the table using the `table` "
                             "argument.")
        # check if it's oracle dialect
        if str(path).lower().startswith('oracle'):
            kwargs = utils.inject_and_copy_kwargs(kwargs, **{"arraysize": cls.DEFAULT_SQL_ARRAYSIZE,
        ...
        with engine.connect() as connection:
            ...
            if table_name.lower().startswith("select"):
                ...
                utils.print_user_message(
                    ...
                    <h6>For example, </h6>
                        <code>ds = DatasetFactory.open(uri, format="sql", table="SELECT prod_id, cust_id, time_id FROM SH.sales")</code>
                    ''',
                    display_type='info')
                sql_query = f'select * from {table_name}'

            chunks = pd.read_sql_query(sql_query, con=connection,
                                       **_validate_kwargs(pd.read_sql_query, utils.inject_and_copy_kwargs(kwargs,
                                                                                                          **{
                                                                                                              "chunksize": cls.DEFAULT_SQL_CHUNKSIZE})))
            df = pd.DataFrame()
            ...
            if df.shape[0] == 0:
                utils.print_user_message(
                    "The SQL expression returned no rows, no <code>ADSdataset</code> will be created",
                    display_type='info')
    ...
    def read_log(path, **kwargs):
        ...
        df = pd.read_csv(
            ...
            na_values='-',
            header=None,
            names=['host', 'identity', 'user', 'time', 'request',
                   'http_code', 'response_bytes', 'referer', 'user_agent', 'unknown'],
            converters={'time': parse_apache_log_datetime,
        ...
        df["time"] = df["time"].map_partitions(pd.to_datetime, utc=True, meta='datetime64[ns]')
        return df
...
def get_format_reader(path: ElaboratedPath, **kwargs) -> Callable:
    ...
    try:
        ...
        raise ValueError(f'We were unable to load the specified dataset. We have interpreted the format '
...
def load_dataset(path: ElaboratedPath, reader_fn: Callable, **kwargs) -> pd.DataFrame:
    ...
    for filename in path.paths:
        try:
            dfs.append(reader_fn(filename, **_validate_kwargs(reader_fn, kwargs)))
        except (ValueError, DatasetLoadException) as e:
            raise e
    if len(dfs) == 0:
        raise ValueError(f'We were unable to load the specified dataset. Read more here: '
    ...
    try:
        ...
        assert all([isinstance(part, pd.DataFrame) for part in dfs]), "The Reader_fn should return a pandas dataset"
        raise e
    if df is None:
        raise ValueError(f'We were unable to load the specified dataset. Read more here: '
                         f'https://docs.cloud.oracle.com/en-us/iaas/tools/ads'
