...
from ads.type_discovery.typed_feature import ContinuousTypedFeature, DateTimeTypedFeature, \
    ConstantTypedFeature, DiscreteTypedFeature, CreditCardTypedFeature, ZipcodeTypedFeature, OrdinalTypedFeature, \
    CategoricalTypedFeature, GISTypedFeature
...
class Plotting:
    ...
    def __repr__(self):
        ...
        if len(choices) > 1:
            utils.print_user_message("Recommended plot type: <code>{0}</code>, Available plot types: <code>{1}</code>\n" \
                  "Add plot_type argument to plot method to choose a different plot type from the available " \
                  "options. " \
                  "For example, <code>ds.plot(“x”, y=“y”, plot_type='{0}')</code>" \
                  .format(choices[0][1].__name__, ', '.join([x[1].__name__ for x in choices])))
    ...
    def select_best_plot(self):
        ...
        if len(choices) > 1:
            logger.info(
                'select_best_plot (%s, %s) called, possible plot types are %s' % (
                    self.x_type.meta_data["type"], self.y_type.meta_data["type"] if self.y_type is not None else ""
        ...
        if self.plot_type != "infer":
            ...
            raise ValueError("plot_type: '%s' invalid, use one of: %s" % (
    ...
    def _categorical_vs_continuous_violin_plot(x, y, data):
        ...
        if len(vc.keys()) > 10:
            utils.print_user_message("Too many distinct values for {} ({:,}) to plot, only displaying top 10 values sorted by cardinality.".format(x, len(vc.keys())), display_type='info', title=None)
            idxes = vc[:10].index
        else:
            idxes = vc.index
    ...
    def _ordinal_vs_continuous_violin_plot(x, y, data):
        ...
        if len(freq) > 10:
            utils.print_user_message("Too many distinct values for {} ({:,}) to plot, only displaying smallest 10 values sorted by x values.".format(x, len(freq)), display_type='info', title=None)
    ...
    def _categorical_vs_continuous_horizontal_violin_plot(x, y, data):
        ...
        if len(vc.keys()) > 10:
            utils.print_user_message(
                "Too many distinct values for {} ({:,}) to plot, only displaying top 10 values sorted by cardinality.".format(x, len(vc.keys())),
                display_type='info', title=None)
            idxes = vc[:10].index
        else:
            idxes = vc.index
        sns.violinplot(x=y, y=x, data=data[data[x].isin(idxes)], order=idxes, orient='h')
    ...
    def _count_plot(x, hue, data, yscale=None):
        if not yscale:
            utils.print_user_message("Set yscale to one of <code>'linear'</code>, <code>'log'</code>, <code>'symlog'</code> to apply scale to y axis")
        ...
        if len(cat_cardi) > 5:
            ...
            data[hue] = np.where(data[hue].isin(top_categoricals), data[hue], 'all_other_categories')
    ...
    def _single_column_count_plot(x, data, yscale=None):
        if not yscale:
            utils.print_user_message(
                "Set yscale to one of <code>'linear'</code>, <code>'log'</code>, <code>'symlog'</code> to apply scale to y axis")
    ...
    def _folium_map(x, data):
        ...
        lat_min, lat_max, long_min, long_max = min(df.Y), max(df.Y), min(df.X), max(df.X)
    ...
    def _multiple_pdf(x, y, data):
        ...
        hues = [colors[x] for x in colors.keys() if isinstance(colors[x], str) and colors[x].startswith('#')]
    ...
    def _matplot(self, plot_method, figsize=(4, 3), **kwargs):
        ...
        #
        text = '{}, "{}" ({})'.format(plot_method.__name__.upper(), self.x, self.feature_types[self.x].type)
        if self.y:
            text = '{} vs "{}" ({})'.format(text, self.y, self.feature_types[self.y].type)
        ...
        if (plot_method is plt.scatter or plot_method is Plotting._gaussian_heatmap) \
        ...
        if plot_method is plt.hist:
            ...
            plt.xticks(np.arange(min(self.df[kwargs['x']].values), max(self.df[kwargs['x']].values)+1, (max(self.df[kwargs['x']].values)-min(self.df[kwargs['x']].values))/30))
    ...
    def _get_plot_method(self):
        ...
        combos[Plotting._build_plot_key(CategoricalTypedFeature, ContinuousTypedFeature)] = [
            (self._matplot, Plotting._categorical_vs_continuous_violin_plot, {"x": self.x, "y": self.y}),
            (self._matplot, Plotting._categorical_vs_continuous_box_plot, {"x": self.x, "y": self.y})]
        combos[Plotting._build_plot_key(OrdinalTypedFeature, ContinuousTypedFeature)] = [
            (self._matplot, Plotting._ordinal_vs_continuous_violin_plot, {"x": self.x, "y": self.y}),
            (self._matplot, Plotting._ordinal_vs_continuous_box_plot, {"x": self.x, "y": self.y})]
        combos[Plotting._build_plot_key(ContinuousTypedFeature, OrdinalTypedFeature)] = [
            (self._matplot, Plotting._ordinal_vs_continuous_horizontal_violin_plot, {"x": self.y, "y": self.x}),
            (self._matplot, Plotting._ordinal_vs_continuous_horizontal_box_plot, {"x": self.y, "y": self.x})]
        combos[Plotting._build_plot_key(ContinuousTypedFeature, CategoricalTypedFeature)] = [
            (self._matplot, Plotting._multiple_pdf, {"x": self.x, "y": self.y})]
        combos[Plotting._build_plot_key(ConstantTypedFeature, ContinuousTypedFeature)] = [
            (self._matplot, sns.barplot, {"x": self.x, "y": self.y})]
        combos[Plotting._build_plot_key(ContinuousTypedFeature, ConstantTypedFeature)] = [
        ...
        combos[Plotting._build_plot_key(DiscreteTypedFeature, ConstantTypedFeature)] = [
            (self._matplot, Plotting._ordinal_vs_constant_count_plot, {"x": self.x, "hue": self.y, "yscale": self.yscale})]
        combos[Plotting._build_plot_key(DateTimeTypedFeature, ContinuousTypedFeature)] = [
            (self._matplot, plt.scatter,
             {"x": self.x, "y": self.y, "s": pi / 10 * (matplotlib.rcParams['lines.markersize'] ** 2), "edgecolor": "white", "linewidths": "0.1"})]
        combos[Plotting._build_plot_key(DateTimeTypedFeature, OrdinalTypedFeature)] = [
            (self._matplot, plt.scatter, {"x": self.x, "y": self.y, "edgecolor": "white", "linewidths": "0.1"})]
        combos[Plotting._build_plot_key(ContinuousTypedFeature, ContinuousTypedFeature)] = [
            (self._matplot, Plotting._gaussian_heatmap,
             {"x": self.x, "y": self.y, "s": pi / 10 * (matplotlib.rcParams['lines.markersize'] ** 2)}),
            (self._matplot, plt.scatter, {"x": self.x, "y": self.y, "edgecolor": "white", "linewidths": "0.1"})]
        combos[Plotting._build_plot_key(OrdinalTypedFeature, OrdinalTypedFeature)] = [
            (self._matplot, sns.scatterplot,
             {"x": self.x, "y": self.y, "s": pi / 10 * (matplotlib.rcParams['lines.markersize'] ** 2), "edgecolor": "white", "linewidths": "0.1"})]
        combos[Plotting._build_plot_key(OrdinalTypedFeature, DiscreteTypedFeature)] = [
            (self._matplot, sns.countplot, {"x": self.x, "hue": self.y})]
        combos[Plotting._build_plot_key(OrdinalTypedFeature, CategoricalTypedFeature)] = [
            (self._matplot, Plotting._count_plot, {"x": self.x, "hue": self.y, "yscale": self.yscale})]
        combos[Plotting._build_plot_key(CategoricalTypedFeature, OrdinalTypedFeature)] = [
            (self._matplot, Plotting._count_plot, {"x": self.y, "hue": self.x, "yscale": self.yscale})]
        combos[Plotting._build_plot_key(DiscreteTypedFeature, OrdinalTypedFeature)] = [
            (self._matplot, sns.countplot,
             {"x": self.x, "hue": self.y, "order": self.df[self.x].value_counts(ascending=True).iloc[:10].index})]
        combos[Plotting._build_plot_key(DiscreteTypedFeature, CategoricalTypedFeature)] = [
            (self._matplot, sns.countplot, {"x": self.x, "hue": self.y, "order": self.df[self.x].value_counts().iloc[:10].index})]
        combos[Plotting._build_plot_key(DateTimeTypedFeature, DateTimeTypedFeature)] = [
            (self._matplot, plt.scatter,
             {"x": self.x, "y": self.y, "s": pi / 10 * (matplotlib.rcParams['lines.markersize'] ** 2), "edgecolor": "white", "linewidths": "0.1"})]

        combos[Plotting._build_plot_key(ContinuousTypedFeature, None)] = [(self._matplot, plt.hist, {"x": self.x})]
        combos[Plotting._build_plot_key(CategoricalTypedFeature, None)] = [
            (self._matplot, Plotting._single_column_count_plot, {"x": self.x, "yscale": self.yscale})]
        combos[Plotting._build_plot_key(OrdinalTypedFeature, None)] = [
            (self._matplot, Plotting._ordinal_count_plot, {"x": self.x, "yscale": self.yscale})]
        combos[Plotting._build_plot_key(ConstantTypedFeature, None)] = [(self._matplot, sns.countplot, {"x": self.x})]
        combos[Plotting._build_plot_key(DateTimeTypedFeature, None)] = [(self._matplot, plt.hist, {"x": self.x, "bins": 10, "color": "#1f77b4"})]
        combos[Plotting._build_plot_key(GISTypedFeature, None)] = [(self._generic_plot, Plotting._folium_map, {"x": self.x})]

        y_type_name = None if self.y_type is None else self.y_type.__class__
        keys_to_check = list([Plotting._build_plot_key(self.x_type.__class__, y_type_name)])
        ...
        keys_to_check.append(Plotting._build_plot_key(self.x_type.__class__, new_y_type))
        ...
        if y_type_name is not None:
            raise NotImplementedError("Plotting for the feature combination ({0} vs {1}) is not yet supported."
                                      .format(self.x_type.meta_data["type"], self.y_type.meta_data["type"]))
        else:
            raise NotImplementedError("Plotting for feature type {0} is not supported"
                                      .format(self.x_type.meta_data["type"]))

    def _change_type(feature_type):
        if feature_type is None:
            return None

        return DiscreteTypedFeature if isinstance(feature_type, DiscreteTypedFeature) or isinstance(feature_type,
                                                                                                    CreditCardTypedFeature) \
                                       or isinstance(feature_type, ZipcodeTypedFeature) or isinstance(feature_type,
                                                                                                      OrdinalTypedFeature) \
