...
from bokeh.models import (BasicTicker, ColorBar, ColumnDataSource, FactorRange,
                          HoverTool, LabelSet, LinearColorMapper, Panel,
                          PrintfTickFormatter, Tabs)
...
class BokehHeatMap(object):
    ...
    def __init__(self, ds):
        ...
        self.bokehpalette = [mpl.colors.rgb2hex(m) for m in self.colormap(np.arange(self.colormap.N))]
    ...
    def flatten_corr_matrix(self, corr_matrix):
        ...
        corr_flatten = pd.DataFrame([(r, c, corr_matrix[r][c]) for c in columns for r in rows], columns=['x', 'y', 'corr'])
        return corr_flatten
    
    def plot_heat_map(self, matrix, xrange: list, yrange: list, low: float = 1, high=1, title: str = None, tool_tips: list = None):
        """
        ...
        p = figure(title=title, x_range=xrange, y_range=yrange
                   , toolbar_location='below', toolbar_sticky=False, plot_width=600, plot_height=600)
    ...
    def plot_hbar(self, matrix, low: float = 1, high=1, title: str = None, tool_tips: list = None, column_name: str = None):
        """
        ...
        p = figure(title=f"{title} ({column_name})", x_range=(low, high), y_range=(0, len(matrix['Y'])+1)
                   , toolbar_location='below', toolbar_sticky=False, plot_width=600, plot_height=600)

        p.hbar(y='Y', height=0.5, left=0, source=source,
               right='corr', fill_color={'field': 'corr', 'transform': mapper}, line_color=None)
        ...
        labels = LabelSet(x='label', y='Y', text='X', level='glyph',
    ...
    def generate_heatmap(self, corr_matrix, title: str, msg: str, correlation_threshold: float):
        """
        ...
        p = self.plot_heat_map(corr_flatten, corr_matrix.index.values.tolist(), corr_matrix.columns.values.tolist(), low, high, title, tool_tips=[("X", "@x"), ("Y", "@y"), ("Corr", "@corr")])

        tab = Panel(child=p, title=title)
        return tab

    def generate_target_heatmap(self, corr_matrix, title: str, correlation_target: str, msg: str,
                                correlation_threshold: float):
        """
        ...
        corr_flatten['Y'] = [i for i in range(1, len(corr_clean.index.values.tolist())+1)]

        corr_flatten['corr'] = corr_clean.values.tolist()
        corr_flatten['label'] = [x+0.01 if x > 0 else 0.01 for x in corr_clean.values.tolist()]
        ...
        p = self.plot_hbar(corr_flatten, low, high, title, tool_tips=[("Feature", "@X"), ("Corr", "@corr")],
                           column_name=correlation_target)
        
        tab = Panel(child=p, title=title)
        return tab

    def plot_correlation_heatmap(self, ds, plot_type: str = "heatmap", correlation_target: str = None,
        ...
        correlation_list = plot_ds.corr(frac=frac,
        ...
        # generate the msg
        cts_cts_msg = _generate_msg('pearson', correlation_methods, correlation_target, 'continuous')
        cat_cat_msg = _generate_msg('cramers v', correlation_methods, correlation_target, 'categorical')
        cat_cts_msg = _generate_msg('correlation ratio', correlation_methods, correlation_target, 'continuous and categorical')
...
def _generate_msg(method: str, correlation_methods: list, correlation_target: str, feature_type: str):
    if method in correlation_methods or 'all' in correlation_methods:
        if correlation_target is not None:
            msg = correlation_target + f'is not of the {feature_type} type or not enough of the {feature_type} type(s)'
        else:
...
def _has_categorical_features(ds):
  ...
  return 'categorical' in df_type['type'].values or 'zipcode' in df_type['type'].values
...
def _corr_filter(correlation_threshold, corr):
    ...
    new_corr = new_corr[new_corr >= correlation_threshold].dropna(
