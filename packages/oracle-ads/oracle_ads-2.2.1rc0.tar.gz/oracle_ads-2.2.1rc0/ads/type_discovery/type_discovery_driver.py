...
class TypeDiscoveryDriver:
    ...
    def discover(self, name: str, s: pd.Series, is_target: bool = False):
        ...
        assert isinstance(s, pd.Series) or isinstance(s, dd.Series), "Cannot perform type discovery on series that " \
                                                                     "is not dask or ads."
        ...
        #
        return utils.first_not_none((plugin.discover(name, s) for plugin in discovery_plugins))
