...
mpl.rcParams['axes.prop_cycle'] = cycler(color=['teal', 'blueviolet', 'forestgreen', 'peru', 'y', 'dodgerblue', 'r'])
...
class ADSEvaluator(object):
    ...
    def __init__(self, test_data, models, training_data=None, positive_class=None, legend_labels=None,
        ...
        self.classes = []
        self.is_classifier = hasattr(models[0], 'classes_') and models[0].classes_ is not None
        pclass = positive_class
        if self.is_classifier:
            self.classes = list(models[0].classes_)
            if len(self.classes) == 2:
                self.metrics_to_show = ['accuracy', 'hamming_loss', 'precision', 'recall', 'f1', 'auc']
                if positive_class is None or positive_class not in self.classes:
                    pclass = next((x for x in list(self.classes) if str(x).lower()
                                   in ADSEvaluator.Positive_Class_Names), self.classes[0])
                    utils.print_user_message(f"No positive class set, so we will assume that {pclass} is the positive "
                                             f"class. To change this, recreate the evaluator object with the "
                                             f"parameter positive_class", title="Warning")
            else:
                # Multi-class
                self.metrics_to_show = ['accuracy', 'hamming_loss', 'precision_weighted',
                                        'precision_micro', 'recall_weighted', 'recall_micro', 'f1_weighted', 'f1_micro']
        else:
            # Regression
        ...
        for m in models:
            if not (isinstance(m, ADSModel)):
                try:
                    ...
                    utils.print_user_message("This model cannot be converted to an ADS Model")
    ...
    def add_metrics(self, funcs, names):
        ...
        for name, f in zip(names, funcs):
            ...
            for m in self.evaluations[1].columns:
                res = f(self.evaluations[1][m]['y_true'], self.evaluations[1][m]['y_pred'])
                f_res.append(res)
            pd_res = pd.DataFrame([f_res], columns=self.evaluations[1].columns, index=[name])
            self.evaluations[1] = pd.concat([self.evaluations[1], pd_res])
            if self.evaluations[0].shape != (0, 0):
                f_res = []
                for m in self.evaluations[0].columns:
                    res = f(self.evaluations[0][m]['y_true'], self.evaluations[0][m]['y_pred'])
                    f_res.append(res)
                pd_res = pd.DataFrame([f_res], columns=self.evaluations[0].columns, index=[name])
            ...
            for i, m in enumerate(models):
                ...
                if self.training_data is not None:
                    ...
                    train_metrics = ModelEvaluator(
                        ...
                        y_score=y_score,
                        positive_class=self.positive_class).get_metrics()
                    total_train_metrics = pd.concat([total_train_metrics, train_metrics], axis=1)
                ...
                test_metrics = ModelEvaluator(
                    ...
                    y_score=y_score,
                    positive_class=self.positive_class).get_metrics()
                total_test_metrics = pd.concat([total_test_metrics, test_metrics], axis=1, sort=False)
    ...
    def show_in_notebook(self, plots=None, use_training_data=False, perfect=False, baseline=True, legend_labels=None):
        ...
        if use_training_data:
            if self.training_data is None:
                raise ValueError("Training data is not provided. Re-build ADSData with training and test data")
            model_evaluation = self.evaluations[0]
        else:
            model_evaluation = self.evaluations[1]
        legend_labels = legend_labels if legend_labels is not None else self.legend_labels
        # pass to plotting class
        EvaluationPlot.plot(model_evaluation, plots, len(self.classes), perfect, baseline, legend_labels)

    def calculate_cost(self, tn_weight, fp_weight, fn_weight, tp_weight, use_training_data=False):
        """Returns a cost associated with the input weights
        ...
        if len(self.classes) != 2:
            raise ValueError("The calculate_cost api is not supported for non-binary classification datasets.")
        cost_per_model = []
        if use_training_data:
            if self.training_data is None:
                raise ValueError("Training data is not provided. Re-build ADSData with training and test data.")
        ...
        for m in list_of_model:
            ...
            cost_per_model.append(tn * tn_weight + fp * fp_weight + fn * fn_weight + tp * tp_weight)
    ...
    class EvaluationMetrics(object):
        ...
        DEFAULT_LABELS_MAP = {"accuracy": "Accuracy",
        ...
        def __init__(self, ev_test, ev_train, use_training=False, less_is_more=None, precision=4):
            self.ev_test = ev_test
            self.ev_train = ev_train
            self.use_training = use_training
            self.precision = precision
            if isinstance(less_is_more, list):
                self.less_is_more = ['hamming_loss', 'hinge_loss', 'mse', 'mae'] + less_is_more
            else:
        ...
        def show_in_notebook(self, labels=DEFAULT_LABELS_MAP):
            ...
            def _display_metrics(df, data_name, labels, precision):
                ...
                Returns:
                    Nothing
                """                
                display(HTML(_pretty_label(df, labels).style.apply(highlight_max, axis=1)
                         .set_precision(precision)
                         .set_properties(**{'text-align': 'center'})
                         .set_table_styles(table_styles)
                         .set_table_attributes('class=table')
                         .set_caption('<div align="left"><b style="font-size:20px;">' +
                                      'Evaluation Metrics (' + data_name + '):</b></div>')
                         .render()))
    ...
    def metrics(self):
        ...
        ev_train = self.evaluations[0].loc[self.metrics_to_show] if use_training else None
    ...
    def _get_model_name(self, name, show_full_name=False):
        ...
        if '(' in name_edit and not show_full_name:
            ...
            utils.print_user_message("Pass the argument <code>show_full_name = True</code> to the ADSEvaluator to show the full argument of this model", title=None)
        if name_edit in self.model_names:
            ...
            if num_tries == 1:
                utils.print_user_message("Multiple models have the name %s, rename the models with the .rename function"
                                         " for easier interpretation. These models have been renamed by the order in "
                                         "which they were given " % name_edit[:-2])
                ...
                if self.positive_class is not None:
                    if self.positive_class not in list(est.classes_):
                        raise ValueError(
                            ...
                            % (self.positive_class, est.est.__class__.__name__, list(est.classes_)))
