...
import ads
from ads.common.utils import ml_task_types, wrap_lines, \
    is_documentation_mode, is_notebook, print_user_message
...
class AutoMLProvider(ABC):
    ...
    def setup(self, X_train, y_train, ml_task_type, X_valid=None, y_valid=None, class_names=None, client=None):
        '''
...
class BaselineAutoMLProvider(AutoMLProvider):
    ...
    def train(self, **kwargs):
        ...
        if self.est is None:
            raise ValueError("Baseline model for (%s) is not supported" % self.ml_task_type)
        try:
            self.est.fit(self.X_train, self.y_train)
        except Exception as e:
            warning_message = "The following baseline estimator failed to fit properly and is not possible to evaluate: " + str(self.est) + " Exception: " + str(e)
    ...
    def decide_estimator(self, **kwargs):
        ...
        if self.est is not None:
            ...
            elif self.ml_task_type in [ml_task_types.BINARY_CLASSIFICATION, ml_task_types.MULTI_CLASS_CLASSIFICATION,
...
class OracleAutoMLProvider(AutoMLProvider, ABC):
    def __init__(self, n_jobs=-1, loglevel=None, logger_override=None):
        ...
        try:
            ...
            print_user_message(["automl", "ads[labs]"], display_type="module")
            raise e
        ...
        if not logger_override:
            self.automl.init(engine="dask",engine_opts={'dask_scheduler':ml_runtime.compute_accelerator()}, loglevel=loglevel)
        else:
            self.automl.init(engine="dask",engine_opts={'dask_scheduler':ml_runtime.compute_accelerator()},logger=logr)
    ...
    def get_transformer_pipeline(self):
        ...
        if hasattr(self.est, 'text') and not self.est.text:
            msg1 = wrap_lines(self.est.selected_features_names_,
    ...
    def print_summary(self, max_rows=None, sort_column='Mean Validation Score', ranking_table_only=False):
        '''
        ...
        '''
        if is_documentation_mode() and is_notebook(): # pragma: no cover
            print_user_message("Training complete ({0:.2f} seconds)".format(time.time()-self.train_start_time), title=None)
            info = [
                ['Training Dataset size', self.X_train.shape],
                ['Validation Dataset size', self.X_valid.shape if self.X_valid is not None else None],
                ...
                ['End-to-end Elapsed Time (seconds)', self.train_end_time - self.train_start_time],
            ...
            all_trials_ = self.est.model_selection_trials_ + self.est.adaptive_sampling_trials_ + \
                self.est.feature_selection_trials_ + self.est.tuning_trials_[1:]
            col_names = ['Algorithm', '#Samples', '#Features', 'Mean Validation Score', \
                'Hyperparameters', 'All Validation Scores', 'CPU Time']
            if ranking_table_only:
                dropped_cols = ['#Samples', '#Features', 'All Validation Scores', 'CPU Time']
            else:
                dropped_cols = 'All Validation Scores'
            summary_df = pd.DataFrame(all_trials_, columns=col_names).drop(dropped_cols, axis=1)
            sorted_summary_df = summary_df.sort_values(sort_column, ascending=False)
            # Add a rank column at the front
            sorted_summary_df.insert(0, 'Rank based on Performance', np.arange(2, len(sorted_summary_df)+2))
            ...
            with pd.option_context('display.max_colwidth', 1000, 'display.width', None, 'display.precision', 4):
                ...
                if max_rows is None:
                    ...
                    display(HTML(sorted_summary_df.to_html(index=False, max_rows=max_rows)))
    ...
    def train(self, **kwargs):
            ...
            if 'model_list' in kwargs:
                ...
                for model in kwargs['model_list']:
                    for item in bad_model_list:
                        if item in model:
                            raise ValueError("SVC, KNeighborsClassifier are disabled for datasets with > 10K samples")
        ...
        with warnings.catch_warnings():
            ...
            self.est.fit(self.X_train, self.y_train, X_valid=self.X_valid,
                y_valid=self.y_valid, time_budget=self.time_budget)
    ...
    def _decide_estimator(self, **kwargs):
        ...
        if self.ml_task_type == ml_task_types.BINARY_CLASSIFICATION or \
            self.ml_task_type == ml_task_types.BINARY_TEXT_CLASSIFICATION:
            test_model_list = ['LogisticRegression']
        elif self.ml_task_type == ml_task_types.MULTI_CLASS_CLASSIFICATION or \
            self.ml_task_type == ml_task_types.MULTI_CLASS_TEXT_CLASSIFICATION:
            test_model_list = ['LogisticRegression']
        ...
        if self.ml_task_type == ml_task_types.BINARY_CLASSIFICATION or \
            ...
            est = self.automl.Pipeline(task='classification', score_metric=score_metric, **kwargs)
        elif self.ml_task_type == ml_task_types.BINARY_TEXT_CLASSIFICATION or \
            self.ml_task_type == ml_task_types.MULTI_CLASS_TEXT_CLASSIFICATION:
            est = self.automl.Pipeline(task='classification', text=True, score_metric=score_metric, **kwargs)
        elif self.ml_task_type == ml_task_types.REGRESSION:
            est = self.automl.Pipeline(task='regression', score_metric=score_metric, **kwargs)
        else:
            raise ValueError("AutoML for (%s) is not supported" % self.ml_task_type)
        return est
    ...
    def visualize_algorithm_selection_trials(self, ylabel=None):
        ...
        if not len(trials):
            print_user_message("Unfortunately, there were no trials found for Algorithm Selection, \
            so we cannot visualize it.", display_type="error")
            return
        fig, ax = plt.subplots(1, figsize=(6, 3))
        colors = []
        y_error = []
        mean_scores, models, cvscores = [], [], []
        for (algorithm, samples, features, mean_score, hyperparameters, all_scores, runtime) in trials:
            mean_scores.append(mean_score)
            models.append(algorithm)
            cvscores.append(all_scores)
        mean_scores_ser = pd.Series(mean_scores, index=models).sort_values(ascending=False)
        ...
        ax.errorbar(x=mean_scores_ser.index.values, y=mean_scores_ser.values, yerr=y_error, fmt='none', capsize=4, color='black', zorder=0)
    ...
    def visualize_adaptive_sampling_trials(self):
        ...
        if len(trials) == 0:
            print_user_message("Unfortunately, there were no trials found for Adaptive Sampling, \
            so we cannot visualize it.", display_type="error")
            return
        fig, ax = plt.subplots(1, figsize=(6, 3))
        ...
        scores = [mean_score for (algorithm, samples, features, mean_score, hyperparameters, all_scores, runtime) in trials]
        n_samples = [samples for (algorithm, samples, features, mean_score, hyperparameters, all_scores, runtime) in trials]
    ...
    def visualize_feature_selection_trials(self, ylabel=None):
        ...
        if len(trials) == 0:
            print_user_message("Unfortunately, there were no trials found for Feature Selection, \
            so we cannot visualize it.", display_type="error")
            return
        fig, ax = plt.subplots(1, figsize=(6, 3))
        ...
        ax.set_ylabel(ylabel)
        scores = [mean_score for (algorithm, samples, features, mean_score, hyperparameters, all_scores, runtime) in trials]
        n_features = [features for (algorithm, samples, features, mean_score, hyperparameters, all_scores, runtime) in trials]
        y_margin = 0.10 * (max(scores) - min(scores))
        ax.grid(color='g', linestyle='-', linewidth=0.1)
        ax.set_ylim(min(scores) - y_margin, max(scores) + y_margin)
        ax.plot(n_features, scores, 'k:', marker="s", color='teal', markersize=3)
        ax.axvline(x=len(self.est.selected_features_names_), color='orange', linewidth=2.0)
        plt.show()
    ...
    def visualize_tuning_trials(self, ylabel=None):
        ...
        scores = [mean_score for (algorithm, samples, features, mean_score, hyperparameters, all_scores, runtime) in
                  reversed(self.est.tuning_trials_) if mean_score and not np.isnan(mean_score)]
        if not len(scores) > 1:
            print_user_message(f"Insufficient tuning trials ({len(scores)}r) to visualize in the time budget \
                             of {self.time_budget} seconds.", display_type="error")
            ...
            ax.grid(color='g', linestyle='-', linewidth=0.1)
            ax.set_ylim(min(scores) - y_margin, max(scores) + y_margin)
            ax.plot(range(1, len(scores) + 1), scores, 'k:', marker="s", color='teal', markersize=3)
            plt.show()
