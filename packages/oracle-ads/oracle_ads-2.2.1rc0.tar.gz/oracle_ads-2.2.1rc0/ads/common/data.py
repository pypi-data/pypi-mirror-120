...
class ADSData(object):
    ...
    def build(X=None, y=None, name="", dataset_type=None, **kwargs):
        ...
        if isinstance(y, str):
            try:
                return ADSData(X.drop(y, axis=1), X[y], name=name, dataset_type=dataset_type)
            except AttributeError:
                raise ValueError("If y is a string, then X must be a pandas or dask dataframe")
        else:
    ...
    def to_onnxrt(self, sess, idx_range=None, model=None, **kwargs): # pragma: no cover
        ...
        if model._underlying_model in ["torch"]:
            ...
            for i, name in enumerate(in_name):
                ...
                ret[name] = self.X[:batch_size].reshape(
                    [batch_size] + list(self.X[:1].shape)).detach().cpu().numpy().astype(np.float32)
            return ret
            ...
            for idx, c in enumerate(X_trans.columns):
                inputs[sess.get_inputs()[idx].name] = X_trans[c].values.reshape((X_trans.shape[0], 1)) \
