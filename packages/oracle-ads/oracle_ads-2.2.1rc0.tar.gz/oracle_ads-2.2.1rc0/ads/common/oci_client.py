...
class OCIClientFactory:
    ...
    """
    A factory class to create OCI client objects. The constructor takes in config, signer and extra_args. `extra_args` is passed
    ...
    """
    def __init__(self, config={}, signer=None, extra_args=None):
        self.config = config
        self.signer = signer
    ...
    def _client_impl(self, client):
        ...
        assert client in client_map, f"Invalid client name. Client name not found in {client_map.keys()}"
    ...
    def create_client(self, client_name):
        ...
        client = self._client_impl(client_name) if isinstance(
            client_name, str) else client_name
        if self.extra_args:
            return client(config=self.config, signer=self.signer, **self.extra_args)
        else:
            return client(config=self.config, signer=self.signer)
