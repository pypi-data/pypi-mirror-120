...
def _get_pre_selected_features(est,
...
def init_lime_explainer(explainer,
                        est,
                        X_train,
                        y_train,
                        mode,
    ...
    if explainer is None:
        ...
        if not isinstance(explainer, LimeExplainer):
            raise TypeError("Invalid explainer provided to "
                            "init_lime_explainer: {}".format(type(explainer)))
        exp = explainer
    exp_type = check_tabular_or_text(est, X_train)
    exp.set_config(type=exp_type, mode=mode, discretizer=discretization, client=client, batch_size=batch_size,
                   scale_weight=scale_weight, surrogate_model=surrogate_model, num_samples=num_samples,
                   exp_sorting=exp_sorting, kernel_width='dynamic')
...
def init_permutation_importance_explainer(explainer,
                                          est,
                                          X_train,
                                          y_train,
                                          mode,
    ...
    if check_tabular_or_text(est, X_train) == 'text':
        raise TypeError("Global feature importance explainers are currently not "
                        "supported for text datasets.")
    exp.set_config(mode=mode, client=client, random_state=random_state)
    selected_features = _get_pre_selected_features(est, use_pre_selected_features)
    exp.fit(est, X_train, y=y_train, target_names=class_names, selected_features=selected_features)
    return exp


def init_partial_dependence_explainer(explainer,
                                      est,
                                      X_train,
                                      y_train,
                                      mode,
    ...
    if explainer is None:
        ...
        exp = explainer
    if check_tabular_or_text(est, X_train) == 'text':
        raise TypeError("Global partial dependence explainers are currently not "
                        "supported for text datasets.")
...
def init_ale_explainer(explainer,
                       est,
                       X_train,
                       y_train,
                       mode,
...
def init_whatif_explainer(explainer,
                          est,
                          X_test,
                          y_test,
                          mode,
    ...
    if explainer is None:
        ...
        if not isinstance(explainer, WhatIf):
            raise TypeError("Invalid explorer provided to "
                            "init_explorer: {}".format(type(explainer)))
        exp = explainer
    exp_type = check_tabular_or_text(est, X_test)
    if exp_type == 'text':
        raise TypeError("WhatIf explainer are currently not "
                        "supported for text datasets.".format(type(explainer)))
    exp.fit(model=est, X=X_test, y=y_test, target_names=class_names, train=train, target_title=target_title)
    return exp
