...
class MLXWhatIfExplainer(WhatIfExplainer):
    ...
    def explore_sample(self, row_idx=None, features=None, max_features=32, **kwargs):
        ...
        if self.explainer is None:
            self.explainer = init_whatif_explainer(
                ...
                y_test=self.y_test_,
                mode=self.mode_,
                class_names=self.class_names_ if self.mode_ == 'classification' else ['prediction'],
                train=self.X_train_,
                target_title=None,
                **kwargs
            )
        ...
        if row_idx is not None and row_idx not in self.X_test.index:
            utils.print_user_message(msg="<code>row_idx</code> {} not in the index, reset to 0.".format(str(row_idx)),
                                     display_type='tip')
        ...
        if features:
            ...
            if not all(np.isin(features, feature_names)):
                utils.print_user_message(msg="One or more <code>features</code> {} does not exist in data. \n"
                                             "Existing features: {}.".format(str(features), str(feature_names)),
    ...
    def explore_predictions(self, x=None, y=None, label=None, plot_type='scatter', discretization=None, **kwargs):
        """
        ...
        if isinstance(x, list) or isinstance(y, list):
            utils.print_user_message(msg="You may only pass one feature in.",
                                     display_type='error')
            return
        for feature in [x, y]:
            if feature is not None:
                ...
                if not np.isin(feature_upper, feature_names):
                    utils.print_user_message(msg="<code>{}</code> does not exist in data. \n"
                                                 "Existing features: "
                                                 "<code>{}</code>".format(feature_upper, str(self.X_test.columns.tolist())),
                                             display_type='error',
                                             title='Invalid <code>feature</code>')
                    return
        if self.class_names_ is not None:
            ...
            if isinstance(label, str) and label not in self.class_names_:
                utils.print_user_message(msg="<code>label</code> can only be one of {}".format(self.class_names_),
                                         display_type='error')
                return
            if isinstance(label, int) and label > num_classes - 1:
                utils.print_user_message(msg="The total number of classes is {}, "
                                             "hence label can not exceed {}".format(int(num_classes),
                                                                        int(num_classes - 1)),
                                         display_type='error')
                return
        ...
        if plot_type not in plot_type_options:
            utils.print_user_message(msg="<code>plot_type</code> has to be one of : {}".format(str(plot_type_options)),
                                     display_type='error',
                                     title='Invalid <code>plot_type</code>')
            return
        discretization_options = [None, 'quartile', 'decile', 'percentile']
        if discretization not in discretization_options:
            utils.print_user_message(msg="<code>discretization</code> has to be "
                                         "one of : {}".format(str(discretization_options)),
                                     display_type='error',
                                     title='Invalid <code>discretization</code>')
            return
