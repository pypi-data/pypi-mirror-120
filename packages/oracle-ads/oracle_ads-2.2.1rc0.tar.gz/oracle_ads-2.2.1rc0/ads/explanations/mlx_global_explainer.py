...
from ads.explanations.mlx_interface import init_partial_dependence_explainer, init_ale_explainer
...
class MLXGlobalExplainer(GlobalExplainer):
    ...
    def compute_feature_importance(self,
        ...
        if self.explainer.config.type == 'text':
            ...
            explanation = self.explainer.explain_aggregate_local(self.X_test, sampling=sampling, labels=labels)
        else:
            if self.mode_ == "regression":
                allowed_metrics = ["r2", "neg_mean_squared_error", "neg_root_mean_squared_error", "neg_mean_absolute_error",
                    "neg_median_absolute_error", "neg_mean_absolute_percentage_error",
                    "neg_symmetric_mean_absolute_percentage_error"]
            elif self.mode_ == "classification" and len(self.class_names) == 2:
                #Binary classification
                allowed_metrics = ["f1_weighted", "f1_micro", "f1_macro", "recall_weighted", "recall_micro", "recall_macro",
                    "accuracy", "balanced_accuracy", "roc_auc", "precision_weighted", "precision_macro", "precision_micro"]
            else:
                #Multiclass classification
                allowed_metrics = ["f1_weighted", "f1_micro", "f1_macro", "recall_weighted", "recall_micro", "recall_macro",
                    "accuracy", "balanced_accuracy", "roc_auc", "precision_weighted", "precision_macro", "precision_micro"]
            if scoring_metric not in allowed_metrics and scoring_metric is not None:
                raise Exception('Scoring Metric not supported for this type of problem: {}, for problem type {}, the availble supported metrics are {}'.format(scoring_metric, self.mode_, allowed_metrics))
            if balance and sampling is None:
                sampling = {'technique': 'random'}
            explanation = self.explainer.compute(self.X_test, self.y_test, n_iter=n_iter, sampling=sampling,
                                                 balance=balance, scoring_metric=scoring_metric)
    ...
    def compute_partial_dependence(self,
        ...
        if check_tabular_or_text(self.est, self.X_train) == "tabular":
            ...
            if hasattr(self.est, "selected_features_"):
                ...
                filtered_list = [i for indx, i in enumerate(feature_ids) if boolean_list[indx] == False]
                if not all(boolean_list):
                    print("Feature:", self.X_train.columns.values[filtered_list], "is not selected by the model.")
                    return

            if len(feature_ids) > 2:
                raise ValueError("Maximum number of partial dependency features is 2.")

            return MLXPartialDependencies(pdp=self.pdp_explainer.compute(
                data=self.X_train, partial_ids=feature_ids, partial_range=partial_range,
                num_samples=num_samples, sampling=sampling), pdp_exp=self.pdp_explainer)
        else:
            raise ValueError("Partial Dependence Plot is not supported for text classification dataset.")

    def compute_accumulated_local_effects(self,
                                          feature,
                                          partial_range=(0.00, 1.0),
                                          num_samples=30,
                                          sampling=None,
        ...
        corr_threshold : float, optional
             Value between 0.0 and 1.0 for which a feature is considered highly correlated with another feature (Default = 0.7).
        ...
        if check_tabular_or_text(self.est, self.X_train) == "tabular":
            ...
            if len(feature_ids) > 1:
                raise ValueError("Maximum number of Accumulated Local Effects features is 1.")

            return MLXAccumulatedLocalEffects(ale=self.ale_explainer.compute(
                data=self.X_train, partial_ids=feature_ids, partial_range=partial_range,
                num_samples=num_samples, sampling=sampling, corr_threshold=corr_threshold),
                ale_exp=self.ale_explainer)
        else:
            raise ValueError("Accumulated Local Effects Plot is not supported for text classification dataset.")
    ...
    def configure_feature_importance(self, **kwargs):
        ...
        if check_tabular_or_text(self.est, self.X_train) == "tabular":
            ...
            avail_args = ['client', 'random_state', 'surrogate_model', 'num_samples',
                          'exp_sorting', 'scale_weight', 'batch_size']

        for k, _ in kwargs.items():
            if k not in avail_args:
                raise ValueError('Unexpected argument for the feature importance explainer: {}'.format(k))

        if kwargs.get('client', None) is not None:
            raise ValueError('Invalid client provided. Currently only supports disabling parallelization '
                             'by setting client=None')
        if kwargs.get('surrogate_model', None) not in ['linear', 'decision_tree', None]:
            raise ValueError('Invalid surrogate_model provided. Currently only supports linear or decision_tree')
    ...
    def configure_partial_dependence(self, **kwargs):
        ...
        for k, _ in kwargs.items():
            if k not in ['client']:
                raise ValueError('Unexpected argument for the partial dependence explainer: {}'.format(k))
        if kwargs.get('client', None) is not None:
            raise ValueError('Invalid client provided. Currently only supports disabling parallelization '
                             'by setting client=None')
    ...
    def configure_accumulated_local_effects(self, **kwargs):
        ...
        for k, _ in kwargs.items():
            if k not in ['client']:
                raise ValueError('Unexpected argument for the accumulated local effects explainer: {}'.format(k))
        if kwargs.get('client', None) is not None:
            raise ValueError('Invalid client provided. Currently only supports disabling parallelization '
                             'by setting client=None')
    ...
    def _init_feature_importance(self,
                                 **kwargs):
        """
        Internal function to initializes the feature importance explainer. Tabular datasets
        ...
        **kwargs : dict
        ...
        if check_tabular_or_text(self.est, self.X_train) == "tabular":
            self.explainer = init_permutation_importance_explainer(self.explainer, self.est,
                                                                   self.X_train, self.y_train,
                                                                   self.mode, class_names=self.class_names,
                                                                   use_pre_selected_features=True, **kwargs)
        else:
            self.explainer = init_lime_explainer(self.explainer, self.est, self.X_train, self.y_train,
                                                 self.mode, class_names=self.class_names,
                                                 use_pre_selected_features=True, **kwargs)

    def _init_partial_dependence(self,
                                 **kwargs):
        """
        ...
        **kwargs : dict
        ...
        if self.mode == "regression":
            self.class_names_ = ["Target"]
        self.pdp_explainer = init_partial_dependence_explainer(self.pdp_explainer, self.est,
                                                                self.X_train, self.y_train, self.mode,
    ...
    def _init_accumulated_local_effects(self, **kwargs):
        ...
        **kwargs : dict
        ...
        if self.mode == "regression":
            self.class_names_ = ["Target"]
        self.ale_explainer = init_ale_explainer(self.ale_explainer,
                                                self.est,
                                                self.X_train,
                                                self.y_train,
                                                self.mode,
...
class MLXFeatureDependenceExplanation(ABC):
    ...
    def show_in_notebook(self,
                         ...
                         centered=False,
                         show_median=True):  # pragma: no cover
        """
...
class MLXPartialDependencies(MLXFeatureDependenceExplanation):
    ...
    def show_in_notebook(self,
        ...
        str
            Plotly HTML object containing a line chart, heat map, or violin plot for this feature dependence explanation
        """
        return self.fd.show_in_notebook(mode=mode, labels=labels, cscale=cscale, show_distribution=show_distribution,
                                        discrete_threshold=discrete_threshold, line_gap=line_gap,
                                        show_correlation_warning=show_correlation_warning,
                                        centered=centered, show_median=show_median)
...
class MLXAccumulatedLocalEffects(MLXFeatureDependenceExplanation):
    ...
    def show_in_notebook(self,
        ...
        str
            Plotly HTML object containing a line chart, heat map, or violin plot for this feature dependence explanation
        """
        return self.fd.show_in_notebook(mode='pdp', labels=labels, cscale=cscale, show_distribution=show_distribution,
                                        discrete_threshold=discrete_threshold, line_gap=0,
                                        show_correlation_warning=show_correlation_warning,
                                        centered=centered, show_median=show_median)
    ...
    def show_in_notebook(self,
        ...
        **kwargs : dict
        ...
        if self.type.type == 'text':
            if labels:
                labels = [self.class_names.index(label) if isinstance(label, str) else label for label in labels]
            else:
                if len(self.class_names) > 1:  # classification
                    utils.print_user_message("Specify <code>'labels'</code> to plot the local explanation for specific target labels")
                labels = list(range(len(self.class_names)))
            if len(self.class_names) == 2:
                return self.explanation.show_in_notebook()
            else:
                return self.explanation.show_in_notebook(mode=mode if mode else "wordcloud", show=show, labels=labels,
                                                         cscale=cscale, colormap=colormap, return_wordcloud=return_wordcloud,
                                                         **kwargs)
        else:
            if labels:
                raise ValueError("label is supported only for text explanation.")
            return self.explanation.show_in_notebook(n_features=n_features, mode=mode if mode else "bar")
