...
class TestNotebooks(unittest.TestCase):
    ...
    def runTest(self):
        ...
        with open(self.notebook_filename) as f:
            ...
            nb.cells.insert(0, nbf.new_code_cell("import ads; ads.set_debug_mode(True)"))
            ...
            try:
                ep.log = logger
                ep.preprocess(nb, {'metadata': {'path': os.path.dirname(self.notebook_filename)}})
            except CellExecutionError as e:
                msg = 'Error {0} in executing notebook {1}'.format(str(e), self.notebook_filename)
                error_flag = True
...
def get_suite(kernel=None):
    ...
    notebooks = (os.path.join(stable_notebook_folder, nb)
