...
from ads.dataset.helper import fix_column_names, convert_columns, get_feature_type, convert_to_html, DatasetDefaults
...
from ads.type_discovery.typed_feature import DateTimeTypedFeature, ContinuousTypedFeature, GISTypedFeature, \
    ConstantTypedFeature, CreditCardTypedFeature, ZipcodeTypedFeature, PhoneNumberTypedFeature, OrdinalTypedFeature, \
    CategoricalTypedFeature, DocumentTypedFeature, AddressTypedFeature
...
class PandasDataset(object):
    ...
    """
    def __init__(self, sampled_df, type_discovery=True, types={}, metadata=None, progress=DummyProgressBar()):
        ...
        self.numeric_columns = self.sampled_df.select_dtypes(utils.numeric_pandas_dtypes()).columns.values
            ...
            if type_discovery:
                    try:
                        ...
                        self.feature_types = {
                            ...
                            for col_name, col_vals in self.sampled_df.items() if col_name not in types}
    ...
    def _find_feature_subset(self, df, target_name, include_n_features=32):
        ...
        if len(df.columns) <= include_n_features:
            ...
            try:
                ...
                subset_features = [self.target.name] + list(itertools.compress(X_columns, est.get_support()))
    ...
    def _calculate_dataset_statistics(self, is_wide_dataset, out):
        ...
        if is_wide_dataset:
            ...
            for column_name in self.ddf.columns:
                self._update_multiple_outputs(out, f"calculating missing values (<code>{column_name}</code>)...")
            ...
            for column_name in self.numeric_columns:
                self._update_multiple_outputs(out, f"calculating skew (<code>{column_name}</code>)...")
                if len(self.ddf[column_name].dropna()) > 0:
                    d[column_name] = stats.skew(self.ddf[column_name].dropna()).compute().item(0)
                else:
            ...
            for col in self.ddf.columns:
                if df_missing['missing'][col] == len(self.ddf[col]):
                    self.ddf[col] = self.ddf[col].astype('object') # an column of null with type object is ignored by dask (while with type float will lead to error)
        ...
        #
        self._update_multiple_outputs(out, "Assembling statistics into single result...")
        if not df_missing.empty: df_stats = df_stats.join(df_missing).fillna('')
        if not df_skew.empty: df_stats = df_stats.join(df_skew).fillna('')

        return df_stats

    def _generate_features_html(self, is_wide_dataset, n_features, df_stats, visualizations_follow):
        ...
        if is_wide_dataset:
            html += ('''<p>The dataset has too many columns ({:,}) to
                ...
                statistics</p>'''.format(n_features))

        html += '<p><b>&#x2022; Note</b> these are computed on the entire dataset.</p>'
        html += '<hr>'
        html += df_stats\
                    .replace(np.nan, '')\
                    .style.set_table_styles(utils.get_dataframe_styles(max_width=50))\
    ...
    def _generate_warnings_html(self, is_wide_dataset, n_rows, n_features, df_stats, out, accordion):
        ...
        if 'unique' in df_stats.columns:
            for column_name, count in df_stats[['unique']][df_stats["unique"] != ""].iterrows():
                # ignore document types
                if self.feature_types[column_name]['type'] not in ignored_feature_types:
                    out.value = f"Analyzing cadinalities (<code>{column_name}</code>)..."
        ...
        if not is_wide_dataset:
            ...
            if 'min' in df_stats.columns:
                for column_name, count in df_stats[['min']][df_stats["min"] != ""].iterrows():
                    if self.feature_types[column_name]['type'] not in ignored_feature_types:
        ...
        if accum:
            ...
            '''.strip() for feature_warning in accum]
    ...
    def summary(self, feature_name=None):
        ...
        if feature_name is None:
            feature_n_datatype = {col: self.feature_types[col].type + "/" + self.feature_types[col].low_level_type for
                                  col in self.sampled_df.columns}
            df = pd.DataFrame(feature_n_datatype.items(), columns=['Feature', 'Datatype'])
        else:
            ...
            for col in feature_names:
                if col in self.sampled_df.columns:
                    feature_n_datatype[col] = {k: v for k, v in self.feature_types[col].meta_data.items() if
                                               k not in ['internal', 'feature_name']}
                    new_dict = utils.flatten(feature_n_datatype[col])
                    tmp_df = pd.DataFrame.from_dict(new_dict, orient='index',
    ...
    def timeseries(self, date_col):
        ...
        if date_col in self.feature_types and isinstance(self.feature_types[date_col], DateTimeTypedFeature):
            return Timeseries(date_col, self.sampled_df)
        else:
            raise ValueError("Not a date time column.")

    def plot(self, x, y=None, plot_type='infer', yscale=None, verbose=True, sample_size=0):
        """
        ...
        plot = Plotting(sub_samp_df, self.feature_types, x, y=y, plot_type=plot_type, yscale=yscale)
        if verbose:
            if len(self.ddf) != sub_samp_size:
                utils.print_user_message("Visualizations use a sampled dataset of size {:,} (confidence level: {}, confidence interval: {})".format(
                    sub_samp_df.shape[0],
    ...
    def plot_gis_scatter(self, lon="longitude", lat="latitude", ax=None):
        ...
        if lon in self.sampled_df.columns and lat in self.sampled_df.columns:
            ...
            gdf = geopandas.GeoDataFrame(self.sampled_df, geometry=geopandas.points_from_xy(self.sampled_df[lon], self.sampled_df[lat]))
            world = geopandas.read_file(geopandas.datasets.get_path('naturalearth_lowres'))
            ax1 = world.plot(ax=ax, color='lightgrey', linewidth=0.5, edgecolor='white')
            gdf.plot(ax=ax1, color='blue', markersize=10)

        else:
            utils.print_user_message("lon and/or lat must be columns (available columns are: {})".format(", ".join(self.sampled_df.columns)), display_type='info', title='Note')
    ...
    def _visualize_feature_distribution(self, html_widget):
        ...
        props = {'boxstyle': BoxStyle("Round", pad=0),
                 'facecolor': 'white',
                 'linestyle': 'solid',
                 'linewidth': 0,
                 'edgecolor': 'white'}
        ...
        blues = ['#AED6F1', '#85C1E9', '#5DADE2', '#3498DB', '#2E86C1', '#2874A6', '#1B4F72']
        ...
        for col_index, col in enumerate(self.sampled_df.columns):
            ...
            text = "\n".join([
                f'{col}\n',
                '  - type: {} ({})'.format(self.feature_types[col].type, self.feature_types[col].low_level_type),
                '  - missing_percentage: {:.1f}%'.format(self.feature_types[col].missing_percentage)
            ])

            fig, ax = PandasDataset._init_fig_ax(figsize)

            if isinstance(self.feature_types[col], ContinuousTypedFeature):
                text += PandasDataset._format_stats(self.feature_types[col].type, feature_metadata["stats"])
                ax.text(-1.1, 1, text, transform=ax.transAxes, verticalalignment='top', bbox=props)
                self.sampled_df[col].plot(kind='box', vert=False, flierprops=red_square, ax=ax, figsize=figsize)
                ...
                self.sampled_df[col].hist(bins=50, grid=False, xrot=45, ax=ax, rwidth=0.95, color=blues[-1],
                                          figsize=figsize)

            elif isinstance(self.feature_types[col], GISTypedFeature):
                text += PandasDataset._format_stats(self.feature_types[col].type, feature_metadata["stats"])
                ax.text(-1.1, 1, text, transform=ax.transAxes, verticalalignment='top', bbox=props)
                df = pd.DataFrame(feature_metadata['internal']['sample'], columns=['lat', 'lon'])
                gdf = geopandas.GeoDataFrame(df, geometry=geopandas.points_from_xy(df["lon"], df["lat"]))
                self.world.plot(ax=ax, color='lightgrey', linewidth=0.5, edgecolor='white')
                gdf.plot(ax=ax, color='blue', markersize=10)
                ax.set_aspect('auto')

            elif isinstance(self.feature_types[col], ConstantTypedFeature) and feature_metadata['missing_percentage'] < 100:
                text += PandasDataset._format_stats(self.feature_types[col].type, feature_metadata["stats"])
                ax.text(-1.1, 1, text, transform=ax.transAxes, verticalalignment='top', bbox=props)
                pd.Series(feature_metadata['internal']['counts'], name=col,
                          index=feature_metadata['internal']['counts'].keys()).plot(
                    kind='barh', ax=ax, width=0.95, figsize=figsize, color=['black'])

            elif isinstance(self.feature_types[col], CreditCardTypedFeature):
                text += PandasDataset._format_stats(self.feature_types[col].type, feature_metadata["stats"])
                ax.text(-1.1, 1, text, transform=ax.transAxes, verticalalignment='top', bbox=props)
                sorted_by_value = sorted(feature_metadata['internal']['counts'],
                                         key=feature_metadata['internal']['counts'].get, reverse=True)
                pd.Series(feature_metadata['internal']['counts'], name=col, index=sorted_by_value).plot(
                    kind='bar', ax=ax, width=0.95, figsize=figsize, color=blues)

            elif isinstance(self.feature_types[col], ZipcodeTypedFeature):
                ax.text(-1.1, 1, text, transform=ax.transAxes, verticalalignment='top', bbox=props)
                pd.Series(feature_metadata['internal']['histogram'], name=col,
                          index=feature_metadata['internal']['histogram'].keys()).plot(
                    kind='bar', ax=ax, figsize=figsize, color=blues)

            elif isinstance(self.feature_types[col], PhoneNumberTypedFeature):
                text += PandasDataset._format_stats(self.feature_types[col].type, feature_metadata["stats"])
                ax.text(-1.1, 1, text, transform=ax.transAxes,
                ...
                very_high_cardinality = feature_metadata['internal']['very_high_cardinality']

                ax.text(-1.1, 1, text, transform=ax.transAxes, verticalalignment='top', bbox=props)

                if very_high_cardinality:
                    addrtext = 'Samples:\n\n'
                    addrtext += ', '.join(feature_metadata['internal']['counts'].keys().astype(str).to_list()[:6])

                    ax.text(0.05, 0.95, addrtext, transform=ax.transAxes, fontsize=10, verticalalignment='top',
                            bbox=props)
                    ax.axis('off')
                    ...
                    if feature_metadata['internal']['unique'] < 24:
                        pd.Series(feature_metadata['internal']['counts'], name=col,
                                  index=feature_metadata['internal']['counts'].keys()).plot(
                            kind='bar', ax=ax, width=0.90, color=blues[-1], figsize=figsize)
                    else:
                        self.sampled_df[col].plot(kind='hist', grid=False, rwidth=0.95, ax=ax, color=blues[-1],
                                                  figsize=figsize)
                ...
                if very_high_cardinality:
                    ...
                    addrtext = 'Samples:\n\n'
                    addrtext += '\n'.join(
                        [utils.ellipsis_strings(x, 65) for x in
                         feature_metadata['internal']['counts'].keys().astype(str).to_list()[:3]])
                    ax.text(0.05, 0.95, addrtext, transform=ax.transAxes, fontsize=10, verticalalignment='top',
                            bbox=props)
                    ax.axis('off')
                    ...
                    if feature_metadata['internal']['unique'] == 2:
                        ...
                        # binary
                        #
                        count_series = pd.Series(feature_metadata['internal']['counts'], name=col,
                                                 index=feature_metadata['internal']['counts'].keys()).astype(float)
                        ax1 = count_series.plot(kind='barh', ax=ax, width=0.95, figsize=figsize, color=[blues[0],
                                                                                                        blues[-1]])
                        ...
                        ax1 = pd.Series(feature_metadata['internal']['counts'], name=col,
                                        index=feature_metadata['internal']['counts'].keys()).plot(
                            kind='bar', ax=ax, width=0.95, color=blues,
                            figsize=figsize)

                        x_labels = utils.ellipsis_strings(feature_metadata['internal']['counts'].keys())
                        ax1.set_xticklabels(x_labels)

            elif isinstance(self.feature_types[col], DocumentTypedFeature) or isinstance(self.feature_types[col],
                                                                                         AddressTypedFeature):
                text += PandasDataset._format_stats(self.feature_types[col].type,
                ...
                if 'word_frequencies' in feature_metadata['internal']:
                    ...
                    stats = "\n".join([
                        '  - word count: %d' % (len(word_freqs.keys()))
                    ])
                    text = text + "\n"
                    text += stats
                    wordcloud = WordCloud(width=1000, height=int(1000 * (figsize[1] / figsize[0])),
                ...
                ax.text(-1.1, 1, text, transform=ax.transAxes, verticalalignment='top', bbox=props, weight='bold')
                ...
                ax.text(0.05, 0.95, addrtext, transform=ax.transAxes, fontsize=14, verticalalignment='top', bbox=props,
                        weight='bold')
                ax.axis('off')
    ...
    def _format_stats(feature_type_name, stats):
        ...
        for k in six.iterkeys(stats):
            if 'percentage' in k:
                text += "\n    - {}: {:.3f}%".format(k, stats[k])
            elif isinstance(stats[k], (int, np.int64)) or (isinstance(stats[k], float) and not np.isnan(stats[k])
