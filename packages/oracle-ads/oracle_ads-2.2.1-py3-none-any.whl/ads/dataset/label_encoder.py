...
class DataFrameLabelEncoder(TransformerMixin):
    ...
    def fit(self, X):
        ...
        for column in X.columns:
            if X[column].dtype.name in ["object", "category"]:
                ...
                label_encoder_classes_ = [str(class_) for class_ in self.label_encoders[column].classes_.tolist()]
    ...
    def transform(self, X):
        ...
        def _label_encode_with_unknown(name, series):
            return self.label_encoders[name].transform(series.map(
                lambda x: str(x)).map(
                lambda x: x if x in self.label_encoders[name].classes_ else "unknown"))
