...
class GitHubDatasets(DatasetBrowser):
    ...
    def __init__(self, user: str, repo: str, branch: str):
        ...
        self.repo = repo
        self.branch = branch
        self.api_url = f'https://api.github.com/repos/{user}/{repo}/git/trees/{branch}?recursive=1'
    ...
    def _generate_filelist(self):
        ...
        if response.status_code == 200:
            ...
            raise ValueError(f"Error accessing GitHub API ({self.api_url}): {response.status_code}")

    def list(self, filter_pattern: str = '.*') -> List[str]:
        return super().filter_list([x['name'] for x in self._generate_filelist()], filter_pattern)
    ...
    def open(self, name: str, **kwargs):
        ...
        for obj in self._generate_filelist():
            if obj['name'] == name:
                return DatasetFactory.open(**inject_and_copy_kwargs(kwargs, **{
                        'source': obj['url'],
                        'format': obj['format'],
                        'name': obj['name'],
...
class LocalFilesystemDatasets(DatasetBrowser):
    ...
    def _generate_filelist(self):
        if not isdir(self.folder):
            raise ValueError(f'The path "{self.folder}" does not exist, or is not a folder')
        else:
...
class WebDatasets(DatasetBrowser):
    ...
    def __init__(self, index_url: str):  # pragma: no cover
        ...
        try:
            _, raw_listing = htmllistparse.fetch_listing(
        ...
        for f in [x for x in raw_listing if x.size]:
            ...
            if format.lower() in ['json', 'csv', 'tsv', 'hdf']:
                d = {'url': urllib.parse.urljoin(self.index_url, f.name),
                     'size': f.size,
                     'name': filename,
                     'format': format,
                     'description': f"""
    ...
    def open(self, name: str, **kwargs):
        ...
        for obj in self.listing:
            if obj['name'] == name:
                return DatasetFactory.open(obj['url'], format=obj['format'], name=obj['name'], description=obj['description'])
        raise ValueError(f"dataset [{name}] does not exist, use .list() to display all")
...
class SeabornDatasets(DatasetBrowser):
    ...
    def open(self, name: str, **kwargs):
        if name in self.dataset_names:
            return DatasetFactory.open(sns.load_dataset(name), name=name, description='from seaborn')
        else:
            raise ValueError("dataset [{name}] does not exist, use .list() to display all")


class SklearnDatasets(DatasetBrowser):

    sklearn_datasets = ['boston', 'breast_cancer', 'diabetes',  'iris',  'wine', 'digits']
    ...
    def open(self, name: str, **kwargs):
        if name in SklearnDatasets.sklearn_datasets:
            ...
            if 'images' in data:
                ...
                df = pd.DataFrame(cols, columns=['f%d' % (i) for i in range(cols.shape[1])])
                ...
                df['target'] = pd.Series([data.target_names[x] for x in data.target]).astype('category')
            else:
                df = pd.DataFrame(data.data, columns=data.feature_names)
                df['target'] = pd.Series(data.target)

            return DatasetFactory.open(df, target='target', name=name, description=description)
