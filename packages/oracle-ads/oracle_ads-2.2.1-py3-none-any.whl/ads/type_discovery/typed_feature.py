...
class ContinuousTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        stats = {
            ...
            "outlier_percentage": 100 * np.count_nonzero(np.abs(sp.stats.zscore(non_null_series) >= 2.999)) / series.size
        }
        stats.update({k: v for k, v in desc.items()})

        return ContinuousTypedFeature(name, {
            "type": "continuous",
...
class OrdinalTypedFeature(DiscreteTypedFeature):
    ...
    def build(name, series):
        ...
        return OrdinalTypedFeature(name, {
            "type": "ordinal",
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "low_level_type": series.dtype.name,
            "stats": stats,
            "internal": {
                "sample": series.head(5),
                "unique": desc['unique'],
                "counts": utils.truncate_series_top_n(value_counts, n=min(16, len(value_counts))),
                "high_cardinality": bool(desc['unique'] > 30),
                "very_high_cardinality": bool(desc['unique'] >= 0.95 * desc['count'])
            }
        })
...
class CategoricalTypedFeature(DiscreteTypedFeature):
    ...
    def build(name, series):
        ...
        return CategoricalTypedFeature(name, {
            "type": "categorical",
            "low_level_type": series.dtype.name,
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "stats": stats,
...
class IPAddressTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        return IPAddressTypedFeature(name, {
            "type": "ipaddress",
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "low_level_type": series.dtype.name,
            "stats": {
...
class PhoneNumberTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        area_codes = [g.group(2) for g in filter(None, [pat.match(x) for x in nulls_removed]) if
                      g and len(g.groups()) >= 2]
        value_counts = pd.Series(area_codes).value_counts()

        # resolve area codes
        return PhoneNumberTypedFeature(name, {
            "type": "phone",
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "low_level_type": series.dtype.name,
            "stats": {
...
class AddressTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        return AddressTypedFeature(name, {
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "type": "address",
            "low_level_type": series.dtype.name,
            "stats": {
            ...
            "internal": {
                "sample": series.sample(n=min(100, series.size)),
                "word_frequencies": DocumentTypedFeature.vectorization(name, sampled_nulls_removed,
...
class DocumentTypedFeature(TypedFeature):
    ...
    def corpus_processor(series):
        ...
        for doc in series:
            ...
            yield " ".join([x for x in pat_splitter.split(text) if len(x) >= 3 and x not in STOPWORDS])

    @staticmethod
    def sub_vectorization(feature_name, series, min_df=0.0, max_df=1.0, min_tf=2, ):
        start = time()

        v1 = CountVectorizer(ngram_range=(1, 1), max_features=2000, decode_error='ignore', strip_accents='unicode',
                             stop_words=None, min_df=min_df, max_df=max_df)
        X1 = v1.fit_transform(DocumentTypedFeature.corpus_processor(series))
        unigrams = {k: int(v) for k, v in dict(zip(v1.get_feature_names(), np.asarray(X1.sum(axis=0)).ravel())).items()}

        v2 = CountVectorizer(ngram_range=(2, 2), max_features=2000, decode_error='ignore', strip_accents='unicode',
                             stop_words=None, min_df=min_df, max_df=max_df)
        X2 = v2.fit_transform(DocumentTypedFeature.corpus_processor(series))
        bigrams = {k: int(v) for k, v in dict(zip(v2.get_feature_names(), np.asarray(X2.sum(axis=0)).ravel())).items()}
    ...
    def vectorization(feature_name, series, mean_document_length):
        ...
        if mean_document_length > 5000:
            ...
            max_df = 0.7
        try:
            return DocumentTypedFeature.sub_vectorization(feature_name, series, min_df=min_df, max_df=max_df)
        except ValueError:
            return DocumentTypedFeature.sub_vectorization(feature_name, series, min_df=0.0, max_df=1.0)
    ...
    def build(name, series, is_cjk, is_html):
        ...
        if is_cjk:
            ...
            nulls_removed = series.loc[~series.isnull()]
            if is_html:
                nulls_removed = nulls_removed.replace(to_replace='<[^<]+?>', value=' ', regex=True).replace('\n',' ', regex=True)

            internal['sample'] = nulls_removed.sample(n=min(100, nulls_removed.size))

            mean_document_length = nulls_removed.str.len().mean()
            sample_size = int(2e6 // mean_document_length)  # vectorize at most 2MB of text

            sampled_nulls_removed = nulls_removed.sample(n=min(len(nulls_removed), sample_size))
            ...
            internal["word_frequencies"] = DocumentTypedFeature.vectorization(name, sampled_nulls_removed, mean_document_length)

        return DocumentTypedFeature(name, {
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "type": "document",
            "low_level_type": series.dtype.name,
            "stats": stats,
...
class ZipcodeTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        return ZipcodeTypedFeature(name, {
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "type": "zipcode",
            "low_level_type": series.dtype.name,
            "stats": {
...
class UnknownTypedFeature(TypedFeature):
    ...
    @staticmethod
    def build(name, series):
        return UnknownTypedFeature(name, {
            "type": "unknown",
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "low_level_type": series.dtype.name,
            "stats": {
...
class ConstantTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        return ConstantTypedFeature(name, {
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "low_level_type": series.dtype.name,
            "type": "constant",
            "stats": {
...
class CreditCardTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        sampled_series = null_removed_series.sample(n=min(1000, len(null_removed_series)))
        ...
        return CreditCardTypedFeature(name, {
            "type": "creditcard",
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "low_level_type": series.dtype.name,
            "stats": {
...
class DateTimeTypedFeature(TypedFeature):
    ...
    def build(name, series):
        ...
        return DateTimeTypedFeature(name, {
            "type": "datetime",
            "missing_percentage": 100 * series.isna().sum() / series.size,
            "low_level_type": series.dtype.name,
            "stats": {
