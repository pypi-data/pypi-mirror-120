    ...
    def __init__(self, secret_id: str = None,
        ...
        if secret_id:
            ...
            if utils.is_resource_principal_mode():
                ...
                self.secrets_client = SecretsClient(from_file(oci_config_location, oci_config_profile))
                try:
                    ...
                    if se.status == 404:
                        ...
                        raise
            secret_content = ast.literal_eval(Vault._secret_to_dict(secret_bundle.data.secret_bundle_content.content))
            # declaring priority order
        ...
        if "database_type" not in self.config:
            raise ValueError(f"The database_type needs to be specified. "
                             f"Valid database types are {valid_database_types}")

        if self.config['database_type'] not in valid_database_types:
            raise ValueError(f"{self.config['database_type']} is not a valid database type. "
                             f"Valid database types are {valid_database_types}")
...
def update_repository(value: dict,
                      key: str,
                      replace: bool = True,
...
def import_wallet(wallet_path: str, key: str, repository_path: str = None) -> None:
    ...
    with ZipFile(wallet_path, 'r') as zipObj:
        ...
        zipObj.extractall(os.path.join(os.path.expanduser("~"), f"{repository_path}/{key}"))
...
def _update_sqlnet(db_path):
    ...
    with open(sqlnet_path, 'rt') as sqlnet:
        for line in sqlnet:
            tmp.write(bytearray(sqlnet_re.subn(r'\1{}\3'.format(db_path), line)[0],
                                encoding='utf-8'))
    tmp.flush()
...
def _get_repository_path(repository_path):
    return os.path.join(os.path.expanduser("~"), ".database") if repository_path is None else repository_path
...
def _create_connection_str(config):
    return 'oracle+cx_oracle://' + config["username"] + ':' + config["password"] + '@' + config["database_name"]
