...
from ads.type_discovery.typed_feature import OrdinalTypedFeature, CategoricalTypedFeature
...
class DiscreteDetector(DiscreteDiscoveryDetector):
    ...
    def _get_categorical_or_ordinal(self, name, series):
        ...
        if low_level_type_name == 'category' or low_level_type_name == 'bool':
            ...
            if nulls_removed.dtype.name == 'category' or nulls_removed.dtype.name == 'bool':
                return 'categorical'

            count_distinct = series.nunique()
            observations = series.size

            tot = type_of_target(list(nulls_removed.head(min(nulls_removed.size, 2000))))
            ...
            if tot == 'binary':
                ...
                if low_level_type_name.startswith('int') or low_level_type_name.startswith('float'):
                    if nulls_removed.min() >= 0:
                        if low_level_type_name.startswith('int') or nulls_removed.sum() == nulls_removed.astype('int64').sum():
                            return 'ordinal'
    ...
    def discover(self, name, series):
        ...
        guessed_type = self._get_categorical_or_ordinal(name, series.loc[~series.isnull()])
...
if __name__ == '__main__':
    ...
    print(dd.discover("str-categorical", pd.Series(["a", "a", "a", "b", "c", "a"], dtype="category")))
    print(dd.discover("bool-categorical", pd.Series([True, False, True, True, True, None, True])))
    print(dd.discover("continuous", pd.Series([None, 3.14, 12.0, 1, 2, 3, None])))
    print(dd.discover("int-1-categorical", pd.Series([1, 1, 1, 5, 5, 5, 5, 5, 5, 5, 9])))
