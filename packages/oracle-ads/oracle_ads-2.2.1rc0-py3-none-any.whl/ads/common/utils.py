...
ml_task_types = Enum('ml_task_types',
...
def get_oci_config():
    ...
    """
    oci_config_location = os.environ.get('OCI_CONFIG_LOCATION', f"{os.environ['HOME']}/.oci/config")  # os.environ['HOME'] == home/datascience
...
def oci_key_location():
    ...
    """
    return os.environ.get("OCI_CONFIG_DIR", os.path.join(os.path.expanduser('~'),'.oci'))
...
def print_user_message(msg, display_type='tip', see_also_links=None, title="Tip"):  # pragma: no cover
    """
    ...
    if display_type.lower() == 'module':
        ...
        print(f"ERROR: {module_name} module not found. Make sure you have installed {module_pkg} in order to download all of necessary modules.")
    ...
    if is_documentation_mode() and is_notebook():
        ...
        if display_type.lower() == 'tip':
            ...
            if '\n' in msg:
                ...
                user_message = "{}{}".format(t,
                                             ''.join(["<br>&nbsp;&nbsp;+&nbsp;{}".format(x.strip()) for x in
                                                      msg.strip().split('\n')])
                                             )
            else:
...
def get_progress_bar(max_progress, description="Initializing"):
    ...
    if is_notebook():  # pragma: no cover
        return TqdmProgressBar(max_progress, description=description, verbose=is_debug_mode())
    else:
...
def split_data(X, y, random_state=random_state, test_size=test_size):
    ...
    train_test_split = dask_train_test_split if isinstance(X, dd.core.DataFrame) else sklearn_train_test_split
    return train_test_split(X, y, train_size=1 - test_size, test_size=test_size, random_state=random_state)
...
def get_sqlalchemy_engine(connection_url, *args, **kwargs):
    ...
    if connection_url not in _engines:
        ...
        _engines[connection_url] = create_engine(connection_url, pool_recycle=10, *args, **kwargs)
...
def inject_and_copy_kwargs(kwargs, **args):
    ...
    example usage:
        replace: foo(arg1, arg2, **kwargs)
        with:    foo(arg1, args, utils.inject_and_copy_kwargs(kwargs, arg3=12, arg4=42))
...
def get_base_modules(model):
    ...
    try:
        ...
        pass
    return add_bases + list(type(model).__mro__) if hasattr(type(model), "__mro__") else add_bases
...
def extract_lib_dependencies_from_model(model) -> dict:
    ...
    module_versions = {}
    modules_to_include = set(mod.__module__.split('.')[0] for mod in get_base_modules(model)
...
def generate_requirement_file(requirements: dict, file_path: str, file_name: str = 'requirements.txt'):
    """
...
def generate_schema(X_sample=None, y_sample=None, **model_kwargs):
    """
    Generate a schema.

    Parameters
    ----------
    X_sample: Pandas or Dask DataFrame
        A sample of X data

    y_sample: Pandas or Dask Series
        A sample of y data

    model_kwargs: Variable
        Kwargs associated to the model
    ...
    data_sample: Dict
        A sample of the data in the Schema
    """
    schema = {}
    data_sample = {}
    if X_sample is not None:
        assert (isinstance(X_sample, pd.DataFrame) or
                isinstance(X_sample, dd.core.DataFrame)), "X Sample must be a Pandas or Dask dataframe"
        if isinstance(X_sample, dd.core.DataFrame):
            X_sample_df = X_sample.compute()
        else:
            X_sample_df = X_sample
        data_sample["input"] = np.asarray(X_sample_df.values)

        # build schema
        features = {}
        for column in X_sample_df.columns.values:
            feature_type, dtype = _get_feature_type_and_dtype(X_sample_df[column])
            features[str(column)] = {"type": feature_type, "dtype": dtype}
        schema["predict_arguments"] = features
        if y_sample is None:
            raise ValueError("Please specify y_sample corresponding to the given X_sample")
    if y_sample is not None:
        assert (isinstance(y_sample, pd.Series) or
                isinstance(y_sample, dd.core.Series)), "y Sample must be a Pandas or Dask Series"
        data_sample["output"] = np.asarray(y_sample.values)
        feature_type, dtype = _get_feature_type_and_dtype(y_sample)
        schema["predict_response"] = {y_sample.name: {"type": feature_type, "dtype": dtype}}
    if len(data_sample) > 0:
        if model_kwargs.get('is_classifier', False):
            if len(model_kwargs.get('classes_', 0)) == 2:
                schema["problem_type"] = "binary_classification"
            else:
                schema["problem_type"] = "multiclass_classification"
        else:
            schema["problem_type"] = "regression"
    return schema, data_sample
