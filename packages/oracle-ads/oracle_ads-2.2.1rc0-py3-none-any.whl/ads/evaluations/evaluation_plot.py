...
import numpy as np
import math
from ads.common import utils, model
import itertools
...
class EvaluationPlot:
    ...
    font_sz = {"xl": 16,  # Plot type title
               "l": 14,  # Individual plot title (name of model)
               "m": 12,  # Axis titles
               "s": 10,  # Axis labels
               "xs": 8}  # test within the plot
    ...
    _pretty_titles_map = {"normalized_confusion_matrix": "Normalized Confusion Matrix",
    ...
    _bin_plots = ["pr_curve", "roc_curve", "lift_chart", "gain_chart", "normalized_confusion_matrix"]
    _multi_plots = ["normalized_confusion_matrix", "roc_curve", "pr_curve", "precision_by_label", "recall_by_label",
                    "f1_by_label", "jaccard_by_label"]
    _reg_plots = ["observed_vs_predicted", "residuals_qq", "residuals_vs_predicted", "residuals_vs_observed"]
    ...
    def get_legend_labels(cls, legend_labels):
        ...
        def render_legend_labels(label_dict):
            encodings = pd.DataFrame(pd.Series(label_dict, index=label_dict.keys()), columns=["Shortened labels"])
            display(HTML(encodings.style.set_precision(4)
                         .set_properties(**{'text-align': 'center'})
                         .set_table_styles([dict(props=[("text-align", "center")])])
                         .set_table_attributes('class=table')
                         .set_caption('<div align="left"><b style="font-size:20px;">' +
        ...
        if legend_labels is not None:
            ...
            if isinstance(legend_labels, dict) and set(cls.classes).issubset(set(legend_labels.keys())):
                render_legend_labels(legend_labels)
                cls.legend_labels = legend_labels
            utils.print_user_message("The `legend_labels` you provided is either not a dict, or does not possess every"
                                     "class label", display_type="error", title='error')
            return
        ...
        def _check_for_redundant_words(label_vec, prefix, max_len=MAX_LEGEND_LEN):
            ...
            while len(set([lab.split()[0] for lab in classes])) <= 1:
                ...
                prefix = prefix[:-3] + additional_prefix + " ..." if words_found else additional_prefix + " ..."
                classes = [lab[len(additional_prefix) + 1:] for lab in classes]
                words_found = True

            classes = classes if words_found else [label[max_len:] for label in label_vec]
        ...
        if set(cls.legend_labels.keys()) != set(cls.legend_labels.values()):
            utils.print_user_message("We have found class labels greater than 13 characters long, we have "
                                     "auto-shortened them on the graphs to improve rendering. If you would like "
                                     "change the auto-generated labels, either pass in a dict mapping every class "
                                     "label as `legend_labels`, or rename the class labels in the dataset and "
                                     "re-build the Evaluator object.", display_type="Tip", title='Note')
            render_legend_labels(cls.legend_labels)

    # evaluation is a DataFrame with models as columns and metrics as rows
    @classmethod
    def plot(cls, evaluation, plots, num_classes, perfect=False, baseline=True, legend_labels=None):
        ...
        if plots is None:
            ...
            utils.print_user_message(
                "\nShowing plot types: {}\nTo select specific type of plot use <code>show_in_notebook([x, y, "
                "z])</code> with {}".format(
                    ', '.join(["<b>{}</b>".format(EvaluationPlot._pretty_titles_map[str(p)]) for p in plots]),
                    ', '.join(["<b>{}</b>".format(x) for x in map(str, plots)])))
            utils.print_user_message(plot_details, display_type="info", title='Note')

        else:
            utils.print_user_message("Showing only: {}".format(', '.join(["<code>{}</code>".format(p) for p in plots])),
                                     display_type='info', title=None)
        ...
        if cls.prob_type == '_bin':
            ...
            if "normalized_confusion_matrix" in plots and len(evaluation[evaluation.columns[0]]['classes']) >= 10:
                utils.print_user_message(
                    "Evaluation plotting is not yet supported for Multiclass problems with 10 or more classes.",
                    display_type="error", title='Apologies')
                plots = []
        ...
        for i, plot_type in enumerate(plots):
            ...
            try:
                fig_title, ax_title = plt.subplots(1, 1, figsize=(18, .5), dpi=144)
                ax_title.text(0.5, .5, cls._pretty_titles_map[plot_type], fontsize=16, fontweight='semibold',
                ...
                if cls.prob_type == '_bin' and plot_type in cls.double_overlay_plots:
                    ...
                    axs[1].axis('off')
                    ax = [axs[0]]
                elif cls.prob_type == '_bin' and plot_type in ["lift_chart", "gain_chart"]:
                    fig, axs = plt.subplots(1, 2, figsize=(12, 4.5), dpi=144)
                    axs[1].axis('off')
                    ax = axs[0]
                else:
                    nrows = math.ceil(len(evaluation.columns) / MAX_PLOTS_PER_ROW)
                    fig, ax = plt.subplots(nrows, MAX_PLOTS_PER_ROW, figsize=(10, 4 * nrows), dpi=144)  # 10, 3.5
                    ax = ax.flatten()
                ...
                try:
                    ...
                    pass
                utils.print_user_message(f"WARNING: Evaluator was not able to make the plot "
                                         f"{cls._pretty_titles_map.get(plot_type, plot_type)}, because the relevant "
                                         f"metrics had complications. Please check that predict and predict_proba are "
                                         f"performing as expected for the target data.")
    ...
    def _lift_chart(cls, ax, evaluation):
        for mod_name, col in evaluation.iteritems():
            if col['y_score'] is not None:
                ax.plot(col['percentages'][1:], [1] + list(col['lift']), label=cls._get_formatted_title(mod_name))
        if cls.baseline:
            ax.plot([-10, 110], [1, 1], **cls.baseline_kwargs)
        if cls.perfect:
            perf_idx = next(idx for idx, scores in enumerate(evaluation.loc['y_score']) if scores is not None)
            ax.plot(evaluation.loc['percentages'][perf_idx][1:], [1] + list(evaluation.loc['perfect_lift'][perf_idx]),
                    **cls.perfect_kwargs)
    ...
    def _gain_chart(cls, ax, evaluation):
        for mod_name, col in evaluation.iteritems():
            if col['y_score'] is not None:
                ax.plot(col['percentages'], list(col['cumulative_gain']), label=cls._get_formatted_title(mod_name))
        if cls.baseline:
            ax.plot([-10, 110], [-10, 110], **cls.baseline_kwargs)
        if cls.perfect:
            perf_idx = next(idx for idx, scores in enumerate(evaluation.loc['y_score']) if scores is not None)
            ax.plot(evaluation.loc['percentages'][perf_idx], evaluation.loc['perfect_gain'][perf_idx],
                    **cls.perfect_kwargs)
    ...
    def _pr_curve(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            ...
            if cls.prob_type == '_bin':
                for mod_name, col in evaluation.iteritems():
                    if col['y_score'] is not None:
                        ax.plot(col['recall_values'], col['precision_values'],
                                label="%s (Precision: %s)" % (cls._get_formatted_title(mod_name),
                                                              "{:.3f}".format(col['precision'])))
                        ax.plot(*col['pr_best_model_score'], color=ax.get_lines()[-1].get_color(), marker='*')
            else:
                ...
                if mod['y_score'] is not None:
                    for j, lab in enumerate(mod.classes):
                        ...
                        lab = str(lab)
                        ax.plot(mod['recall_values'][j], mod['precision_values'][j],
                                label="%s (Precision: %s)" % (cls.legend_labels[lab],
            ...
            star = mlines.Line2D([], [], color='black', marker='*', linestyle='None', markersize=5,
                                 label='Minimum Error Rate')
            handles.append(star)
            labels.append("Minimum Error Rate")
            ax.legend(loc='upper right', labels=labels, handles=handles, frameon=False, fontsize='x-small')
    ...
    def _roc_curve(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            ...
            if cls.prob_type == '_bin':
                for mod_name, col in evaluation.iteritems():
                    if col['y_score'] is not None:
                        ax.plot(col['false_positive_rate'], col['true_positive_rate'],
                                label="%s (AUC: %s)" % (
                                    cls._get_formatted_title(mod_name), "{:.3f}".format(col['auc'])))
                        ax.plot(*col['roc_best_model_score'], color=ax.get_lines()[-1].get_color(), marker='*')
            else:
                model_name = evaluation.columns[i]
                mod = evaluation[model_name]
                if mod['y_score'] is not None:
                    for j, lab in enumerate(mod.classes):
                        # cls.legend_labels contains only strings as keys.
                        lab = str(lab)
                        ax.plot(mod['fpr_by_label'][j], mod['tpr_by_label'][j],
                                label="%s (AUC: %s)" % (cls.legend_labels[lab], "{:.3f}".format(mod['auc'][j])))
                        ax.plot(*mod['roc_best_model_score'][j], color=ax.get_lines()[-1].get_color(), marker='*')
            if cls.baseline:
                ax.plot([-.1, 1.1], [-.1, 1.1], **cls.baseline_kwargs)
            ...
            ax.legend(loc='lower right', labels=labels, handles=handles, frameon=False, fontsize='x-small')
    ...
    def _ks_statistics(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            ...
            if mod['y_score'] is not None:
                ax.plot(mod['ks_thresholds'], mod['ks_pct1'], lw=3, label=mod['ks_labels'][0])
                ax.plot(mod['ks_thresholds'], mod['ks_pct2'], lw=3, label=mod['ks_labels'][1])
                if cls.baseline:
                    idx = np.where(mod['ks_thresholds'] == mod['max_distance_at'])[0][0]
                    ax.axvline(mod['max_distance_at'], *sorted([mod['ks_pct1'][idx], mod['ks_pct2'][idx]]),
                               label='KS Statistic: {:.3f} at {:.3f}'.format(mod['ks_statistic'],
                                                                             mod['max_distance_at']),
                               linestyle='--', color='.2')
    ...
    @classmethod
    def _pretty_barh(cls, ax, x, y, axis_labels=None, title=None, axis_lim=None, plot_kwargs=None):
        # cls.legend_labels contains only strings as keys.
        new_lab = [cls.legend_labels[str(item)] for item in x]
        ax.barh(new_lab, y, color=['teal', 'blueviolet', 'forestgreen', 'peru', 'y', 'dodgerblue', 'r'])
    ...
    def _precision_by_label(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            if i < n_models:
                ...
                cls._pretty_barh(ax, evaluation[col]['classes'], evaluation[col]['precision_by_label'], axis_lim=[0, 1],
                                 axis_labels=['Precision', None], title=col)
            else:
                ax.axis('off')
    ...
    def _recall_by_label(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            if i < n_models:
                col = evaluation.columns[i]
                cls._pretty_barh(ax, evaluation[col]['classes'], evaluation[col]['recall_by_label'], axis_lim=[0, 1],
                                 axis_labels=['Recall', None], title=col)
            else:
                ax.axis('off')
    ...
    def _f1_by_label(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            if i < n_models:
                col = evaluation.columns[i]
                cls._pretty_barh(ax, evaluation[col]['classes'], evaluation[col]['f1_by_label'], axis_lim=[0, 1],
                                 axis_labels=['F1 Score', None], title=col)
            else:
                ax.axis('off')
    ...
    def _jaccard_by_label(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            if i < n_models:
                col = evaluation.columns[i]
                cls._pretty_barh(ax, evaluation[col]['classes'], evaluation[col]['jaccard_by_label'], axis_lim=[0, 1],
                                 axis_labels=['Jaccard Score', None], title=col)
            else:
                ax.axis('off')

    @classmethod
    def _pretty_scatter(cls, ax, x, y, s=5, alpha=1.0, title=None, legend=False, axis_labels=None, axis_lim=None,
                        grid=True, label=None, plot_kwargs=None):
    ...
    def _residuals_qq(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            ...
            cls._pretty_scatter(ax, mod['norm_quantiles'], mod['residual_quantiles'],
                                title=model_name, axis_lim=[(-2.7, 2.7), (-3.1, 3.1)],
    ...
    def _residuals_vs_predicted(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            ...
            cls._pretty_scatter(ax, y_pred, resid, s=4,
                                alpha=.5, title=model_name, axis_labels=["Predicted Values", "Residuals"])
            x_lim = (y_pred.min() - y_pred.min() * .05, y_pred.max() +
                     y_pred.max() * .05)
            if cls.baseline:
    ...
    def _residuals_vs_observed(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            ...
            cls._pretty_scatter(ax, y_true, mod['residuals'], s=4,
                                alpha=.5, title=model_name, axis_labels=["Observed Values", "Residuals"])
            x_lim = (y_true.min() - y_true.min() * .05, y_true.max() + y_true.max() * .05)
            if cls.baseline:
                ax.plot(x_lim, (0, 0), **cls.baseline_kwargs)
            ax.set_xlim(x_lim)
            ax.xaxis.set_major_formatter(FormatStrFormatter('%.2f'))
    ...
    def _observed_vs_predicted(cls, axs, evaluation):
        ...
        for i, ax in enumerate(axs):
            ...
            model_name = evaluation.columns[i]
            mod = evaluation[model_name]
            ax.scatter(mod['y_true'], mod['y_pred'], s=4, marker="o", alpha=.5)
            ...
            x_lim = (yt_min - yt_min * .05, yt_max + yt_max * .05)
            if cls.baseline:
    ...
    def _normalized_confusion_matrix(cls, axs, evaluation):
        for model_num, ax in enumerate(axs):
            ...
            for i, j in itertools.product(range(raw_cm.shape[0]), range(raw_cm.shape[1])):
                ax.text(j, i, "%s [%s]" % (round(cm[i][j], 3), raw_cm[i, j]),
