...
from ads.dataset.feature_engineering_transformer import \
...
class ADSDatasetWithTarget(ADSDataset):
    ...
    def __init__(self, ddf, sampled_df, target, target_type, shape, sample_max_rows=-1, type_discovery=True, types={},
                 parent=None, name="", metadata=None, transformer_pipeline=None, description=None,
                 progress=DummyProgressBar(), **kwargs):
        self.recommendation_transformer = None

        if parent is None:
            cols = sampled_df.columns.tolist()
            cols.insert(0, cols.pop(cols.index(target)))
            ADSDataset.__init__(self, ddf, sampled_df[[*cols]], shape, name=name, description=description,
                                type_discovery=type_discovery, types=types, progress=progress, metadata=metadata,
                                transformer_pipeline=transformer_pipeline, sample_max_rows=sample_max_rows)
        else:
            ...
            if parent.correlation is None:
                ...
                corr_cols = parent.sampled_df.select_dtypes(exclude=['object']).columns.values.tolist()
            ...
            if len(parent.feature_dist_html_dict) > 0:
                ...
                self.feature_dist_html_dict = {target: parent_feature_dist_html_dict.pop(target)}
        ...
        if self.ddf[target].isna().sum().compute():
            ...
            #
            self.sampled_df = helper.generate_sample(self.ddf,
                                                     sampled_df.shape[0],
        ...
        for step in self.transformer_pipeline.steps:
            if step[0] == "type_discovery" and self.target.name in step[1].kw_args["dtypes"]:
    ...
    def select_best_features(self, score_func=None, k=12):
        ...
        return self._build_new_dataset(tf[1].transform(self.ddf),
                                       tf[1].transform(self.sampled_df),
                                       transformers=tf)

    def auto_transform(self, correlation_threshold: float = 0.7, frac: float = 1.0, sample_size = 1.0,
        ...
        with utils.get_progress_bar(5) as progress:
            ddf, sampled_df, transformer_pipeline = self._transform(progress=progress,
                                                                    correlation_threshold=correlation_threshold,
                                                                    frac=sample_size,
                                                                    correlation_methods=correlation_methods)
            return self._build_new_dataset(ddf, sampled_df=sampled_df, transformers=transformer_pipeline.steps,
                                           progress=progress)
    ...
    def visualize_transforms(self):
        ...
        helper.visualize_transformation(self.transformer_pipeline,
    ...
    def _suggested_code(self, action, recommendation_type, variable):
        ...
        if recommendation_type == 'positive_class' and action != 'Do nothing':
            return '.set_positive_class({}, missing_value=False)'.format('"' + action + '"')
        if recommendation_type == 'imputation':
            fill_val = helper.get_fill_val(self.feature_types, variable, action, constant='constant')

            fill_val = fill_val if isinstance(fill_val, Number) else '"' + fill_val + '"'
            return '.fillna({}{}: {}{})'.format('{', '"' + variable + '"', fill_val, '}')
        else:
            return ''

    def suggest_recommendations(self, correlation_methods: Union[str, list] = 'pearson', print_code: bool = True, 
                                correlation_threshold: float = 0.7, overwrite: bool = None, force_recompute: bool = False,
                                frac: float = 1.0, sample_size:float = 1.0, **kwargs):
        """
        ...
        recommended = self._get_recommendations_transformer(auto_transform=False,
                                                            correlation_threshold=correlation_threshold,
                                                            correlation_methods=correlation_methods,
        ...
        if len(recommended.reco_dict_) == 0:
            utils.print_user_message('Dataset looks good. No recommendations from ADS.', display_type='info', title='Info')
            return pd.DataFrame()

        column_names = ['Message', 'Variables', 'Action', 'Selected Action', 'Recommendation Type']
        ...
        for recommendation_type, column_dict in recommended.reco_dict_.items():
            if recommendation_type == 'constant_column':
                ...
                df_dict['Recommendation Type'].extend([recommendation_type] * n_constant)
            ...
            for column, details_dict in column_dict.items():
                ...
                df_dict['Recommendation Type'].extend([recommendation_type] * max_length)
                df_dict['Variables'].extend([column] * max_length)

        suggestions_df = pd.DataFrame.from_dict(df_dict)[column_names]
        suggestions_df['Code'] = suggestions_df.apply(lambda x:
                                                      self._suggested_code(x['Action'],
                                                                           x['Recommendation Type'],
                                                                           x['Variables']),
                                                      axis=1)
        suggestion_df = suggestions_df \
            .drop(columns=['Recommendation Type']) \
        ...
        if print_code:
            ...
            suggested_code = suggestion_df_.loc[suggestion_df_.Suggested == suggestion_df_.Action].Code.unique()
            for code in suggested_code:
                if '.drop_columns' in code:
                    ...
                    columns_to_impute[impute_pair.split(':')[0].replace("\"", "")] = float(
                        impute_pair.split(':')[1].strip()) if impute_pair.split(':')[1].strip().replace('.',
                                                                                                        '').isdigit() \
                        else impute_pair.split(':')[1].strip().replace("\"", "")
                else:
                    consolidated_code += code
            consolidated_code = 'No more!' if len(consolidated_code) == 0 else consolidated_code
            utils.print_user_message(msg="Suggested columns to drop: {} \n"
                                         "Suggested columns to impute: {} \n"
                                         "Others: {}".format(columns_to_drop, columns_to_impute, consolidated_code))
        return suggestion_df

    def get_recommendations(self, correlation_methods: str = 'pearson', correlation_threshold: float = 0.7, frac: float = 1.0, sample_size:float = 1.0,
                            overwrite: bool = None, force_recompute: bool = False, display_format: str = 'widget'):
        """
        ...
        if display_format == 'widget':
            ...
            if len(recommended.reco_dict) == 0:
                utils.print_user_message('There are no recommendations.', display_type='warning',
                                         title='Warning')
            ...
            display(HTML(
                df_suggestion.to_html().replace(' `', '<code>').replace('` ', '</code>').replace('\\n', '<br>')
            ))
    ...
    def get_transformed_dataset(self):
        ...
        utils.print_user_message(
            "Use <code>get_recommendations()</code> to view/update recommendation or <code>auto_tranform()</code> "
            "first.")
        utils.print_user_message(msg='<code>get_transformed_dataset</code> has been deprecated and will be removed in a future release.', display_type='warning', title='Warning')
        return
    ...
    def train_test_split(self, test_size=0.1, random_state=utils.random_state):
        ...
        """
        X_train, X_test, y_train, y_test = train_test_split(self.ddf.drop(self.target.name, axis=1),
                                                            self.ddf[self.target.name], test_size=test_size,
                                                            train_size=1 - test_size, random_state=random_state)
        train = ADSData.build(X=X_train, y=y_train, name="Train Data", dataset_type=self.__class__)
        train.transformer_pipeline = self.transformer_pipeline
        test = ADSData.build(X=X_test, y=y_test, name="Test Data", dataset_type=self.__class__)
        return train, test

    def train_validation_test_split(self, test_size=0.1, validation_size=0.1, random_state=utils.random_state):
        """
        ...
        """
        train, test = self.train_test_split(test_size=test_size, random_state=random_state)
        X_train, X_valid, y_train, y_valid = train_test_split(train.X, train.y, test_size=validation_size,
                                                              random_state=random_state)
        train.X = X_train
        train.y = y_train
        valid = ADSData.build(X=X_valid, y=y_valid, name="Validation Data", dataset_type=self.__class__)
    ...
    def _transform(self, progress=DummyProgressBar(), fix_imbalance=True, correlation_threshold=0.7, frac=None, correlation_methods='pearson'):
        progress.update("Building the transformer pipeline")
        if self.recommendation_transformer is None:
            transformer_pipeline = TransformerPipeline(steps=[("recommendations",
                                                               self._get_recommendations_transformer
            ...
            transformer_pipeline = TransformerPipeline(steps=[("feature_engineering",
                                                               FeatureEngineeringTransformer(
                                                                   feature_metadata=self.feature_types))])
            transformer_pipeline.steps = [("recommendations",
                                           self.recommendation_transformer)] + transformer_pipeline.steps
    ...
    def _get_best_features(self, score_func=None, k=12):
        if isinstance(self.target.type, DateTimeTypedFeature):
            return FeatureImportance._get_feature_ranking(self.sampled_df.copy(), self.target.name,
                                                          self.type_of_target(),
                                                          score_func=score_func, k=k)
        else:
            return FeatureImportance._get_feature_ranking(
                self.sampled_df.copy(), self.target.name, self.type_of_target(),
                score_func=score_func, k=k)

    def _get_best_features_transformer(self, score_func=None, k=12):
        feature_set = self._get_best_features(k=k, score_func=score_func)["features"].tolist()
        ...
        return ("select_{0}_best_features".format(k),
                FunctionTransformer(func=_select_features, validate=False,
                                    kw_args={"feature_names": feature_set, "target": self.target.name}).fit(
                    self.sampled_df))

    def _get_recommendations_transformer(self, fix_imbalance=True, auto_transform=True, correlation_threshold=0.7, **kwargs):
        ...
        return RecommendationTransformer(feature_metadata=self.feature_types,
                                         correlation=self.corr(force_recompute=force_recompute, frac=frac, correlation_methods=correlation_methods, **kwargs),
