...
class Node:
    def __init__(self, pool, bot: Union[commands.Bot, discord.Client, commands.AutoShardedBot, discord.AutoShardedClient], host: str, port: int, password: str, identifier: str, **kwargs):
        ...
        self._websocket_uri = f"ws://{self._host}:{self._port}/magma"
        ...
        self._headers = {
            ...
            "Client-Name": f"Obsidian.py/{__version__}"
        }
    ...
    def __repr__(self):
        return f"<Obsidian.node ws_uri={self._websocket_uri} rest_uri={self._rest_uri} player_count={len(self._players)}>"
    ...
    async def _handle_payload(self, op, data: dict) -> None:
    ...
    async def connect(self): 
        ...
        try:
            self._websocket = await self._session.ws_connect(self._websocket_uri, headers=self._headers)
            self.task = self._bot.loop.create_task(self._listen())
    ...
    async def get_tracks(self, query: str):
        ...
        async with self._session.get(url=f"{self._rest_uri}/loadtracks", params=dict(identifier=query),
                                           headers={"Authorization": self._password}) as response:
