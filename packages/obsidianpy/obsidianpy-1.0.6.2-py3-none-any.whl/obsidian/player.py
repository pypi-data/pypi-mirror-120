...
class Player(VoiceProtocol):
    ...
    async def set_volume(self, volume: int):
        # This funky line below tries to convert the volume from an int to a float between 0.0 and 5.0 so that the Obsidian node can interpret it
        # i.e: if the volume is set to anything below 99, the volume will output as "0.<volume>"
        # If its above 100, it'll format to "<first digit of volume>.<second and third digit of volume>"
        # i.e if the volume is set to 125, it'll output as "1.25" and so forth...
        # I know its a bit scuffed, but i didn't implement the way the volume works in Obsidian
        volume = float("0.0" + str(volume)) if volume < 9 else float("0." + str(volume)) if volume < 99 else float(str(volume)[0] + "." + str(volume)[1:])
        if self._filter:
            await self._node.send(op=9, d={'guild_id': str(self._guild.id), 'filters': {**self._filter.payload, 'volume': volume}})
        else:
    ...
    async def set_filter(self, filter: filters.Filter):
        await self._node.send(op=9, d={'guild_id': str(self._guild.id), 'filters': {**filter.payload, 'volume': self.volume}})
        await self.seek(self.position)
        self._filter = filter
        return filter
