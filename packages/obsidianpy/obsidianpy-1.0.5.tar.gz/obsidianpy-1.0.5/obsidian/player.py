...
class Player(VoiceProtocol):
    ...
    async def set_volume(self, volume: int):
        ...
        volume = float("0." + str(volume)) if volume < 99 else float(str(volume)[0] + "." + str(volume)[1:])
        await self._node.send(op=9, d={'guild_id': str(self._guild.id), 'filters': {**self._filters, 'volume': volume}})
    ...
    async def set_filter(self, filter: filters.Filter):
        ...
        await self.seek(self.position)
        self._filters.update({**filter.payload})
        return filter
