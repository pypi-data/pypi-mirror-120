...
class Node(object):
    ...
    def __init__(self, node_id: int, node_name: str, node_icon_id: int, node_content: str, node_in_ports: list, node_out_ports: list):
        ...
        self.in_ports: [InPort] = []
        self.out_ports: [OutPort] = []
...
class BaseDigraph(object):
    ...
    def add_python_node(self, name: str, content: str, in_ports: [], out_ports: []) -> Node:
        """
...
class Digraph(BaseDigraph):
    ...
    5. Generating a local HTML file:

    >>> digraph.generate_html('Test1')

    """
    def __init__(self, digraph_name: str, make_text_center: bool = False):
        super(Digraph, self).__init__()
        self.name: str = digraph_name
        self.make_text_center: str = 'false'
        if make_text_center:
            self.make_text_center = 'true'

        self.fullscreen: Fullscreen = Fullscreen('digraph')
        self.frame_id: str = self.fullscreen.target_frame_id
    ...
    def build(self):
        ...
        self.html = self.template.render(
            ...
            makeTextCenter=self.make_text_center)
...
class MultiDigraph(object):
    ...
    def __init__(self, multi_digraph_name: str, make_text_center: bool = False):
        ...
        self.lock: Lock = Lock()
        self.child_digraphs: [MultiDigraph.ChildDigraph] = []
    ...
    def build(self):
        ...
        if len(self.child_digraphs) == 0:
            raise Exception('No child digraphs were added!')
