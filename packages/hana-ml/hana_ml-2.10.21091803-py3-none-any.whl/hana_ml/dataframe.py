...
class ConnectionContext(object):
    ...
    def create_virtual_table(self, table, data_lake_table, schema=None, data_lake_container='SYSRDL#CG'):
        """
    ...
    def read_onnx(self, onnx_file, table, schema=None):
        """
        ...
        """
        self.create_table(table=table, table_structure={"ID": "INT", "VERSION": "INT", "MODEL": "BLOB"}, schema=schema)
    ...
    def table(self, table, schema=None, save_source=True):
        """
...
class DataFrame(object):#pylint: disable=too-many-public-methods
    ...
    def save(self, where, table_type=None, force=False, save_source=True, append=False, data_lake=False, data_lake_container='SYSRDL#CG'):
        """
    ...
    def collect(self, fetch_size=32000, geometries=True, convert_geo_to_shapely=True):
        ...
        if not self.connection_context.pyodbc_connection:
            try:
                ...
                logger.error("Errors in checking geometries, %s", exc)
                pass
            except pyodbc.Error as exc:
                logger.error("Errors in checking geometries, %s", str(exc.args[1]))
                pass
        # pylint: disable=W0511
        ...
        if not self.connection_context.pyodbc_connection:
            try:
                ...
                logger.error("Errors in Converting geometries to shapely objects, %s", exc)
                pass
            except pyodbc.Error as exc:
                logger.error("Errors in Converting geometries to shapely objects, %s", str(exc.args[1]))
                pass
