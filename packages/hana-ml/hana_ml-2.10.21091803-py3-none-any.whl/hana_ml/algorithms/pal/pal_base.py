...
class PALBase(hana_ml.ml_base.MLBase, ModelSavingServices):
    ...
    def get_fit_execute_statement(self):
        """
        Return the execute_statement for train, predict and score.
        """
    ...
    def _call_pal_auto(self, conn_context, funcname, *args):
        self.execute_statement = call_pal_auto_with_hint(conn_context,
                                                         ...
                                                         *args)
        try:
            call_string = self._get_store_procedure()
            if call_string:
                if "PREDICT" in call_string.upper() or "FORECAST" in call_string.upper() or "CLASSIFY" in call_string.upper():
