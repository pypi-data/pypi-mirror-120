...
def runList(*taskList):
    ...
    try:
        ...
        if b.countLogCritical:
            printcolor.set_cmd_text_color(printcolor.FOREGROUND_WHITE | printcolor.BACKGROUND_DARKPINK)
        elif b.countLogError:
            printcolor.set_cmd_text_color(printcolor.FOREGROUND_WHITE | printcolor.BACKGROUND_DARKRED)
        elif b.countLogWarning:
            printcolor.set_cmd_text_color(printcolor.FOREGROUND_BLACK | printcolor.BACKGROUND_DARKYELLOW)
        else:
            printcolor.set_cmd_text_color(printcolor.FOREGROUND_BLACK | printcolor.BACKGROUND_DARKGREEN)

        msgAry = ["任务结束"]
        if b.countLogCritical:
            msgAry.append(f"critical({b.countLogCritical})")
        if b.countLogError:
            msgAry.append(f"error({b.countLogError})")
        if b.countLogWarning:
            msgAry.append(f"warning({b.countLogWarning})")

        b.info(" ".join(msgAry))
        passTime = str(datetime.datetime.now() - startTime)
        if passTime.startswith("0:"):
            passTime = "0" + passTime
        b.info(f"用时：{passTime}")
        printcolor.resetColor()
        b.remove(lockFile)
        if not isDev():
            while True:
