    ...
    def load_snapshots(self, snapshot_names:List[str]):
        for snapshot_name in snapshot_names:
            if self.workspace.getSnapshot(snapshot_name):
                ...
                _PRINT_.info(f'INFO: Loaded snapshot "{snapshot_name}"')
            else:
                _PRINT_.warning(f'WARNING: Unable to load snapshot "{snapshot_name}"')
    ...
    def generate_toys(self, n_toys:int=1, seed:int=0, binned:bool=True, 
        ...
        for i in range(n_toys):
            ...
            if do_import:
                ...
                _PRINT_.info(f'INFO: Generated toy dataset "{toy_name}"')
            toys.append(toy)
