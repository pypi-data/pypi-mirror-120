...
from blocktorch.automl.engine.cf_engine import CFClient, CFComputation, CFEngine
from blocktorch.automl.engine.engine_base import (
    JobLogger,
    evaluate_pipeline,
    train_pipeline,
)
from blocktorch.automl.engine.sequential_engine import SequentialEngine
from blocktorch.automl.utils import AutoMLConfig
from blocktorch.pipelines import BinaryClassificationPipeline
from blocktorch.pipelines.pipeline_base import PipelineBase
from blocktorch.tests.automl_tests.dask_test_utils import (
    DaskPipelineSlow,
...
def test_init(process_pool):
    with CFClient(process_pool) as client:
        ...
        with pytest.raises(
            TypeError,
            match="Expected blocktorch.automl.engine.cf_engine.CFClient, received",
        ):
...
def test_cfengine_convenience():
    ...
    with pytest.raises(
        TypeError, match="Expected blocktorch.automl.engine.cf_engine.CFClient, received"
    ):
