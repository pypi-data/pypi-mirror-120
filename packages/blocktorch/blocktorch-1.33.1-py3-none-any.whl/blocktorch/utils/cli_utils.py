...
import blocktorch
from blocktorch.utils import get_logger


def print_info():
    """Prints information about the system, blocktorch, and dependencies of blocktorch."""
    logger = get_logger(__name__)
    logger.info("blocktorch version: %s" % blocktorch.__version__)
    logger.info("blocktorch installation directory: %s" % get_blocktorch_root())
    print_sys_info()
    print_deps()
...
def get_blocktorch_root():
    """Gets location where blocktorch is installed.

    Returns:
        Location where blocktorch is installed.
    """
    return os.path.dirname(blocktorch.__file__)
