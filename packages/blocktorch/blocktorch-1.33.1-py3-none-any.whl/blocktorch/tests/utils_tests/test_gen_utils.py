...
from blocktorch.pipelines.components import ComponentBase
...
def test_import_or_raise_errors(dummy_importlib):
    ...
    with pytest.raises(ImportError, match="Missing optional dependency '_blocktorch'"):
        import_or_raise("_blocktorch")
    with pytest.raises(
        ImportError,
        match="Missing optional dependency '_blocktorch'. Please use pip to install _blocktorch. Additional error message",
    ):
        import_or_raise("_blocktorch", "Additional error message")
    with pytest.raises(
        Exception,
...
def test_import_or_warn_errors(dummy_importlib):
    ...
    with pytest.warns(UserWarning, match="Missing optional dependency '_blocktorch'"):
        import_or_raise("_blocktorch", warning=True)
    with pytest.warns(
        UserWarning,
        match="Missing optional dependency '_blocktorch'. Please use pip to install _blocktorch. Additional error message",
    ):
        import_or_raise("_blocktorch", "Additional error message", warning=True)
    with pytest.warns(
        UserWarning,
        match="An exception occurred while trying to import `attr_error_lib`: Mock Exception executed!",
    ):
...
@patch("blocktorch.utils.gen_utils.import_or_raise")
