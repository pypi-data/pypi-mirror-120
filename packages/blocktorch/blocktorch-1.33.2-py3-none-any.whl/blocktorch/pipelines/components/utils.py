"""Utility methods for blocktorch components."""
import inspect
...
from blocktorch.exceptions import MissingComponentError
from blocktorch.model_family.utils import handle_model_family
from blocktorch.pipelines.components.component_base import ComponentBase
from blocktorch.pipelines.components.estimators.estimator import Estimator
from blocktorch.pipelines.components.transformers.transformer import Transformer
from blocktorch.problem_types import ProblemTypes, handle_problem_types
from blocktorch.utils import get_importable_subclasses
...
class WrappedSKClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, pipeline):
        """Scikit-learn classifier wrapper class. Takes an blocktorch pipeline as input and returns a scikit-learn classifier class wrapping that pipeline.

        Args:
            pipeline (PipelineBase or subclass obj): blocktorch pipeline.
        """
...
class WrappedSKRegressor(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, pipeline):
        """Scikit-learn regressor wrapper class. Takes an blocktorch pipeline as input and returns a scikit-learn regressor class wrapping that pipeline.
...
def scikit_learn_wrapped_estimator(blocktorch_obj):
    """Wraps an blocktorch object as a scikit-learn estimator."""
    from blocktorch.pipelines.pipeline_base import PipelineBase

    """Wrap an blocktorch pipeline or estimator in a scikit-learn estimator."""
    if isinstance(blocktorch_obj, PipelineBase):
        if blocktorch_obj.problem_type in [
            ProblemTypes.REGRESSION,
            ProblemTypes.TIME_SERIES_REGRESSION,
        ]:
            return WrappedSKRegressor(blocktorch_obj)
        elif (
            blocktorch_obj.problem_type == ProblemTypes.BINARY
            or blocktorch_obj.problem_type == ProblemTypes.MULTICLASS
        ):
            return WrappedSKClassifier(blocktorch_obj)
    else:
        # blocktorch Estimator
        if blocktorch_obj.supported_problem_types == [
            ProblemTypes.REGRESSION,
            ProblemTypes.TIME_SERIES_REGRESSION,
        ]:
            return WrappedSKRegressor(blocktorch_obj)
        elif blocktorch_obj.supported_problem_types == [
            ProblemTypes.BINARY,
            ...
            return WrappedSKClassifier(blocktorch_obj)
    raise ValueError("Could not wrap blocktorch object in scikit-learn wrapper.")


def generate_component_code(element):
    """Creates and returns a string that contains the Python imports and code required for running the blocktorch component.
