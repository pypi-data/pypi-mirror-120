...
from blocktorch.pipelines.components import ComponentBase, RandomForestClassifier
...
def test_scikit_learn_wrapper_invalid_problem_type():
    blocktorch_pipeline = MulticlassClassificationPipeline([RandomForestClassifier])
    blocktorch_pipeline.problem_type = None
    with pytest.raises(
        ValueError, match="Could not wrap blocktorch object in scikit-learn wrapper."
    ):
        scikit_learn_wrapped_estimator(blocktorch_pipeline)
...
def test_scikit_learn_wrapper(X_y_binary, X_y_multi, X_y_regression, ts_data):
    for estimator in [
        ...
        for problem_type in estimator.supported_problem_types:
            ...
            blocktorch_pipeline = pipeline_class([estimator])
            scikit_estimator = scikit_learn_wrapped_estimator(blocktorch_pipeline)
