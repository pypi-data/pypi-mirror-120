...
from blocktorch.objectives import BinaryClassificationObjective
...
from blocktorch.pipelines.components.ensemble.sklearn_stacked_ensemble_base import (
    _nonstackable_model_families,
)
from blocktorch.pipelines.components.utils import _all_estimators
from blocktorch.preprocessing import load_data
...
class _AutoMLTestEnv:
    ...
    def _pipeline_class(self):
        return {
            ...
            ProblemTypes.TIME_SERIES_MULTICLASS: "blocktorch.pipelines.TimeSeriesMulticlassClassificationPipeline",
            ProblemTypes.TIME_SERIES_BINARY: "blocktorch.pipelines.TimeSeriesBinaryClassificationPipeline",
        }[self.problem_type]
    ...
    def test_context(
        ...
        mock_get_names = patch(
            "blocktorch.pipelines.components.FeatureSelector.get_names", return_value=[]
        )
        ...
        if is_regression(self.problem_type):
            pipeline_to_mock = "blocktorch.pipelines.BinaryClassificationPipeline"
        ...
        mock_optimize = patch(
            "blocktorch.objectives.BinaryClassificationObjective.optimize_threshold",
            return_value=optimize_threshold_return_value,
        )

        mock_tell = patch("blocktorch.tuners.skopt_tuner.Optimizer.tell")
        ...
        mock_sleep = patch(
            "blocktorch.automl.AutoMLSearch._sleep_time", new_callable=sleep_time
        )
