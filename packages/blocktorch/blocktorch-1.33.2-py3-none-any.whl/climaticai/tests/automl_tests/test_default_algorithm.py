...
from blocktorch.automl.automl_algorithm import DefaultAlgorithm
...
@patch("blocktorch.pipelines.components.FeatureSelector.get_names")
...
@patch("blocktorch.pipelines.components.FeatureSelector.get_names")
def test_blocktorch_algo_pipeline_params(mock_get_names, X_y_binary):
    X, y = X_y_binary
    mock_get_names.return_value = ["0", "1", "2"]
...
@patch("blocktorch.pipelines.components.FeatureSelector.get_names")
def test_blocktorch_algo_custom_hyperparameters(mock_get_names, X_y_binary):
    X, y = X_y_binary
    mock_get_names.return_value = ["0", "1", "2"]
    problem_type = ProblemTypes.BINARY
    sampler_name = None
