...
from blocktorch.__main__ import cli
from blocktorch.utils.cli_utils import (
    get_blocktorch_root,
...
def test_print_cli_info_cmd(caplog):
    ...
    assert result.exit_code == 0
    assert "blocktorch version:" in caplog.text
    assert "blocktorch installation directory:" in caplog.text
...
def test_print_info(caplog):
    print_info()
    out = caplog.text
    assert "blocktorch version:" in out
    assert "blocktorch installation directory:" in out
...
def test_get_blocktorch_root(current_dir):
    root = os.path.abspath(os.path.join(current_dir, "..", ".."))
    assert get_blocktorch_root() == root
