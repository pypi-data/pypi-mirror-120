...
class StandardScaler(Transformer):
    ...
    def transform(self, X, y=None):
        ...
        X_t = BlockTransformer(func).fit_transform(X)
        # X_t_df = dd.DataFrame(X_t, columns=X.columns, index=X.index)
        return _retain_custom_types_and_initalize_woodwork(
            original_ltypes, X_t, ltypes_to_ignore=[Integer, Categorical, Boolean]
        )
