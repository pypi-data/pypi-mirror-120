...
sg.Print("Welcome to Swachh: ClointFusion's PC Cleaner", do_not_reroute_stdout=False,font='Helvetica 14',keep_on_top= True,grab_anywhere=True, no_titlebar=True)
...
def clean_site_packages():
    ...
    for fldr in folder_lst:
        ...
        if "~" in str(fldr_path.stem) or str(fldr_path.stem).startswith("-"):
            sg.Print("Deleting broken folder(s) : {fldr}")
            os.system("rmdir /q /s {fldr}")
...
def cli_launch_swachh():
    try:
        if os_name == "windows":
            print("Swachh, version 0.0.2")
            sg.Print("Swachh, version 0.0.2")
            main()
        else:
