...
def gather_experiment(exp_id, group, print_all_data=False):
    data = []
    ...
    while len(data) > 0:
        if data[0]['time_diff'] <= time.time() - sim_start_timestamp:
            ...
            del data[0]
    # elif method == "produce":
    #     print("Implement re-producer method")
...
def replay_experiment(id, group, local=False):
    """
    Create an experiment that writes all messages from the given topics to a new singular topic
    ...
    """
    if local:
        gather_experiment(id, group, True)
    else:
        experiment_replay_schema = {
            ...
            "required": [ "time", "experiment_id", "group" ]
        }
