...
def experiment_spawner():
    experiments={}
    exp_consumer = kafka_mdml_consumer(["mdml-experiment-service"], "mdml-experiment-service")
    for msg in exp_consumer.consume(overall_timeout=-1):
        ...
        if msg['value']['status'] == "on":
            ...
            t = threading.Thread(target=experiment_task, args=(msg,))
            experiments[msg['value']['experiment_id']] = t
            t.setDaemon(True)
            t.start()

def experiment_task(msg):
    exp_id = msg['value']['experiment_id']
    topics = msg['value']['topics']
    topics.append("mdml-experiment-service")
    exp_producer = kafka_mdml_producer_schemaless(f"mdml-experiment-{exp_id}")
    exp_consumer = kafka_mdml_consumer(topics, f"mdml-experiment-{exp_id}", auto_offset_reset="latest")
    num_msgs = 0
    for msg in exp_consumer.consume(overall_timeout=-1, verbose=False):
        if msg['topic'] == "mdml-experiment-service":
            if msg['value']['status'] == "off":
                exp_producer.flush()
                print("Stopping consumer loop...")
                break
        num_msgs += 1
...
def replay_experiment(exp_id, group, print_all_data=False):
    data = []
    exp_consumer = kafka_mdml_consumer_schemaless([f"mdml-experiment-{exp_id}"], group)
    print("Gathering experiment data for replaying.")
    for msg in exp_consumer.consume(overall_timeout=5, verbose=False):
        data.append(json.loads(msg['value']))
    if len(data) == 0:
        print("No experiment data found. You must use a unique group ID for each replay.")
        return
    print("Calculating simulation timeline.")
    for d in data:
        d['time'] = d['value']['time']
    data = sorted(data, key=lambda k: k['time'])
    real_start_timestamp = data[0]['time']
    for d in data:
        d['time_diff'] = d['time'] - real_start_timestamp
    if print_all_data:
        print(data)
...
def log_topics(topics, group, output_filepath, verbose=False):
    """
    Create an experiment that records the offsets of given topics
    """
    consumer = kafka_mdml_consumer(topics, group=group)
    with open(output_filepath, 'w') as f:
        for msg in consumer.consume():
            f.write(json.dumps(msg))
            if verbose:
                print(f"Logged message on topic: {msg['topic']}")
