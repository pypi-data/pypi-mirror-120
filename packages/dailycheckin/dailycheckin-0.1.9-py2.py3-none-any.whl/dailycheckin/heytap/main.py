                ...
                if data["name"] == "浏览商品":
                    ...
                    if qd["completeStatus"] == 0:
                        ...
                        if res["meta"]["code"] == 200:
                            ...
                            if res2:
                                return {"name": "每日浏览商品", "value": f"任务完成!积分领取+{qd['credits']}"}
                            else:
                                return {"name": "每日浏览商品", "value": f"领取积分奖励出错"}
                        else:
                            return {"name": "每日浏览商品", "value": f"错误，获取商品列表失败"}
                        ...
                        return {"name": "每日浏览商品", "value": f"任务已完成!"}
        except Exception as e:
            return {"name": "每日浏览商品", "value": f"错误，原因为: {e}"}
            ...
            for data in res:
                if data["name"] == "分享商品到微信":
                    ...
                    if qd["completeStatus"] == 0:
                        ...
                        if res2:
                            return {"name": "每日分享商品", "value": f"任务完成!积分领取+{qd['credits']}"}
                        else:
                            return {"name": "每日分享商品", "value": f"领取积分奖励出错!"}
                    elif qd["completeStatus"] == 1:
                        res2 = self.cashing_credits(
                            session=session,
                            cookie=cookie,
                        ...
                        return {"name": "每日分享商品", "value": f"任务已完成!"}
        except Exception as e:
            return {"name": "每日分享商品", "value": f"错误，原因为: {e}"}
