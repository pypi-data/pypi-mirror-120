...
def check_config(task_list):
    ...
    if config_path:
        ...
        with open(config_path, "r", encoding="utf-8") as f:
            try:
                data = json.load(f)
                flag, _tmp = format_data(data)
                if flag:
                    raise BaseException(
                        "config.json 配置格式已更新\n"
                        "请复制本次运行输出的 config 配置到 config.json 文件中，再重新运行\n"
                        + "-" * 100 + "\n"
                        + json.dumps(_tmp, ensure_ascii=False)
                        + "\n" + "-" * 100 + "\n" +
                        "详细配置请参考: https://sitoi.gitee.io/dailycheckin/settings/" +
                        "替换新版 config.json 配置后，即可正常运行"
                    )
...
def checkin():
    ...
    content_list.append(f"项目地址: https://github.com/Sitoi/dailycheckin")
