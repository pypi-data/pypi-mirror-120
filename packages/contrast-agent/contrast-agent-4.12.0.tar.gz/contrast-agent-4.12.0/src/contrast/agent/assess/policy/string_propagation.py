...
# TODO: PYT-1674 These are used only from the C extension hooks. It may be possible to
# remove `propagate` once we completely switch to the new architecture. The others will
# still need to be imported here.
...
def build_string_propagator_functions():
    ...
    # Handle cast propagation special case
    cast_location = policy.policy_by_name["BUILTIN.str.CAST"]
    for strtype in HOOK_TYPE_NAMES:
        _create_propagator_function(
            strtype,
            cast_location,
            cast_location.propagator_nodes[0],
            build_bytearray_cast_propagator
            if strtype == "bytearray"
            else build_generic_cast_propagator,
        )
