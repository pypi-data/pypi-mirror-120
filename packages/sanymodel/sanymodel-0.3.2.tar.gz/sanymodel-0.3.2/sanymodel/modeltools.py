...
class Figure(object):
    ...
    def format_fig(fig, auto_legend=False, date_format=None, margin=None, template='presentation') -> go.Figure:
        ...
        def generator_legend(trace):
            x = trace['xaxis']
            y = trace['yaxis']
            subplot = '{}_{}'.format(x, y)
            if x + y not in legend_group_set.keys():
                legend_group_set[subplot] = 'subplot {}'.format(len(legend_group_set) + 1)
                trace['legendgrouptitle_text'] = legend_group_set[subplot]

            trace['legendgroup'] = legend_group_set[subplot]
            trace['showlegend'] = True

        if auto_legend:
            fig.for_each_trace(generator_legend)

        new_plot = make_subplots(rows=len(legend_group_set), cols=1)

        count = 0
        for xy in legend_group_set.keys():
            count += 1
            x, y = xy.split('_')
            d = dict(xaxis=x, yaxis=y)
            traces = list(fig.select_traces(selector=d))
            new_plot.add_traces(traces, rows=count, cols=1, )
