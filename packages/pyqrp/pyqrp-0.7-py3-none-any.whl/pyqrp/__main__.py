...
if file_path := pyautogui.prompt(text="Please enter the full path to your file.", default="Python-Pilot.xlsx"):
    ...
    while isinstance(file_path, str):
        file_path = sorted(Path.home().rglob(f'{file_path}*'))[0].__str__()
        file_path = Path(file_path) if Path(file_path).exists() and Path(file_path).is_file() else file_path
