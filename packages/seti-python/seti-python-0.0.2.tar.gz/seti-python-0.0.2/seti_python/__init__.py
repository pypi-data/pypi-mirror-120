...
from seti_python.class_read import ResponseReadCash, ResponseReadCashOut, ResponseReadPayment, ResponseReadTransactions
from seti_python.wallet_class import ResponseCreateWallet
from seti_python.class_tools import BasicResponse, ResponseCreateTransaction
...
SETI_DEVELOP ="http://localhost:5001/v1/"#"https://api-ve74dbrnra-uc.a.run.app/"

class Seti():
    
    def __init__(self,path_to_private_key:str,api_key:str,public_key='',test=True) -> (None):
    ...
    def send_request(self,data:dict,sign_data:str,path:str):
        url =   SETI_DEVELOP if self.test else  SETI_PRODUCTION
        r = requests.post(url=url+path, json=data,headers={
    ...
    # Payments    
    def create_payment(self,value:int,description:str,currency:str,reference:str,redirectUrl:str,email:str,customMethods:List[str])-> (ResponseCreatePayment):
        self.is_error =False
        self.validate_currency(currency)
        self.validate_value(value)
        self.validate_payment_method(customMethods)
        data_create_payment = {
            "value":value,
    ...
    def validate_wallet(self,code:str,secret:str):
        ...
        return BasicResponse(self.send_request(data_validate_wallet,encode_data,VALIDATE_WALLET_URL))
    ...
    def create_recharger(self,value:int,secret:str):
        ...
        return ResponseCreateTransaction(self.send_request(data_create_recharger,encode_data,CREATE_RECHARGER_URL))
