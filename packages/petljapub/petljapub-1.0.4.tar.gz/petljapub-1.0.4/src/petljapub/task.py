...
class Task:
    ...
    # name of the directory where testcases are stored
    @staticmethod
    def testcases_dir_name():
        return "test-data"
    
    # full path of the directory where testcases are stored
    def testcases_dir(self):
        return os.path.join(self.build_dir(), Task.testcases_dir_name())
    
    # full path of a testcase with a given number
    def testcase_path(self, i):
        in_file = self.id() + "_" + str(i).zfill(2) + ".in"
        return os.path.join(self.testcases_dir(), in_file)
    ...
    # generate tests from examples on the problem statement
    def generate_example_testcases(self):
        logger.info("Generating tests from examples:", self.id())
        # ensure that the directory for storing test cases exists
        if not os.path.isdir(self.testcases_dir()):
            os.makedirs(self.testcases_dir())
        # process all examples given in the problem statement
        examples = self.examples(strip_md_verbatim=True)
        for i, example in enumerate(examples):
            logger.info("Generating example testcase", i)
            # extract input file
            input = os.path.join(self.testcases_dir(), "_{}-example-{:02d}.in".format(self.id(), i+1))
    ...
    def generate_testcases(self):
        ...
        p = subprocess.Popen([os.path.abspath(tgen_exe),
                              Task.testcases_dir_name()], cwd=self.build_dir())
        p.wait()
    ...
    def run_exe(self, sol, lang, testcase, timeout=2, output=None):
        ...
        if isinstance(testcase, int):
            testcase = self.testcase_path(testcase)
    ...
    def check(self, sol, lang, timeout=2, verbosity=1, force_recompile=False, reporter=None):
        ...
        # ensure that testcases exist
        if self.number_of_testcases() <= len(self.examples()):
            self.generate_example_testcases()
    ...
    def check_all(self, langs=["cpp", "cs"], sols=None, timeout=2, force_recompile=False, verbosity=1, reporter=None):
        ...
        for sol in sols:
            ...
            for lang in sol["lang"]:
                ...
                # run the check
                self.check(sol["name"], lang=lang, timeout=timeout, force_recompile=force_recompile, verbosity=verbosity, reporter=reporter)
...
if __name__ == "__main__":
    ...
    print("testcases_dir:", task.testcases_dir())
    print("testcaase 3 path:", task.testcase_path(3))
    print("testcases list:", task.testcases())
