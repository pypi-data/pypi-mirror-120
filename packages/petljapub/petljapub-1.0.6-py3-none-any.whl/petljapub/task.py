...
class Task:
    ...
    def prepare_crafted_testcases(self, crafted_dir):
        ...
        if not os.path.isdir(self.crafted_testcases_dir()):
            os.makedirs(self.crafted_testcases_dir())
        ...
        for input in glob.glob(os.path.join(self.dir(), crafted_dir, "*.in")):
            ...
            if os.path.isfile(output):
                ...
                try:
                    ...
                    run_exe(exe_file, input_file, output_file)
                except:
                    import traceback
                    traceback.print_exc()
    ...
    def run(self, sol, lang, testcase, timeout=2, output=None):
        # open in_file
        ...
        if lang == "py":
            status = run_py(self.src_file_path(sol, lang), in_file, out_file, timeout)
        else:
            exe_file = self.exe_file_path(sol, lang)
            status = run_exe(exe_file, in_file, out_file, timeout)
    ...
    def run_interactive(self, sol, lang):
        ...
        if lang == "py":
            ...
            exe_file = self.exe_file_path(sol, lang)
            return run_exe(exe_file, sys.stdin, sys.stdout)

    # Testing correctness of solutions
    
    # check the correctness of a given solution on the given testcase
    def test_on_testcase(self, sol, lang, input, expected_output, timeout=2):
    ...
    def test_on_generated_testcases(self, sol, lang, timeout=2, force_recompile=False, reporter=None):
    ...
    def test_on_example_testcases(self, sol, lang, force_recompile=False, reporter=None):
        ...
        return self.test_on_given_testcases(sol, lang, self.example_testcases(), force_recompile=force_recompile, reporter=reporter)

    # test correctness of a given solution on all crafted testcases
    def test_on_crafted_testcases(self, sol, lang, timeout=2, force_recompile=False, reporter=None):
    ...
    def test_on_all_testcases(self, sol, lang, timeout=2, force_recompile=False, reporter=None):
    ...
    def test_on_given_testcases(self, sol, lang, testcases, timeout=2, force_recompile=False, reporter=None):
    ...
    def test_all(self, langs=["cpp", "cs"], sols=None, timeout=2, force_recompile=False, reporter=None):
