...
def test(ctx, lang, sol=0, recompile=True, timeout=2, verbosity=3):
    """
...
@task
def t(ctx, lang, sol=0, recompile=True, timeout=2, verbosity=3):
    """
...
@task(help={'recompile': "Recompile all solutions (and the custom checker if it exists) before checking",
            ...
            'verbosity': "Level of messages printed to the user (from 0/quiet to 5/full)"})
def test_all(ctx, recompile=True, timeout=2, verbosity=3):
    """
...
def yaml_test(ctx, yaml, tasks_dir=None, pub_dir=None, recompile=False, timeout=2, verbosity=3):
    """
