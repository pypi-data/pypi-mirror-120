...
def run(ctx, lang, sol=0, testcase=-1):
    ...
    task = Task(os.getcwd())
    if testcase >= 1:
        task.run_exe(solution(sol), lang, testcase, output="stdout")
    else:
        task.run_exe_interactive(solution(sol), lang)
...
def tests_zip(ctx):
    ...
    task.generate_testcases()
    i = 1
    for testcase_in in (task.example_testcases() + task.generated_testcases()):
...
@task(help={
    ...
    'timeout': "Timeout in seconds for each testcase (2 seconds is the default value)",
    'verbosity': "Level of messages printed to the user (from 0/quiet to 3/full)"
})
def test(ctx, lang, sol=0, recompile=False, timeout=2, verbosity=1):
    """
    ...
    task = Task(os.getcwd())
    task.check_all_testcases("ex" + str(sol), lang, force_recompile=recompile, verbosity=verbosity, timeout=timeout)

@task
def t(ctx, lang, sol=0, recompile=False, verbosity=1, timeout=2):
    """
    Test the given solution for the current task (shortcut for 'petljapub test')
    """
    test(ctx, lang, sol, recompile, verbosity, timeout)
...
@task(help={'recompile': "Recompile all solutions (and the custom checker if it exists) before checking",
            ...
            'verbosity': "Level of messages printed to the user (from 0/quiet to 3/full)"})
def test_all(ctx, recompile=False, timeout=2, verbosity=1):
    """
...
def test_examples(ctx, lang, sol=0, recompile=False):
    """
    ...
    if not ensure_task_dir():
        return
    task = Task(os.getcwd())
    task.check_example_testcases("ex" + str(sol), lang, recompile)
...
def show_runtime(ctx, force_run=False, timeout=2, repeat=3, plot=False, sol=None, lang=None):
    """
...
def yaml_test(ctx, yaml, tasks_dir=None, pub_dir=None, recompile=False, timeout=2, verbosity=1):
    """
...
def yaml_runtime(ctx, yaml, tasks_dir=None, pub_dir=None, force_run=False, timeout=2, repeat=3):
    """
