...
def get_version(command, options):
    try: 
        result = subprocess.run([command] + options, stdout=subprocess.PIPE)
...
def configure_compilers():
    ...
    if check_compiler("mono", "mono", ["--version"], "6.8") and not cs_compiler:
        cs_compiler = "MONO"
    if check_compiler("MSVC#", "csc", ["--version"], "3.1") and not cs_compiler:
        cs_compiler = "MSVC#"
    if check_compiler(".NET", "dotnet", ["--version"], "3.1") and not cs_compiler:
        cs_compiler = ".NET"
