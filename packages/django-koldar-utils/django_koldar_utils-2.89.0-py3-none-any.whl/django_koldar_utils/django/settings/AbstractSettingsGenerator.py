...
class AbstractSettingsGenerator(abc.ABC):
    ...
    def _configure_cors(self, original: AttrDict):
        """
        Configure the CORS for the django server.
...
class AbstractGraphQLSettingsGeneratorMixin(AbstractSettingsGenerator, abc.ABC):
    """
    Configure the project in such a way that it can use graphiql and graphene.
    """
    ...
    def _configure_app(self, settings_name: str, app_name: str, original_data: any) -> any:
        if settings_name == "GRAPHENE_AUTHENTICATION":
            ...
            raise ValueError(f"cannot configure app {settings_name}!")


class StandardGraphQLSettingsGenerator(AbstractGraphQLSettingsGeneratorixin):
    """
