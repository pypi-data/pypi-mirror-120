...
class Client(Recall, Socket):
    ...
    def gen_captcha(self): val = "".join(random.choices(string.ascii_uppercase + string.ascii_lowercase + "_-", k=462)).replace("--", "-"); return val
    def handle(self, data): return self.solve(data)
    def change_lang(self, lang: str = "ar-SY"): self.headers["NDCLANG"] = lang[0:lang.index("-")]; self.headers["Accept-Language"] = lang
    ...
    def login(self, email: str, password: str, asWeb: bool = True):
        if asWeb:
            ...
            try:
                ...
                headers.sid = self.sid
                try: self.sid_login(self.sid.replace("sid=", ""))
                except: pass
