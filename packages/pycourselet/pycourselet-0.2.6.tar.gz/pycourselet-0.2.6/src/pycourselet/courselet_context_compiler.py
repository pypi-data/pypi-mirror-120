...
class CourseletContextCompiler:
    ...
    def _change_line_feed(self, filename):
...
class CourseletXmlContextCompiler(CourseletContextCompiler):
    ...
    def end(self, ctx: ContextManager, **kwargs):
        ...
        filename = os.path.join(self.build_dir, f'courselet.xml')
        root.write(filename, xml_declaration=True, encoding='UTF-8')
        self._change_line_feed(filename)
    ...
    def end_page_context(self, context: PageContext, **kwargs):
        ...
        if page_context:
            ...
            filename = os.path.join(self.page_dir, f'{context.name}.xml')
            root.write(filename, xml_declaration=True, encoding='UTF-8')
            self._change_line_feed(filename)
