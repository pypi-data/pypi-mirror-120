...
class TemplateContext:
    ...
    def load_context_variables(self) -> Dict[str, Any]:
        ...
        if "encrypt" not in ret:
            ret["encrypt"] = self.crypto.encrypt
        if "decrypt" not in ret:
            ret["decrypt"] = self.crypto.decrypt
        return ret
    ...
    def safe(self, request: DiscoveryRequest) -> Dict[str, Any]:
        ...
        if request.hide_private_keys:
            ret["encrypt"] = self.disabled_suite.encrypt
            ret["decrypt"] = self.disabled_suite.decrypt
        return ret
