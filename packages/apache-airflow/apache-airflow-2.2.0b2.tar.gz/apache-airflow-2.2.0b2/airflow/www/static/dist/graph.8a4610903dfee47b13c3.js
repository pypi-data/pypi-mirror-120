...
function draw() {
  ...
  d3.selectAll('g.node').on('click', nodeId => {
    ...
    if (node.children !== undefined && Object.keys(node.children).length > 0) {
      ...
      let tryNumber;
      if (nodeId in taskInstances) tryNumber = taskInstances[nodeId].try_number;else tryNumber = 0;
...
function groupTooltip(nodeId, tis) {
  ...
  Object.entries(numMap).forEach(([key, val]) => {
    tt += `<strong>${Object(_main__WEBPACK_IMPORTED_MODULE_1__["escapeHtml"])(key)}:</strong> ${val} <br>`;
  });
  return tt;
