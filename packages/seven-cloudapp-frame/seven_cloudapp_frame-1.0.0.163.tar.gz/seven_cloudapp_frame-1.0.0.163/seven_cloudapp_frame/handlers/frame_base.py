...
@LastEditTime: 2021-09-18 09:06:02
...
class ClientBaseHandler(FrameBaseHandler):
    ...
    def prepare(self):
        ...
        if not dict_param["open_id"]:
            dict_param["open_id"] = self.get_param("tb_user_id") if self.get_param("tb_user_id") else self.get_param("user_id")
    ...
    def get_user_nick(self):
        ...
        if plat_type == 1 and self.get_param("source_app_id") == "":
    ...
    def get_source_app_id(self):
        ...
        plat_type = config.get_value("plat_type", 1)  # 平台类型 1淘宝2微信3抖音
        if plat_type == 1 and source_app_id == config.get_value("client_template_id"):
            #前端（在IDE上返回前端模板id，在千牛上返回正确的id）
