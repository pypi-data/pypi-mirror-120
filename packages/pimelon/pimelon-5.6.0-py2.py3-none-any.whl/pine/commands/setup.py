...
def setup_supervisor(user=None, yes=False, skip_redis=False):
	...
	pine.config.supervisor.generate_supervisor_config(pine_path=".", user=user, yes=yes, skip_redis=skip_redis)
...
def setup_letsencrypt(site, custom_domain, non_interactive):
	...
	pine.config.lets_encrypt.setup_letsencrypt(site, custom_domain, pine_path=".", interactive=not non_interactive)
...
def setup_wildcard_ssl(domain, email, exclude_base_domain):
	...
	pine.config.lets_encrypt.setup_wildcard_ssl(domain, email, pine_path=".", exclude_base_domain=exclude_base_domain)
...
def setup_manager(yes=False, port=23624, domain=None):
	...
	if conf.get("restart_supervisor_on_update") or conf.get("restart_systemd_on_update"):
		...
		if domain not in get_sites(pine_path):
...
def add_domain(domain, site=None, ssl_certificate=None, ssl_certificate_key=None):
	...
	pine.config.site_config.add_domain(site, domain, ssl_certificate, ssl_certificate_key, pine_path=".")
...
def remove_domain(domain, site=None):
	...
	pine.config.site_config.remove_domain(site, domain, pine_path=".")
...
def sync_domains(domain=None, site=None):
	...
	changed = pine.config.site_config.sync_domains(site, domains, pine_path=".")
