import pandas as pd
from nflfastpy.config import BASE_URL, ROSTER_URL, ROSTER_2020_URL, TEAM_LOGO_URL, SCHEDULE_URL, SEASON
...
def load_schedule_data(year=SEASON):
    """
    Load schedule data going back to 1999
    """

    if type(year) is not int:
        raise TypeError(f'Please provide an integer between 1999 and {SEASON} for the year argument.')

    if year < 1999 or year > SEASON:
        raise SeasonNotFoundError(f'Schedule data is only available from 1999 to {SEASON}')
