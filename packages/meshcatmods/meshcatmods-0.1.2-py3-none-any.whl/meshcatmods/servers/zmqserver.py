...
def start_zmq_server_as_subprocess(zmq_url=None, server_args=[]):
    ...
    args = [sys.executable, "-u", "-m", "meshcatmods.servers.zmqserver"]
...
class WebSocketHandler(tornado.websocket.WebSocketHandler):
    def __init__(self, *args, **kwargs):
        ...
        print("websockethandler started")
...
class Client(object):
    def __init__(self, url, timeout):
        ...
        self.ws = None
        self.connect()
        pc = tornado.ioloop.PeriodicCallback(self.keep_alive, 20000)
        pc.start()
        self.ioloop.start()
    ...
    def connect(self):
        ...
        try:
            ...
            self.ws.write_message("VIEW")
            print("VIEW connected to model websocket")
        except Exception as e:
...
class ZMQWebSocketBridge(object):
    ...
    def __init__(self, zmq_url=None, host="127.0.0.1", port=None,
        ...
        # self.web_url = "{protocol}//{host}:{port}/".format(
            # protocol=protocol, host=self.host, port=self.fileserver_port)
        ...
        #     websocket.write_message("VIEW")
        # model_url = "ws://{host}:{port}/".format(
        #     host=self.host, port=int(self.fileserver_port)-1) # model is on a different port
        # client = tornado.websocket.websocket_connect(model_url)
        # client.write_message("VIEW")

    def make_app(self):
        return tornado.web.Application([
            (r"/static/(.*)", StaticFileHandlerNoCache, {"path": VIEWER_ROOT, "default_filename": VIEWER_HTML}),
            # (r"/(.*)", StaticFileHandlerNoCache, {"path": VIEWER_ROOT, "default_filename": VIEWER_HTML}),
...
def main():
    ...
    parser.add_argument('--web-port', '-w', type=str, nargs="?", default=None)
...
is very useful if you would like to make your meshcat server public.""")
    ...
    bridge = ZMQWebSocketBridge(zmq_url=results.zmq_url,port=results.web_port,
    ...
    model_ws = "ws://127.0.0.1:{port}/".format(
            port=int(results.web_port)-1) # model is on a different port
    client = Client(model_ws, 5)
    client.connect()
    # client.write_message("VIEW")
    # client.close()
    if results.open:
