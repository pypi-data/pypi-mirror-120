...
"""
    ...
    def Execute(self, opt, args):
        ...
        newfilename = os.path.join(cur_dir, "solutions", solution_name, '.package.yaml')
        if not os.path.isfile(newfilename):
            # copy package.yaml to .package.yaml
            shutil.copy2(filename, newfilename)

        if os.path.isfile(newfilename):
            pack = Package(newfilename)
            for d in pack.depends:
                if opt.board in d:
                    pack.depends.remove(d)
            pack.depends.append({opt.board: "master"})
            pack.hw_info.board_name = opt.board
            pack.save(newfilename)
        else:
            put_string("File \"%s\" is not existed" % newfilename)
            remove_project_files(cur_dir, created_newdir, inited_workspace)
            exit(-1)
