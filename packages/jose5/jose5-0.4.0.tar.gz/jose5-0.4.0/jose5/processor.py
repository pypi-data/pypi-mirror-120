...
def varsubst(V, s):
    ...
    if isinstance(s, str):
        for match in varre.findall(s):
            ...
            elif isinstance(Vptr, int) or isinstance(Vptr, float) or isinstance(Vptr, list) or isinstance(Vptr, dict):
                if match == s:
                    s = Vptr
                else:
                    s = s.replace(match, str(Vptr))

            if len(vartoks) == 1 and vartoks[0] == "__str":
                s = str(s)
                vartoks.pop()

            # Path still has nodes, error
            if vartoks:
                s = old_s
