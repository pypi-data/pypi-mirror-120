...
class TelemetryLogger:
    def __init__(self, folder, telemetry_api, telemetry_ext):
        # assert isinstance(kdecanapi, KdeCanAPI)
        self.telemetry_api = telemetry_api

        # Test for the methods we need telemetry_api to have
        assert hasattr(self.telemetry_api, 'close')
        assert hasattr(self.telemetry_api, 'get_data')
    ...
    def live_data(self, sampling_period, log_header):
        ...
        while True:
            log_data = self.telemetry_api.get_data()
