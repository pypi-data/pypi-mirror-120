...
class SatLegacyParser(object):
    ...
    def p_loop(self, p):
        ...
        if var_stack != stack:
            self << SatSyntaxError(
                "Loop and domain mismatch.", target=(var_stack ^ stack).pop()
            )
            self.checkpoint()
    ...
    def p_penalties(self, p):
        ...
        for name, level, cor in penalties:  # pylint: disable=unused-variable
            if name not in self.constraint_table:
                self < SatWarning(
                    f"Undefined constraint group {name} to be ignored.", target=name
                )
            else:
                index = 0
                for type_, name, loops, expr in self.constraint_table[name]:
                    if str(type_) == CONS_OPT and level == Number("0"):
                        level = None
                    commands.append(
                        (DEF_CONSTRAINT, type_, f"{name}-{index}", loops, expr, level)
                    )
                    index += 1
