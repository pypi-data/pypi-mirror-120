...
def create_column(feature_df, parts_num):
    ...
    proximity_to_wake_time_column = [
        (y - x).total_seconds() / 60 if (type(x) == float) or (type(y) == float) else np.nan for x, y in
    ...
    wake_duration = [(y - x).total_seconds() / 60 if (type(x) == float) or (type(y) == float) else np.nan for x, y in
                     zip(wake_time_datetime_list, sleep_time_datetime_list)]
    duration_per_part = [x // parts_num if type(x) == float else np.nan for x in wake_duration]
    parts_of_waking_hour_column = [y // x if (type(x) == float) or (type(y) == float) else np.nan for x, y in
