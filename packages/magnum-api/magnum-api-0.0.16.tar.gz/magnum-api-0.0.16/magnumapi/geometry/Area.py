...
class Area:
   ...
   Attributes:
    ...
    def is_outside_of_first_quadrant(area: "Area", eps=1e-30) -> bool:
        ...
        """
        is_x_nonegative = all([(abs(line.p1.x) >= eps) and (abs(line.p2.x) >= eps) for line in area.lines])
        is_y_nonegative = all([(abs(line.p1.y) >= eps) and (abs(line.p2.y) >= eps) for line in area.lines])
